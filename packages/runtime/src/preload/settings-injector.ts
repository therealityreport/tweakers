...
interface RegisteredPage {
  ...
  /** Identity token prevents an old handle from unregistering a replacement. */
...
interface InjectorState {
  ...
  sectionTokens: Map<string, symbol>;
  ...
  /** Our "Tweakers" nav group (Config/Tweaks). */
  ...
  replacementObserver: MutationObserver | null;
  ...
  tweaksPageFilter: TweaksPageFilter;
...
function scopeSettingsObserver(outcome: SettingsProbeOutcome): void {
  ...
  if (sidebarRoot) {
    ...
    state.replacementObserver = new MutationObserver((records) => {
      ...
      if (!mutationRecordsContainRemovedRoot(records, sidebarRoot)) return;
      state.sidebarRoot = null;
      state.fingerprint = null;
      state.probeScheduler?.request({ immediate: true, resetBackoff: true, wake: true });
    });
...
function mutationRecordsContainSettingsMarkers(records: MutationRecord[]): boolean {
  for (const record of records) {
    for (const node of Array.from(record.addedNodes)) {
      if (node instanceof Element && node.matches("[data-settings-panel-slug]")) return true;
      if ((node instanceof Element || node instanceof DocumentFragment)
        && node.querySelector("[data-settings-panel-slug]")) return true;
    }
  }
  return false;
}

function mutationRecordsContainRemovedRoot(records: MutationRecord[], root: Node): boolean {
  return records.some((record) =>
    Array.from(record.removedNodes).some((node) =>
      node === root || (node instanceof Node && node.contains(root)),
    ),
  );
}
...
export function registerPage(
  ...
  const entry: RegisteredPage = { id, tweakId, manifest, page, registrationToken };
...
export function updateListedTweakLifecycle(id: string, lifecycle: SettingsNavigationItem["lifecycle"], error?: string): void {
  const tweak = state.listedTweaks.find((item) => item.manifest.id === id);
  if (!tweak) return;
  tweak.lifecycleOverride = lifecycle;
  if (error) tweak.health = { status: lifecycle === "quarantined" ? "quarantined" : "failed", updatedAt: new Date().toISOString(), error };
...
function settingsNavigationItems(): SettingsNavigationItem[] {
...
function settingsNavigationItem(tweakId: string): SettingsNavigationItem | null {
  return settingsNavigationItems().find((item) => item.tweakId === tweakId) ?? null;
}

function registeredPagesForTweak(tweakId: string): RegisteredPage[] {
  return [...state.pages.values()].filter((entry) => entry.tweakId === tweakId);
}

function lifecycleLabel(lifecycle: SettingsNavigationItem["lifecycle"], warning?: string | null): string {
...
let navGroupInjections: number[] = [];
let navGroupInjectionSuppressedUntil = 0;

function isNavGroupInjectionSuppressed(): boolean {
  return Date.now() < navGroupInjectionSuppressedUntil;
}

function noteNavGroupInjection(outer: HTMLElement): void {
  const now = Date.now();
...
function bindSettingsSearch(root: HTMLElement): void {
  const input = root.closest("aside, nav, [role='navigation'], div")?.parentElement
    ?.querySelector<HTMLInputElement>("input[placeholder*='Search settings' i]")
    ?? document.querySelector<HTMLInputElement>("input[placeholder*='Search settings' i]");
...
function constrainSidebarIconSvg(icon: Element | null | undefined, size = 20): void {
  if (!icon) return;
  icon.setAttribute("width", String(size));
  icon.setAttribute("height", String(size));
  const style = (icon as unknown as { style?: CSSStyleDeclaration }).style;
  if (style) {
    style.width = `${size}px`;
...
function rerender(): void {
  ...
  if (ap.kind === "registered") {
    ...
    if (item.warning) root.sectionsWrap.appendChild(tweakPageWarning(item.warning));
    if (!entries.length) {
  ...
  const root = panelShell(
    title,
    subtitle,
    ap.kind === "tweaks" ? { width: "plugins" } : undefined,
  );
  host.appendChild(root.outer);
...
function tweakLifecycleBadge(item: SettingsNavigationItem): HTMLElement {
  const badge = document.createElement("span");
...
function tweakPageWarning(message: string): HTMLElement {
...
function renderFallbackTweakPage(root: HTMLElement, item: SettingsNavigationItem): void {
  const section = document.createElement("section");
  section.className = "flex flex-col gap-2";
...
function rowCopy(title: string, detail: string): HTMLElement {
...
function renderCodexVersionsSection(
  ...
  let generation = 0;
  const schedulePoll = (snapshot: CodexVersionsSnapshot) => {
    if (polling) clearTimeout(polling);
    polling = null;
    if (!actionInFlight && !codexProgressBusy(snapshot.installProgress)) return;
    polling = setTimeout(() => {
      if (card.isConnected) void load(false);
    }, 900);
  };
  const requestReload: CodexUiReload = (mode) => {
...
function renderCodexVersionsCard(
  card: HTMLElement,
  ...
  if (snapshot.fromCache || snapshot.stale) {
    ...
    card.appendChild(rowSimple(
      snapshot.stale ? "Cached information (refresh needed)" : "Cached information",
...
function codexFeatureRow(
  feature: CodexFeatureEntry,
  lane: CodexCliLane,
  busy: boolean,
  reload: CodexUiReload,
): HTMLElement {
...
function codexFeatureStage(feature: CodexFeatureEntry, lane: CodexCliLane): CodexFeatureStage | null {
  return feature.stages[lane];
}

function codexFeatureEnabled(feature: CodexFeatureEntry, lane: CodexCliLane): boolean | null {
  return feature.enabled[lane];
}

function codexFeatureMutable(feature: CodexFeatureEntry, lane: CodexCliLane): boolean {
  const stage = codexFeatureStage(feature, lane);
...
function codexFilterSelect(label: string, options: string[]): HTMLSelectElement {
...
function codexNeutralBadge(text: string): HTMLElement {
  const badge = document.createElement("span");
...
function codexProgressBusy(progress: CodexInstallProgress): boolean {
  return !["idle", "complete", "failed"].includes(progress.phase);
}

function isCodexSnapshotStale(snapshot: CodexVersionsSnapshot): boolean {
  return snapshot.stale;
}

function installedLatestSummary(
...
function codexScopedError(
  snapshot: CodexVersionsSnapshot,
  scope: "desktop" | CodexCliLane,
): string | null {
  return snapshot.errors[scope] ?? null;
}

function isSafeCodexGithubUrl(url: string | null | undefined): boolean {
  if (!url) return false;
  try {
...
function openCodexGithubUrl(url: string): void {
...
function runCodexAction(
  ...
  const invoke = payload === undefined ? ipcRenderer.invoke(channel) : ipcRenderer.invoke(channel, payload);
  void invoke
    .catch((error) => {
...
function selfUpdateSummary(state: SelfUpdateState | null): string {
  if (!state) return "No automatic Tweakers update has run yet.";
  ...
  if (state.status === "failed" && /404|no (?:published |github )?release/i.test(state.error ?? "")) return `Source checkout is current as of ${checked}; no published release exists yet.`;
  if (state.status === "failed") return `Update check needs attention (${checked}). ${state.error ?? "Unknown error"}`;
...
function tweaksPageFilterLabel(filter: TweaksPageFilter): string {
...
function tweakAvatar(tweak: ListedTweak): HTMLElement {
...
function tweakAuthorName(author: TweakManifest["author"]): string | null {
...
function tweakStatusPill(tweak: ListedTweak): HTMLElement {
  const labels: Record<TweakStatus, string> = {
    installed: "Installed",
...
function panelShell(
  title: string,
  subtitle?: string,
  options?: { wide?: boolean; width?: "default" | "plugins" | "wide" },
): {
  outer: HTMLElement;
  sectionsWrap: HTMLElement;
  subtitle?: HTMLElement;
