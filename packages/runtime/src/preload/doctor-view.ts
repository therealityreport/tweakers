// Mirrors packages/sdk/src/doctor.ts without making the preload build depend on
// generated SDK declarations. The runtime still validates every response at
// the IPC boundary before rendering or enabling an action.
...
export interface DoctorReportV1 {
  ...
  adoption?: {
    ...
    report: { updaterEvidence?: { schemaVersion: 1; protocolFingerprint: string;
      interfaces: Array<{method: string; owner: string; status: string; reasons: string[]}>;
      checks?: Array<{id: string; state: string; summary: string; scope: string}>;
      upstream: {status: string; summary: string; url?: string; digest?: string} }; changes: DoctorChangeV1[]; changelog?: DoctorChangelogV1; sourceVersions?: { before: { version: string | null; build: string | null }; after: { version: string | null; build: string | null } }; limitations: string[]; coverage: { total: number; classified: number; unresolved: number }; fingerprint: string };
    decisions: Array<{ changeId: string; choice: "accept" | "acknowledge_unknown" | "preserve" | "override"; overrideId?: string }>;
    observations: Array<{ changeId: string; before: string; after: string; conditions: string; outcome: "matches" | "differs" | "unavailable"; recordedAt: string; source: "user_manual" }>;
    blockers: string[];
    fingerprint: string;
  } | null;
  findings: DoctorFindingV1[];
  actions: Array<{ id: DoctorActionV1; label: string; enabled: boolean; blockers: string[] }>;
}
...
export interface DoctorChangeV1 {
  ...
  explanation?: { summary: string; evidenceReferences: Array<{ id: string; sha256: string }>; sourceReferences: Array<{ path: string; sha256: string }> };
  unknownPolicy?: "acknowledgment" | "blocking";
  technicalOnly: boolean;
  evidence: Array<{ kind?: "static" | "native_interaction" | "upstream_documentation"; artifact: string; path: string; beforeSha256: string | null; afterSha256: string | null; detail: string }>;
  dependencies: string[];
  compatibility: string[];
  overrides: Array<{ id: string; label: string; verificationFingerprint: string }>;
}
...
export interface DoctorChangelogEntryV1 {
  ...
  evidenceReferences: Array<{ id: string; sha256: string }>;
  limitations: string[];
...
export interface DoctorChangelogV1 {
  ...
  unresolved: Array<{ groupId: string; reason: string }>;
}
...
export interface DoctorPresentation {
  ...
  actionBlockers: Array<{ action: DoctorActionV1; blockers: string[] }>;
}
...
export interface DoctorViewOptions {
  invoke(
    channel: "tweaker:doctor-status" | "tweaker:doctor-action",
    request?: DoctorActionRequestV1,
  ): Promise<unknown>;
  copyText(text: string): Promise<unknown>;
}

export function doctorPresentation(report: DoctorReportV1): DoctorPresentation {
  ...
  return {
    ...
    installationFindings: report.findings.filter((finding) =>
      finding.stage === "installation" || finding.stage === "storage" || finding.stage === "broker"
    ),
    compatibilityFindings: report.findings.filter((finding) =>
      finding.stage === "compatibility" || finding.stage === "candidate"
    ),
...
export function createDoctorController(options: DoctorControllerOptions): DoctorController {
  let disposed = false;
  let revision = 0;
  ...
  const publish = (): void => {
    if (!disposed) options.onChange(copySnapshot(snapshot));
  };
  const acceptReport = (value: unknown): DoctorReportV1 => {
    if (!isDoctorReport(value)) throw new Error("invalid-doctor-report");
    return value;
  };

  async function refresh(): Promise<void> {
    if (disposed || !options.isMounted()) return;
    ...
    try {
      ...
      if (disposed || requestRevision !== revision || !options.isMounted()) return;
  ...
  async function perform(action: DoctorActionV1, details: Pick<DoctorActionRequestV1, "decision" | "observation"> = {}): Promise<boolean> {
    if (disposed || !options.isMounted() || snapshot.busyAction !== null) return false;
    const report = snapshot.report;
    const declaredAction = report?.actions.find((candidate) => candidate.id === action);
    if (!report || !declaredAction?.enabled) return false;
    const requestRevision = ++revision;
    ...
    try {
      ...
      if (disposed || requestRevision !== revision || !options.isMounted()) return false;
  ...
  return {
    get snapshot(): DoctorViewSnapshot {
...
export function mountDoctorView(root: HTMLElement, options: DoctorViewOptions): () => void {
  ...
  const render = (snapshot: DoctorViewSnapshot): void => {
    ...
    if (snapshot.report) renderReport(content, snapshot, controller, options);
    if (snapshot.error) {
...
function renderReport(
  ...
  if (compatibilityWorkflow) {
    if (compatibility) renderCompatibility(content, report.update);
    else renderCompatibilityPending(content);
    if (report.adoption) {
  ...
  } else if (report.adoption) renderBehavioralChangelog(content, report, snapshot, controller);
  const attentionRows = [
  ...
  if (!compatibilityWorkflow && report.update.review?.pause) attentionRows.push(doctorRow(
    "Review paused",
    `${report.update.review.pause.message}\nNext: ${pauseActionLabel(report.update.review.pause.action)}.`,
    doctorBadge("warn", "Action needed"),
  ));
  if (attentionRows.length) content.appendChild(doctorGroup("What needs attention", attentionRows));

  const advanced = document.createElement("details");
  advanced.className = "p-3 text-sm";
  const advancedSummary = document.createElement("summary"); advancedSummary.textContent = "Show technical report";
  const advancedText = document.createElement("pre"); advancedText.className = "mt-2 whitespace-pre-wrap break-words text-token-text-secondary";
  ...
  if (report.update.handoff && report.update.execution?.status !== "running") {
    ...
    const copy = doctorButton(snapshot.copied ? "Copied" : "Copy Handoff", () => {
      if (!report.update.handoff) return;
  ...
  for (const action of report.actions.filter(action => ["reconnect", "scan", "repair", "retry", "update"].includes(action.id))) {
    ...
    if (!action.enabled && action.blockers.length > 0) button.title = action.blockers.join("\n");
    actionRow.appendChild(button);
  }
  content.appendChild(doctorGroup("Actions", [actionRow]));
}

function renderCompatibilityPending(content: HTMLElement): void {
  content.appendChild(doctorGroup("Compatibility", [doctorRow(
    "Candidate compatibility",
    "Checking compatibility evidence for this candidate. Installation remains unavailable until verification publishes a result.",
    doctorBadge("warn", "Checking compatibility"),
  )]));
}

export function isCompatibilityWorkflowInProgress(update: DoctorReportV1["update"]): boolean {
  return update.state === "checking" && ["preparing", "resuming", "comparing", "applying_patches", "checking", "repairing"].includes(update.phase);
}

function renderCompatibility(content: HTMLElement, update: DoctorReportV1["update"]): void {
  ...
  if (compatibility.postInstallChecks.length) rows.push(doctorRow("After installation", compatibility.postInstallChecks.join("\n")));
  content.appendChild(doctorGroup("Compatibility", rows));
}

export function compatibilityPresentation(update: DoctorReportV1["update"]): DoctorCompatibilityPresentation {
  const compatibility = update.compatibility;
  if (!compatibility) throw new Error("missing-compatibility");
...
export function changelogDecisionGroups(report: Pick<NonNullable<DoctorReportV1["adoption"]>["report"], "changes" | "changelog">, id: string): string[] {
  ...
  while (changed) {
    ...
    for (const sibling of report.changelog?.entries ?? []) {
      if ([...sibling.analysisGroupIds, ...(sibling.decisionDependencies ?? [])].some((groupId) => ids.has(groupId))) {
        for (const groupId of [...sibling.analysisGroupIds, ...(sibling.decisionDependencies ?? [])]) ids.add(groupId);
      }
    }
...
function renderBehavioralChangelog(content: HTMLElement, report: DoctorReportV1, snapshot: DoctorViewSnapshot, controller: DoctorController): void {
  ...
  if (!log) {
    content.appendChild(doctorGroup("What’s new", [doctorRow(
      "Behavioral changelog incomplete",
      "This historical report has technical analysis, but no validated behavioral changelog. It cannot clear the update for installation.",
      doctorBadge("warn", "Incomplete"),
    )]));
  } else {
    for (const category of ["Added", "Changed", "Fixed", "Removed", "Deprecated", "Security"] as const) {
      const entries = log.entries.filter((entry) => entry.category === category);
      if (entries.length) content.appendChild(doctorGroup(category, entries.map((entry) => changelogEntryRow(entry, review, report, snapshot, controller))));
    }
    if (log.unresolved.length) {
      ...
      const details = document.createElement("details"); details.className = "p-3 text-sm";
      const summary = document.createElement("summary"); summary.textContent = "Show pending areas and next actions";
      const reasons = document.createElement("div"); reasons.className = "mt-2 whitespace-pre-wrap break-words text-token-text-secondary";
    ...
    if (log.entries.length === 0 && review.state === "review_required") content.appendChild(doctorGroup("What’s new", [doctorRow("No behavioral entries are ready", "The review is required because its behavioral explanation is incomplete. Open Behavioral changelog gaps or Technical review, then resolve the listed evidence before continuing.", doctorBadge("warn", "Review required"))]));
  }
  const technical = document.createElement("details");
  technical.className = "p-3 text-sm";
  const summary = document.createElement("summary"); summary.className = "cursor-interaction text-token-text-primary";
  summary.textContent = "Inspect technical analysis groups and review decisions";
  const body = document.createElement("div"); body.className = "mt-2 flex flex-col gap-3 whitespace-pre-wrap break-words text-token-text-secondary";
  const technicalText = document.createElement("pre"); technicalText.className = "m-0 whitespace-pre-wrap break-words";
  ...
  for (const unresolved of review.report.changelog?.unresolved ?? []) {
    ...
    if (!group || group.status !== "unknown" || group.unknownPolicy !== "acknowledgment") continue;
...
function pendingAnalysisArea(
  report: NonNullable<DoctorReportV1["adoption"]>["report"],
  unresolved: DoctorChangelogV1["unresolved"][number],
): { title: string; reason: string; nextAction: string } {
...
function pendingAreaSummary(areas: Array<{ title: string }>): string {
...
function readableNextAction(value: string): string | null {
  if (/^[a-z0-9]+(?:-[a-z0-9]+)+$/i.test(value.trim())) return null;
  return value;
}

function changelogEntryRow(entry: DoctorChangelogEntryV1, review: NonNullable<DoctorReportV1["adoption"]>, report: DoctorReportV1, snapshot: DoctorViewSnapshot, controller: DoctorController): HTMLElement {
  ...
  const row = document.createElement("div"); row.className = "flex min-w-0 flex-col gap-3 p-3";
  ...
  const scopeDisclosure = document.createElement("details"); scopeDisclosure.className = "text-sm text-token-text-secondary";
  const scopeSummary = document.createElement("summary"); scopeSummary.className = "cursor-interaction text-token-text-primary"; scopeSummary.textContent = "Review decision scope";
  const scopeText = document.createElement("div"); scopeText.className = "mt-2 whitespace-pre-wrap break-words";
  ...
  scopeDisclosure.append(scopeSummary, scopeText); row.appendChild(scopeDisclosure);
  ...
  const action = (label: string, choice: "accept" | "preserve" | "override", overrideId?: string): void => {
    ...
    button.disabled = !canDecide || (choice === "accept" && (requiredUnknown || unacknowledgedEligible)); actions.appendChild(button);
  };
  action("Accept", "accept"); action("Request preservation", "preserve");
  if (unacknowledgedEligible) {
    ...
    acknowledge.disabled = !canDecide; actions.appendChild(acknowledge);
  }
  if (scope.length === 1 && groups[0]?.overrides.length === 1) action(`Use override: ${groups[0].overrides[0].label}`, "override", groups[0].overrides[0].id);
  ...
  const observe = doctorButton("Record manual observation", () => {
    const before = window.prompt("What did you observe before?"); if (before === null) return;
    const after = window.prompt("What did you observe after?"); if (after === null) return;
    const conditions = window.prompt("Conditions for this observation:"); if (conditions === null) return;
  ...
  observe.disabled = !canObserve; observe.title = canObserve ? "Records user-supplied evidence for this one analysis group." : "Observation requires exactly one underlying analysis group."; actions.appendChild(observe);
  row.appendChild(actions); return row;
}

function evidenceLabel(status: DoctorChangelogEntryV1["status"]): string {
  if (status === "observed") return "Observed in app";
  if (status === "documented_upstream") return "Documented upstream";
  return "Inferred from code";
}

function evidenceMeaning(kind: DoctorChangeV1["evidence"][number]["kind"]): string {
  if (kind === "native_interaction") return "Observed through native interaction.";
  if (kind === "upstream_documentation") return "Upstream documentation; not observed in the native app.";
  return "Static source evidence; not an observation of the native app.";
}

function findingRow(finding: DoctorFindingV1): HTMLElement {
...
function compatibilityEmptyRow(report: DoctorReportV1): HTMLElement {
...
function doctorGroup(title: string, rows: HTMLElement[]): HTMLElement {
...
function doctorSectionTitle(text: string): HTMLElement {
...
function doctorCard(rows: HTMLElement[]): HTMLElement {
...
function doctorRow(titleText: string, detail: string, trailing?: HTMLElement): HTMLElement {
  const row = document.createElement("div");
...
function doctorBadge(tone: "ok" | "warn" | "error", label: string): HTMLElement {
...
function doctorButton(label: string, onClick: () => void): HTMLButtonElement {
...
function copySnapshot(snapshot: DoctorViewSnapshot): DoctorViewSnapshot {
  return { ...snapshot };
}

function updateStateLabel(update: DoctorReportV1["update"]): string {
  if (update.phase === "promotion_failed" || update.phase === "stale") return "Needs attention";
  if (update.phase === "updating") return "Installing update";
  if (update.phase === "installed") return "Update installed";
  if (update.state === "compatible" && update.phase === "candidate_verified" && update.compatibility?.status === "passed") return "Ready to install";
  ...
  if (phaseLabel[update.phase]) return phaseLabel[update.phase];
  ...
  if (state === "fixes_required") return "Fixes required";
  return "Review required";
}

function updateTone(state: DoctorReportV1["update"]["state"]): "ok" | "warn" | "error" {
  if (state === "compatible") return "ok";
  if (state === "fixes_required") return "error";
  return "warn";
}

function pauseActionLabel(action: NonNullable<DoctorReviewProgressV1["pause"]>["action"]): string {
...
function technicalReportSummary(report: DoctorReportV1): string {
  const compatibility = report.update.compatibility;
...
function actionName(action: DoctorActionV1): string {
...
function versionAndBuild(version: string | null, build: string | null): string {
  if (!version && !build) return "Not available";
  if (version && build) return `${version} (${build})`;
  return version ?? `Build ${build}`;
}

function isDoctorReport(value: unknown): value is DoctorReportV1 {
  if (!value || typeof value !== "object") return false;
  const report = value as Partial<DoctorReportV1>;
  const usageDetails = report.update?.usageDetails;
  const review = report.update?.review;
  const validReview = review === undefined || (
    review.version === 1 && review.policy === "finish_automatically"
  ...
  const validUsageDetails = usageDetails === undefined || (
    usageDetails.version === 1
  ...
  const compatibility = report.update?.compatibility;
  const validCompatibility = compatibility === undefined || (
    compatibility.version === 1 && compatibility.policyVersion === 1
  ...
  const validAdoption = adoption === undefined || adoption === null || (
  ...
  return report.schemaVersion === 1
    ...
    && (["scan", "repair", "retry", "update"] as const).every((id) =>
      report.actions?.filter((action) => action.id === id
...
function isDoctorChangelog(value: unknown): value is DoctorChangelogV1 {
  if (!value || typeof value !== "object") return false;
  const log = value as Partial<DoctorChangelogV1>;
...
function isDoctorChange(value: unknown): value is DoctorChangeV1 {
  if (!value || typeof value !== "object") return false;
  const change = value as Partial<DoctorChangeV1>;
  const explanation = change.explanation;
  const validExplanation = explanation === undefined || (
...
function validUpdaterEvidence(value: unknown): boolean {
  if (value === undefined) return true;
  if (!value || typeof value !== "object") return false;
  const evidence = value as NonNullable<NonNullable<DoctorReportV1["adoption"]>["report"]["updaterEvidence"]>;
