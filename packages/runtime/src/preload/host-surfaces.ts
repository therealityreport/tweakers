...
import type {
  ...
  HostMcpFormIdentity,
  HostProjectContext,
  HostSharedHistoryTarget,
  HostSharedHistoryTargetResult,
  HostSurfaceKind,
  HostSurfaceMatch,
  HostSurfaceSnapshot,
  HostUiApi,
...
// Keep this local union explicit while the SDK build is intentionally left to
// the integration phase; the runtime typecheck can otherwise read a prior SDK
// declaration file before the SDK package is rebuilt.
type NativeSettingsSurfaceKind = "apps-settings" | "plugins-settings" | "mcp-settings";

type NativeSettingsSurfaceDefinition = {
...
const NATIVE_SETTINGS_SURFACES: Record<NativeSettingsSurfaceKind, NativeSettingsSurfaceDefinition> = {
...
const SELECTORS: Record<Exclude<HostSurfaceKind, "projects" | "thread-context" | "usage" | NativeSettingsSurfaceKind>, string> = {
...
type NativeSharedHistoryTurn = { nativeTurnId: string; root: Element };
type NativeSharedHistoryTarget = {
  conversationNativeId: string;
  composerNativeId: string;
  statusRoot: Element;
  composerRoot: Element;
  assistantTurns: NativeSharedHistoryTurn[];
};

type SharedHistoryTargetUnavailableReason =
  | "not_found"
  | "ambiguous"
  | "disconnected"
  | "invalid_identity"
...
type NativeSharedHistoryTargetResult =
  | { status: "available"; target: NativeSharedHistoryTarget }
  | { status: "unavailable"; reason: SharedHistoryTargetUnavailableReason };
 ...
 */
export async function getSharedHistoryTarget(): Promise<HostSharedHistoryTargetResult> {
  const native = currentNativeSharedHistoryTarget();
  if (native.status !== "available") return native;
  let response: unknown;
  try {
    response = await ipcRenderer.invoke(SHARED_HISTORY_MAP_NATIVE_TARGET_CHANNEL, {
  ...
  if (!nativeSharedHistoryTargetConnected(native.target)) return { status: "unavailable", reason: "disconnected" };
  const mapped = publicSharedHistoryMapping(response, native.target);
  if (!mapped) return { status: "unavailable", reason: "unmapped" };
  const { statusRoot, composerRoot, assistantTurns } = native.target;
  const target: HostSharedHistoryTarget = Object.freeze({
...
function currentNativeSharedHistoryTarget(): NativeSharedHistoryTargetResult {
  if (typeof document === "undefined") return { status: "unavailable", reason: "not_found" };
  const conversations = uniqueElements(document.querySelectorAll(SHARED_HISTORY_CONVERSATION_SELECTOR));
  if (conversations.length !== 1) return { status: "unavailable", reason: conversations.length > 1 ? "ambiguous" : "not_found" };
  const statusRoot = conversations[0];
  if (!statusRoot.isConnected) return { status: "unavailable", reason: "disconnected" };
  const conversationNativeId = exactNativeFiberIdentity(statusRoot, "conversationId");
  if (!conversationNativeId) return { status: "unavailable", reason: "invalid_identity" };
  ...
  if (composers.length !== 1) return { status: "unavailable", reason: composers.length > 1 ? "ambiguous" : "not_found" };
  const composerRoot = composers[0];
  const composerNativeId = exactNativeFiberIdentity(composerRoot, "composerId");
  if (!composerNativeId) return { status: "unavailable", reason: "invalid_identity" };

  const assistantTurns: NativeSharedHistoryTurn[] = [];
  const seenTurnIds = new Set<string>();
  const rawTurns = uniqueElements(statusRoot.querySelectorAll(SHARED_HISTORY_ASSISTANT_TURN_SELECTOR));
  if (rawTurns.length > MAX_SHARED_HISTORY_TURNS) return { status: "unavailable", reason: "ambiguous" };
  for (const root of rawTurns) {
    ...
    if (!nativeTurnId || seenTurnIds.has(nativeTurnId)) return { status: "unavailable", reason: "invalid_identity" };
...
function exactNativeFiberIdentity(element: Element, key: "conversationId" | "composerId" | "turnId"): string | null {
  if (!element.isConnected) return null;
  const fiber = fiberForNode(element) as ReactFiberNode | null;
  const props = asRecord(fiber?.memoizedProps);
  const value = props?.[key];
  return validNativeSharedHistoryId(value) ? value : null;
}

function validNativeSharedHistoryId(value: unknown): value is string {
  return typeof value === "string" && value.length >= 1 && value.length <= MAX_SHARED_HISTORY_NATIVE_ID_LENGTH
    && /^[A-Za-z0-9._:-]+$/.test(value);
}

function nativeSharedHistoryTargetConnected(target: NativeSharedHistoryTarget): boolean {
...
function publicSharedHistoryMapping(
  value: unknown,
  target: NativeSharedHistoryTarget,
): { conversationId: string; turns: Array<{ turnId: string; root: Element }> } | null {
  ...
  if (!response || response.version !== SHARED_HISTORY_MAP_VERSION || response.status !== "mapped"
    ...
    || response.turnIds.length !== target.assistantTurns.length) return null;
  const seenPublic = new Set<string>();
  const turns: Array<{ turnId: string; root: Element }> = [];
  for (let index = 0; index < response.turnIds.length; index += 1) {
    const turnId = response.turnIds[index];
    if (!isPublicSharedHistoryTurnId(turnId) || seenPublic.has(turnId)) return null;
...
function isPublicSharedHistoryConversationId(value: unknown): value is string {
  return typeof value === "string" && /^conversation_[A-Za-z0-9_-]{43}$/.test(value);
}

function isPublicSharedHistoryTurnId(value: unknown): value is string {
...
export function queryHostSurfaces(kind: HostSurfaceKind): HostSurfaceMatch[] {
  ...
  if (isNativeSettingsSurfaceKind(kind)) return nativeSettingsPageSurface(kind);
...
function ensureObserver(): void {
  ...
  sharedObserver = new MutationObserver((records) => {
    ...
    if (records.length > 0 && records.every((record) => mutationIsInsideOwnedHostSurface(record))) return;
...
function mutationIsInsideOwnedHostSurface(record: MutationRecord): boolean {
...
function isNativeSettingsSurfaceKind(kind: string): kind is NativeSettingsSurfaceKind {
 ...
 */
function nativeSettingsPageSurface(kind: NativeSettingsSurfaceKind): HostSurfaceMatch[] {
  const definition = NATIVE_SETTINGS_SURFACES[kind];
  const activeNativeRoutes = uniqueElements(
    document.querySelectorAll("[data-settings-panel-slug][aria-current='page']"),
  );
  if (activeNativeRoutes.length !== 1) return [];
  if (!routeMatchesNativeSettingsSurface(activeNativeRoutes[0], definition)) return [];
  ...
  if (!container) return [];
  return [{ kind: kind as HostSurfaceKind, element: container, confidence: "high", label: accessibleLabel(headings[0]) }];
}

function routeMatchesNativeSettingsSurface(
  element: Element,
  definition: NativeSettingsSurfaceDefinition,
): boolean {
...
function headingMatchesNativeSettingsSurface(
  element: Element,
  definition: NativeSettingsSurfaceDefinition,
): boolean {
...
function normalizeNativeSettingsToken(value: string | null | undefined): string {
 ...
 */
function boundedNativeSettingsPageContainer(heading: Element): Element | null {
  const container = heading.closest("[data-settings-panel-content], [data-settings-page], [role='main'], main, section, article");
  if (!container || !container.isConnected) return null;
  if (container.querySelector("[data-settings-panel-slug]")) return null;
  return container;
}
