...
export const TOKEN_BALANCE_FILE_V1 = "token-balance-v1.json";
const TOKEN_BALANCE_VERSION = 1 as const;
...
export type TokenBalanceReservationState =
  | "reserved"
  | "dispatched"
  | "observed"
  | "settled"
...
export interface TokenBalanceSnapshot {
  ...
  reservations: { active: number; uncertain: number; missingUsage: number };
  observations: { threadCount: number; lastOnlyCount: number; outOfOrderTotalCount: number };
}
...
interface PersistedAccount {
  ...
  completedAggregateTokens: number;
  /** Conservative cost retained after terminal work lacks trustworthy totals. */
  estimatedCompletedTokens: number;
  /** Debt whose bounded per-thread correlation has been deliberately retired. */
  irreconcilableDebtTokens: number;
  partialObservationCount: number;
  unknownUsageCount: number;
}

interface PersistedReservation extends TokenBalanceReservation {
  dispatchedAt: string | null;
  settledAt: string | null;
  observedUsage: boolean;
  exactUsageObserved: boolean;
  hasUnknownUsage: boolean;
  /** Portion of a terminal estimate awaiting a late trustworthy total delta. */
  estimatedDebtTokens: number;
}

type TotalBasis = "components" | "aggregate" | "partial";

interface PersistedThreadCounter {
  opaqueAccountId: OpaqueAccountId;
  threadId: string;
  basis: TotalBasis | "uninitialized";
  inputTokens: number | null;
  outputTokens: number | null;
  aggregateTokens: number | null;
  priorTotalUnknown: boolean;
  lastOnlyCount: number;
  outOfOrderTotalCount: number;
  /** Folded terminal estimates that only this thread's later totals may replace. */
  unreportedDebtTokens: number;
...
interface TokenBalanceState {
  ...
  baseline: {
    startedAt: string;
    /** Existing native account history predates this broker-owned ledger. */
    importedHistoryUnmeasuredAccounts: OpaqueAccountId[];
  };
  accounts: Record<string, PersistedAccount>;
  reservations: PersistedReservation[];
  threads: Record<string, PersistedThreadCounter>;
  counters: {
    lastOnlyCount: number;
    outOfOrderTotalCount: number;
    evictedThreadCount: number;
  };
}

interface ParsedTotal {
  basis: TotalBasis;
  inputTokens: number | null;
  outputTokens: number | null;
  aggregateTokens: number | null;
 ...
 */
export class TokenBalanceLedger {
  private state: TokenBalanceState;
  private readonly configured = new Map<OpaqueAccountId, TokenBalanceAccountInput>();
  private orderedConfigured: OpaqueAccountId[];
  private readonly now: () => number;
  private readonly random: (size: number) => Buffer;
  readonly fileName: string;

  constructor(private readonly options: TokenBalanceLedgerOptions) {
    if (!Array.isArray(options.accounts)) throw new Error("invalid token-balance accounts");
    for (const account of options.accounts) {
      if (!isOpaqueAccountId(account.opaqueAccountId) || typeof account.included !== "boolean" || this.configured.has(account.opaqueAccountId)) {
    ...
    this.fileName = options.fileName ?? TOKEN_BALANCE_FILE_V1;
    if (!safeFileName(this.fileName)) throw new Error("unsafe token-balance file name");
  ...
  choose(eligibleAccountIds: readonly OpaqueAccountId[]): TokenBalanceChoice | null {
    ...
    let chosen: TokenBalanceChoice | null = null;
    for (const opaqueAccountId of this.orderedConfigured) {
      const configured = this.configured.get(opaqueAccountId)!;
      if (!configured.included || !eligible.has(opaqueAccountId)) continue;
      const projection = this.projectedTokens(opaqueAccountId);
      if (!projection) continue;
      if (!chosen || projection.projectedTokens < chosen.projectedTokens) chosen = { opaqueAccountId, ...projection };
    }
    return chosen;
  }
  ...
  setAccounts(accounts: readonly TokenBalanceAccountInput[]): void {
    ...
    this.update((state) => {
      for (const opaqueAccountId of next.keys()) {
        if (!state.accounts[opaqueAccountId]) state.accounts[opaqueAccountId] = emptyAccount();
      }
    });
    this.configured.clear();
    for (const [opaqueAccountId, account] of next) this.configured.set(opaqueAccountId, account);
  ...
  markImportedHistoryUnmeasured(accounts: readonly OpaqueAccountId[]): void {
    ...
    if (unique.some((opaqueAccountId) => !isOpaqueAccountId(opaqueAccountId) || !this.state.accounts[opaqueAccountId])) {
    ...
    if (unique.every((opaqueAccountId) => existing.has(opaqueAccountId))) return;
    this.update((state) => {
      const marked = new Set(state.baseline.importedHistoryUnmeasuredAccounts);
      for (const opaqueAccountId of unique) marked.add(opaqueAccountId);
      state.baseline.importedHistoryUnmeasuredAccounts = [...marked].sort() as OpaqueAccountId[];
    });
  }

  /** Atomically reserve an account before the caller is permitted to write to its provider child. */
  begin(input: Readonly<{ opaqueAccountId: OpaqueAccountId; reservationId?: string; estimatedTokens: number }>): TokenBalanceReservation {
    const configured = this.configured.get(input.opaqueAccountId);
    if (!configured?.included) throw new Error("token-balance account is not currently included");
    if (!validEstimate(input.estimatedTokens)) throw new Error("invalid token-balance reservation estimate");
    const reservationId = input.reservationId ?? `tbl_${this.random(18).toString("base64url")}`;
    if (!safeIdentifier(reservationId, 128)) throw new Error("invalid token-balance reservation id");
    let result: PersistedReservation | null = null;
    this.update((state) => {
      ...
      if (activeReservationCount(state) >= MAX_ACTIVE_RESERVATIONS) throw new Error("token-balance active reservation bound reached");
  ...
  markDispatched(reservationId: string): void {
    this.update((state) => {
      ...
      if (["dispatched", "observed", "uncertain_after_restart"].includes(reservation.state)) return;
  ...
  bind(reservationId: string, binding: Readonly<{ threadId: string; turnId?: string | null }>): void {
    ...
    this.update((state) => {
      const reservation = reservationById(state, reservationId);
      if (isTerminal(reservation.state)) throw new Error("token-balance cannot bind a terminal reservation");
      if (reservation.threadId !== null && reservation.threadId !== binding.threadId) throw new Error("token-balance thread binding collision");
      const turnId = binding.turnId ?? null;
      if (reservation.turnId !== null && turnId !== null && reservation.turnId !== turnId) throw new Error("token-balance turn binding collision");
      reservation.threadId = binding.threadId;
      if (turnId !== null) reservation.turnId = turnId;
    });
  }
   ...
   */
  reservationForThread(input: Readonly<{ opaqueAccountId: OpaqueAccountId; threadId: string; turnId?: string | null }>): string | null {
    ...
    if (!this.state.accounts[input.opaqueAccountId]) return null;
  ...
  releasePreDispatch(reservationId: string): void {
    this.update((state) => {
      const reservation = reservationById(state, reservationId);
      if (reservation.state === "released_pre_dispatch") return;
      if (reservation.state !== "reserved") throw new Error("token-balance refuses to release possibly dispatched work");
      reservation.state = "released_pre_dispatch";
   ...
   */
  seedThreadBaseline(input: Readonly<{ opaqueAccountId: OpaqueAccountId; threadId: string; tokenUsage: unknown }>): void {
    if (!safeIdentifier(input.threadId, MAX_IDENTIFIER_LENGTH)) throw new Error("invalid token-balance thread id");
    if (!this.state.accounts[input.opaqueAccountId]) throw new Error("unknown token-balance account");
    const total = parseTotal(unwrapTotal(input.tokenUsage));
    if (!total || total.basis === "partial") throw new Error("token-balance baseline requires a complete total");
    this.update((state) => {
      const key = threadKey(input.opaqueAccountId, input.threadId);
      const existing = state.threads[key];
      if (existing && !sameTotal(existing, total)) throw new Error("token-balance baseline already differs");
      if (!existing) {
  ...
  observe(input: Readonly<{
    ...
    if (!this.state.accounts[input.opaqueAccountId]) throw new Error("unknown token-balance account");
    let result: TokenBalanceObservation = { addedTokens: 0, precision: precisionFor(this.state, input.opaqueAccountId), reservationId: null };
    this.update((state) => {
      ...
      if (reservation && input.reservationId && reservation.threadId === null && !isTerminal(reservation.state)) {
        reservation.threadId = input.threadId;
        if (input.turnId) reservation.turnId = input.turnId;
      }
      const nested = isPlainRecord(input.tokenUsage) ? input.tokenUsage : null;
      const total = parseTotal(nested?.total);
      const hasLast = parseTotal(nested?.last) !== null;
      if (!total) {
        if (hasLast) recordLastOnly(state, input.opaqueAccountId, input.threadId);
        else state.accounts[input.opaqueAccountId].partialObservationCount += 1;
        if (reservation) reservation.hasUnknownUsage = true;
      ...
      if (delta.tokens > 0) {
        ...
        if (reservation) {
          ...
          if (reservation.state === "dispatched") reservation.state = "observed";
        }
        repayThreadDebt(state.accounts[input.opaqueAccountId], counter, unrepaidTokens);
      }
      if (reservation) {
        reservation.observedUsage = true;
        if (!counter.priorTotalUnknown) reservation.exactUsageObserved = true;
      }
  ...
  settle(reservationId: string): void {
    this.update((state) => {
      ...
      if (reservation.state === "released_pre_dispatch") throw new Error("token-balance cannot settle a released reservation");
      if (reservation.state === "reserved") throw new Error("token-balance requires a pre-dispatch release or dispatch mark");
      if (!reservation.exactUsageObserved || reservation.hasUnknownUsage) {
        ...
        state.accounts[reservation.opaqueAccountId].estimatedCompletedTokens = safeAdd(
          state.accounts[reservation.opaqueAccountId].estimatedCompletedTokens,
          estimatedDebtTokens,
        );
  ...
  recover(): { uncertainReservations: number } {
   ...
   */
  terminalizeUncertainAfterRecovery(): { terminalized: number } {
    let terminalized = 0;
    this.update((state) => {
      for (const reservation of state.reservations) {
        if (reservation.state !== "uncertain_after_restart") continue;
        reservation.state = "settled";
        reservation.settledAt = this.timestamp();
        reservation.missingUsage = true;
        const estimatedDebtTokens = Math.max(0, reservation.estimatedTokens - reservation.observedTokens);
  ...
  snapshot(): TokenBalanceSnapshot {
    const ordered = [
      ...
      ...Object.keys(this.state.accounts).filter((id) => !this.configured.has(id as OpaqueAccountId)).sort(),
    ] as OpaqueAccountId[];
  ...
  private get path(): string {
    return join(this.options.root, this.fileName);
  }

  private load(): TokenBalanceState {
    if (!existsSync(this.path)) {
      const initial: TokenBalanceState = {
        version: TOKEN_BALANCE_VERSION,
    ...
    assertPrivateRegularFile(this.path, MAX_TOKEN_BALANCE_BYTES);
    const raw = readFileSync(this.path, "utf8");
    if (Buffer.byteLength(raw) > MAX_TOKEN_BALANCE_BYTES) throw new Error("token-balance state exceeds its bounded size");
    let parsed: unknown;
    try {
    ...
    if (!normalized || !validateState(normalized)) throw new Error("token-balance state failed strict validation");
    if (normalized !== parsed) this.persist(normalized);
    return normalized;
  }

  private update(mutator: (state: TokenBalanceState) => void): void {
    const next = structuredClone(this.state);
    mutator(next);
    compactReservations(next);
    if (!validateState(next)) throw new Error("token-balance refused an invalid durable state");
    this.persist(next);
    this.state = next;
  }

  private persist(next: TokenBalanceState): void {
    writePrivateJsonAtomicBounded(this.options.root, this.fileName, next, MAX_TOKEN_BALANCE_BYTES);
  }

  private timestamp(): string {
    const now = this.now();
    if (!Number.isSafeInteger(now) || now < 0) throw new Error("invalid token-balance clock");
    return new Date(now).toISOString();
  }
}

function emptyAccount(): PersistedAccount {
  return {
    completedTokens: 0,
...
function publicReservation(reservation: PersistedReservation): TokenBalanceReservation {
  const {
    dispatchedAt: _dispatchedAt,
    settledAt: _settledAt,
    observedUsage: _observedUsage,
    exactUsageObserved: _exactUsageObserved,
    hasUnknownUsage: _hasUnknownUsage,
    estimatedDebtTokens: _estimatedDebtTokens,
    ...result
  } = reservation;
  return structuredClone(result);
}

function reservationById(state: TokenBalanceState, reservationId: string): PersistedReservation {
  if (!safeIdentifier(reservationId, 128)) throw new Error("invalid token-balance reservation id");
  const reservation = state.reservations.find((candidate) => candidate.reservationId === reservationId);
  if (!reservation) throw new Error("unknown token-balance reservation");
  return reservation;
}

function findAttribution(
  state: TokenBalanceState,
  input: Readonly<{ opaqueAccountId: OpaqueAccountId; threadId: string; turnId?: string | null; reservationId?: string | null }>,
): PersistedReservation | null {
  if (input.reservationId) {
    const reservation = reservationById(state, input.reservationId);
    if (reservation.opaqueAccountId !== input.opaqueAccountId) throw new Error("token-balance reservation account mismatch");
    if (reservation.threadId !== null && reservation.threadId !== input.threadId) throw new Error("token-balance reservation thread mismatch");
    if (reservation.turnId !== null && input.turnId && reservation.turnId !== input.turnId) throw new Error("token-balance reservation turn mismatch");
    return reservation;
  }
...
function parseTotal(value: unknown): ParsedTotal | null {
  ...
  if (inputTokens !== null && outputTokens !== null) return { basis: "components", inputTokens, outputTokens, aggregateTokens: null };
  if (aggregateTokens !== null) return { basis: "aggregate", inputTokens: null, outputTokens: null, aggregateTokens };
  if (inputTokens !== null || outputTokens !== null) return { basis: "partial", inputTokens, outputTokens, aggregateTokens: null };
  return null;
}

function unwrapTotal(value: unknown): unknown {
  return isPlainRecord(value) && "total" in value ? value.total : value;
}
...
function counterFromTotal(
  opaqueAccountId: OpaqueAccountId,
  threadId: string,
  total: ParsedTotal,
  priorTotalUnknown: boolean,
  observedAt: string,
): PersistedThreadCounter {
  return {
    opaqueAccountId,
    threadId,
    basis: total.basis,
...
function counterForUnreportedDebt(opaqueAccountId: OpaqueAccountId, threadId: string, observedAt: string): PersistedThreadCounter {
  return {
    opaqueAccountId,
    threadId,
    basis: "uninitialized",
...
function sameTotal(counter: PersistedThreadCounter, total: ParsedTotal): boolean {
...
function positiveDelta(counter: PersistedThreadCounter, total: ParsedTotal): {
  tokens: number;
  inputTokens: number;
  outputTokens: number;
...
function deltaAndAdvance(counter: PersistedThreadCounter, key: "inputTokens" | "outputTokens" | "aggregateTokens", next: number): number {
...
function charge(account: PersistedAccount, basis: TotalBasis, inputTokens: number, outputTokens: number, aggregateTokens: number): void {
...
function repayEstimatedDebt(account: PersistedAccount, reservation: PersistedReservation, observedTokens: number): number {
  if (reservation.estimatedDebtTokens === 0 || observedTokens === 0) return 0;
...
function repayThreadDebt(account: PersistedAccount, counter: PersistedThreadCounter, observedTokens: number): number {
  if (counter.unreportedDebtTokens === 0 || observedTokens === 0) return 0;
...
function recordLastOnly(state: TokenBalanceState, opaqueAccountId: OpaqueAccountId, threadId: string): void {
  ...
  if (existing) existing.lastOnlyCount += 1;
}

function makeThreadRoom(state: TokenBalanceState): void {
  if (Object.keys(state.threads).length < MAX_THREAD_COUNTERS) return;
  ...
  if (!candidate) throw new Error("token-balance thread counter bound reached by active work");
  const [key, counter] = candidate;
  if (counter.unreportedDebtTokens > 0) {
    state.accounts[counter.opaqueAccountId].irreconcilableDebtTokens = safeAdd(
      state.accounts[counter.opaqueAccountId].irreconcilableDebtTokens,
      counter.unreportedDebtTokens,
    );
  }
...
function compactReservations(state: TokenBalanceState): void {
  ...
  for (const reservation of foldedDebt) foldReservationDebtIntoThread(state, reservation);
...
function foldReservationDebtIntoThread(state: TokenBalanceState, reservation: PersistedReservation): void {
  const debt = reservation.estimatedDebtTokens;
  if (debt === 0) return;
  if (reservation.threadId === null) {
    state.accounts[reservation.opaqueAccountId].irreconcilableDebtTokens = safeAdd(
      state.accounts[reservation.opaqueAccountId].irreconcilableDebtTokens,
      debt,
    );
    return;
  }
...
function activeReservationCount(state: TokenBalanceState): number {
  return state.reservations.filter((reservation) => !isTerminal(reservation.state)).length;
}


function isTerminal(state: TokenBalanceReservationState): boolean {
  return state === "settled" || state === "released_pre_dispatch";
}

function reservedTokensFor(state: TokenBalanceState, opaqueAccountId: OpaqueAccountId): number {
...
function precisionFor(state: TokenBalanceState, opaqueAccountId: OpaqueAccountId): TokenBalancePrecision {
  const account = state.accounts[opaqueAccountId];
  if (!account) return "unknown";
  if (state.baseline.importedHistoryUnmeasuredAccounts.includes(opaqueAccountId)) return "unknown";
  if (account.unknownUsageCount > 0) return "unknown";
  if (account.partialObservationCount > 0) return "partial";
  return "exact";
}

function safeAdd(left: number, right: number): number {
  const value = left + right;
  if (!Number.isSafeInteger(value) || value < 0) throw new Error("token-balance counter overflow");
  return value;
}

function validEstimate(value: unknown): value is number {
  return typeof value === "number" && Number.isSafeInteger(value) && value >= 1 && value <= MAX_ESTIMATED_TOKENS;
}

function validatedAccounts(accounts: readonly TokenBalanceAccountInput[]): Map<OpaqueAccountId, TokenBalanceAccountInput> {
  const result = new Map<OpaqueAccountId, TokenBalanceAccountInput>();
  for (const account of accounts) {
    if (!isOpaqueAccountId(account.opaqueAccountId) || typeof account.included !== "boolean" || result.has(account.opaqueAccountId)) {
      throw new Error("invalid or duplicate token-balance account");
    }
...
function threadKey(opaqueAccountId: OpaqueAccountId, threadId: string): string {
  return createHash("sha256").update(`${opaqueAccountId}\u0000${threadId}`, "utf8").digest("base64url");
}
...
function validateState(value: unknown): value is TokenBalanceState {
  if (!isPlainRecord(value) || Object.keys(value).some((key) => !["version", "baseline", "accounts", "reservations", "threads", "counters"].includes(key))) return false;
  const baseline = value.baseline;
  const accounts = value.accounts;
  const reservations = value.reservations;
  const threads = value.threads;
  const counters = value.counters;
  if (value.version !== TOKEN_BALANCE_VERSION || !isPlainRecord(baseline)
    ...
    || baseline.importedHistoryUnmeasuredAccounts.some((account) => !isOpaqueAccountId(account))
    ...
    || !isPlainRecord(accounts) || !Array.isArray(reservations) || !isPlainRecord(threads) || !isPlainRecord(counters)) return false;
  const accountIds = Object.keys(accounts);
  if (accountIds.some((id) => !isOpaqueAccountId(id) || !validateAccount(accounts[id]))) return false;
  if (accountIds.length > 512 || reservations.length > MAX_RESERVATIONS || Object.keys(threads).length > MAX_THREAD_COUNTERS) return false;
  const accountSet = new Set(accountIds);
  if (!reservations.every((reservation) => validateReservation(reservation, accountSet))) return false;
  if (reservations.filter((reservation) => !isTerminal(reservation.state)).length > MAX_ACTIVE_RESERVATIONS) return false;
  if (!Object.values(threads).every((counter) => validateThread(counter, accountSet))) return false;
...
function normalizeLegacyBaseline(value: unknown): TokenBalanceState | null {
  if (!isPlainRecord(value) || !isPlainRecord(value.baseline)) return null;
  const baseline = value.baseline;
  if (Object.keys(baseline).length !== 1 || !Object.prototype.hasOwnProperty.call(baseline, "startedAt")) {
    return value as unknown as TokenBalanceState;
  }
  return {
    ...value,
    baseline: {
  ...
  } as unknown as TokenBalanceState;
}

function validateAccount(value: unknown): value is PersistedAccount {
  ...
  if (Object.keys(value).length !== keys.length || !keys.every((key) => key in value && validCounter(value[key]))) return false;
...
function validateReservation(value: unknown, accounts: Set<string>): value is PersistedReservation {
  if (!isPlainRecord(value)) return false;
  ...
  return safeIdentifier(value.reservationId, 128) && isOpaqueAccountId(value.opaqueAccountId) && accounts.has(value.opaqueAccountId)
...
function validateThread(value: unknown, accounts: Set<string>): value is PersistedThreadCounter {
  if (!isPlainRecord(value)) return false;
  ...
  if (Object.keys(value).length !== keys.length || !keys.every((key) => key in value)) return false;
  const validOptionalCounter = (candidate: unknown) => candidate === null || validCounter(candidate);
  if (!isOpaqueAccountId(value.opaqueAccountId) || !accounts.has(value.opaqueAccountId) || !safeIdentifier(value.threadId, MAX_IDENTIFIER_LENGTH)
    ...
    || typeof value.priorTotalUnknown !== "boolean" || !validCounter(value.lastOnlyCount) || !validCounter(value.outOfOrderTotalCount) || !validCounter(value.unreportedDebtTokens) || !validTimestamp(value.lastObservedAt)) return false;
  if (value.basis === "uninitialized") return value.inputTokens === null && value.outputTokens === null && value.aggregateTokens === null;
  if (value.basis === "components") return value.inputTokens !== null && value.outputTokens !== null && value.aggregateTokens === null;
  if (value.basis === "aggregate") return value.inputTokens === null && value.outputTokens === null && value.aggregateTokens !== null;
