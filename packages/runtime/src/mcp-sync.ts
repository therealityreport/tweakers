...
const activeMcpConfigMutations = new Set<string>();

export function withMcpConfigMutationLock<T>(configPath: string, mutate: () => T): T {
  const mutationKey = resolve(configPath);
...
export function planManagedMcpReconciliation(
  tweaks: McpSyncTweak[],
  currentToml = "",
  options: ManagedMcpReconciliationOptions = {},
  ...
  for (const [canonicalName, tweak] of ownedByName) {
    const mcp = normalizeMcpServer(tweak.manifest.mcp);
    if (!mcp) throw new Error(`Owned MCP tweak ${tweak.manifest.id} has an invalid MCP declaration`);
    const desired = desiredByName.has(canonicalName);
    if (desired) desiredNames.push(canonicalName);
    ...
    if (observedOptions) {
      ...
      if (Object.keys(nextOptions).length > 0) preservedOptions[canonicalName] = nextOptions;
      else delete preservedOptions[canonicalName];
    }

    if (legacyName && exactlyOwnedLegacy) {
      rangesToRemove.push(legacyTables[0]!);
      if (desired) migrations.push({ from: legacyName, to: canonicalName });
    }
  ...
  const block = entries.length > 0
    ? [MCP_MANAGED_START, ...entries, MCP_MANAGED_END].join("\n")
    : "";
...
interface TopLevelApprovalPolicy {
  start: number;
  end: number;
  raw: string;
  lineEnding: string;
...
function unchangedApprovalPolicy(
  raw: string | null,
  preserved: Readonly<PreservedApprovalPolicy> | null,
  sandboxModeRaw: string | null = null,
...
export function sanitizePreservedApprovalPolicy(value: unknown): PreservedApprovalPolicy | null {
  if (!value || typeof value !== "object" || Array.isArray(value)) return null;
  const record = value as { present?: unknown; rawAssignment?: unknown };
  if (typeof record.present !== "boolean") return null;
  if (record.rawAssignment !== null && typeof record.rawAssignment !== "string") return null;
  if (record.present !== (record.rawAssignment !== null)) return null;
  if (typeof record.rawAssignment === "string") {
    if (/[\r\n]/.test(record.rawAssignment)) return null;
    if (!TOP_LEVEL_APPROVAL_POLICY_ASSIGNMENT.test(record.rawAssignment)) return null;
    try {
    ...
    if (assignments.length !== 1 || assignments[0]?.raw !== record.rawAssignment) return null;
  }
  return { present: record.present, rawAssignment: record.rawAssignment as string | null };
}

function findTopLevelApprovalPolicies(toml: string): TopLevelApprovalPolicy[] {
  return findTopLevelAssignments(toml, TOP_LEVEL_APPROVAL_POLICY_ASSIGNMENT);
}

function findTopLevelSandboxModes(toml: string): TopLevelApprovalPolicy[] {
  return findTopLevelAssignments(toml, TOP_LEVEL_SANDBOX_MODE_ASSIGNMENT);
}

function findTopLevelAssignments(toml: string, assignmentPattern: RegExp): TopLevelApprovalPolicy[] {
  const assignments: TopLevelApprovalPolicy[] = [];
  for (const line of classifyTomlLines(toml)) {
    if (!line.structural) continue;
    if (/^\s*\[/.test(line.text)) break;
    if (!assignmentPattern.test(line.text)) continue;
    assignments.push({
      start: line.start,
      end: line.end,
      raw: line.text,
...
function indexOwnedMcpTweaks(tweaks: McpSyncTweak[]): Map<string, McpSyncTweak> {
  const indexed = new Map<string, McpSyncTweak>();
  for (const tweak of tweaks) {
    if (!normalizeMcpServer(tweak.manifest.mcp)) continue;
...
function indexDesiredMcpTweaks(
  tweaks: McpSyncTweak[],
  ownedByName: ReadonlyMap<string, McpSyncTweak>,
): Map<string, McpSyncTweak> {
  const indexed = new Map<string, McpSyncTweak>();
  for (const tweak of tweaks) {
    if (!normalizeMcpServer(tweak.manifest.mcp)) continue;
    const canonicalName = mcpServerNameFromTweakId(tweak.manifest.id);
...
export function sanitizePreservedMcpOptions(
  ...
  for (const [serverName, rawOptions] of Object.entries(value)) {
    ...
    if (entries.some(([key]) => key !== "defaultToolsApprovalMode")) continue;
    const approval = (rawOptions as { defaultToolsApprovalMode?: unknown }).defaultToolsApprovalMode;
    if (approval === "approve") sanitized[serverName] = { defaultToolsApprovalMode: "approve" };
    else if (approval !== undefined) continue;
  }
  return sanitized;
}

function pickPreservedOptions(spec: NormalizedMcpSpec): PreservedMcpOptions {
...
export function mergeManagedMcpBlock(currentToml: string, managedBlock: string): string {
  ...
  if (!managedBlock && stripped === currentToml.trimEnd()) return currentToml;
 ...
 */
export function assertValidTomlDocument(toml: string): void {
  if (/[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F]/.test(toml)) {
...
type TomlQuote = "basic" | "literal" | "multiline-basic" | "multiline-literal" | null;

function scanTomlStatements(toml: string): TomlStatement[] {
  ...
  let squareDepth = 0;
  let curlyDepth = 0;
  let quote: TomlQuote = null;
  let escaped = false;
  let comment = false;

  const flush = (): void => {
    if (buffer.trim()) statements.push({ value: buffer, line: statementLine });
    buffer = "";
  ...
  for (let index = 0; index < toml.length; index += 1) {
    ...
    if (character === "\n" || character === "\r") {
      if (character === "\n") line += 1;
      if (squareDepth === 0 && curlyDepth === 0) flush();
      else buffer += "\n";
      continue;
    }
    buffer += character;
  ...
  if (quote) throw malformedToml(`unterminated string at line ${line}`);
  if (squareDepth !== 0 || curlyDepth !== 0) {
...
function isValidTomlKeyPath(value: string): boolean {
  const bareOrQuoted = String.raw`(?:[A-Za-z0-9_-]+|"(?:[^"\\]|\\.)*"|'[^']*')`;
  return new RegExp(`^${bareOrQuoted}(?:\\s*\\.\\s*${bareOrQuoted})*$`).test(value);
}

function isValidTomlValue(value: string): boolean {
  if (!value) return false;
  if ((value.startsWith('\"\"\"') && value.endsWith('\"\"\"'))
    || (value.startsWith("'''") && value.endsWith("'''"))) return true;
  if (value.startsWith('\"') && value.endsWith('\"')) return isValidBasicTomlString(value);
  ...
  if (/^[-+]?(?:0|[1-9](?:_?\d)*|0x[0-9A-Fa-f](?:_?[0-9A-Fa-f])*|0o[0-7](?:_?[0-7])*|0b[01](?:_?[01])*)$/.test(value)) return true;
  if (/^[-+]?(?:(?:\d(?:_?\d)*)?\.\d(?:_?\d)*(?:[eE][-+]?\d(?:_?\d)*)?|\d(?:_?\d)*(?:[eE][-+]?\d(?:_?\d)*))$/.test(value)) return true;
  return /^\d{4}-\d{2}-\d{2}(?:[Tt ]\d{2}:\d{2}:\d{2}(?:\.\d+)?(?:[Zz]|[-+]\d{2}:\d{2})?)?$/.test(value)
    || /^\d{2}:\d{2}:\d{2}(?:\.\d+)?$/.test(value);
}

function isValidBasicTomlString(value: string): boolean {
  const body = value.slice(1, -1);
...
function splitTopLevelWithOffsets(
  value: string,
  separator: string,
): Array<{ value: string; end: number }> {
  const parts: Array<{ value: string; end: number }> = [];
  let start = 0;
  let squareDepth = 0;
  let curlyDepth = 0;
  let quote: '"' | "'" | null = null;
...
function malformedToml(detail: string): Error {
  return new Error(`Malformed TOML: ${detail}`);
}

interface ClassifiedTomlLine {
  start: number;
  end: number;
  text: string;
  structural: boolean;
 ...
 */
function classifyTomlLines(toml: string): ClassifiedTomlLine[] {
  const lines: ClassifiedTomlLine[] = [];
  let lineStart = 0;
  ...
  if (lineStart < toml.length || toml.length === 0) pushLine(toml.length);
  return lines;
}
...
function findMcpServerTables(toml: string): McpTableRange[] {
  const allHeaders = classifyTomlLines(toml)
    .filter((line) => line.structural)
    .map((line) => ({ line, match: /^\s*\[([^\]\r\n]+)\]\s*(?:#.*)?$/.exec(line.text) }))
    .filter((entry): entry is { line: ClassifiedTomlLine; match: RegExpExecArray } => entry.match !== null);
  const tables: McpTableRange[] = [];
  for (let index = 0; index < allHeaders.length; index += 1) {
    const { line, match } = allHeaders[index]!;
    const header = match[1] ?? "";
    if (!header.startsWith("mcp_servers.")) continue;
...
function mcpTableContentEnd(toml: string, headerEnd: number, tableEnd: number): number {
...
function legacyMcpServerName(tweakId: string): string | null {
...
function parseMcpTableBody(body: string): NormalizedMcpSpec | null {
  ...
  try {
    const command = JSON.parse(fields.get("command") ?? "") as unknown;
    const args = fields.has("args") ? JSON.parse(fields.get("args") ?? "") as unknown : [];
    ...
    // closed.
    if (fields.has("enabled") && fields.get("enabled") !== "true") return null;
    ...
    const defaultToolsApprovalMode = fields.has("default_tools_approval_mode")
      ? JSON.parse(fields.get("default_tools_approval_mode") ?? "") as unknown
      : undefined;
    if (defaultToolsApprovalMode !== undefined && defaultToolsApprovalMode !== "approve") return null;
...
function parseTomlInlineStringTable(value: string): Record<string, string> | null {
  ...
  for (const part of body.split(",")) {
    ...
    const parsed = JSON.parse(field[2]) as unknown;
    if (typeof parsed !== "string" || Object.hasOwn(result, key)) return null;
    result[key] = parsed;
  }
  return result;
}
