...
function observeInstallation(document: ReadDocument): ManagerDashboardInstallationV1 {
  ...
  if (version === null || installedAt === null || appRoot === null || runtimeUpdatedAt === undefined) {
...
function observeDesktopUpdateReceipt(document: ReadDocument): ParsedReceipt {
...
function observeManagerOperations(
  ...
  if (directory.state === "missing") {
    return { state: "missing", revision: "missing", activeOperationId: null, preparedCount: 0, problem: null };
  }
  if (directory.state === "unreadable") {
    return { state: "unreadable", revision: "unreadable", activeOperationId: null, preparedCount: 0, problem: directory.problem };
  }
  ...
  const observed: Array<{ operationId: string; phase: string; revision: string; reconciledFromReceipt: boolean }> = [];
  for (const name of names) {
    ...
    if (document.state === "missing") {
      return { state: "malformed", revision: "missing-record", activeOperationId: null, preparedCount: 0, problem: `manager operation disappeared: ${operationId}` };
    }
    if (document.state === "unreadable") {
      return { state: "unreadable", revision: "unreadable-record", activeOperationId: null, preparedCount: 0, problem: document.problem };
    }
    ...
    try {
      ...
      // write its own terminal record must not permanently prevent another
      // receipt-bound operation.  The durable receipt is the only authority
      // that can reconcile that projection.  We deliberately leave the raw
      // record as `consumed`: it is never reopened to `prepared`, and a
      // missing, malformed, mismatched, or still-active receipt stays closed.
      const reconciledFromReceipt = consumedRecordReachedBoundTerminalReceipt(record, receipts, updater);
      observed.push({ operationId, phase: record.phase, revision, reconciledFromReceipt });
    } catch (error) {
      return { state: "malformed", revision, activeOperationId: null, preparedCount: 0, problem: errorMessage(error) };
    }
  }
  ...
  const active = observed.filter((record) => (
    ...
    || (record.phase === "consumed" && !record.reconciledFromReceipt)
 ...
 */
function consumedRecordReachedBoundTerminalReceipt(
...
function parseBoundReceiptReference(value: string | undefined): { source: "environment" | "desktop-update"; receiptId: string } | null {
  const match = /^(environment|desktop-update):(.+)$/.exec(value ?? "");
  if (match === null || match[2] === "") return null;
  return { source: match[1] as "environment" | "desktop-update", receiptId: match[2] };
}
...
function actionAvailability(
  ...
  const desktop = receipts.find((receipt) => receipt.entry.source === "desktop-update")?.entry;
  ...
  const desktopActive = desktop?.state === "valid" && desktop.active && desktop.receiptId !== null;
  const availability: ManagerActionAvailabilityV1[] = [
    ...
    desktopActive && updater.resumable === true
      ? { actionId: "desktop-update.resume", available: true, reason: "resume the exact durable desktop-update receipt" }
      : unavailable("desktop-update.resume", `no resumable desktop-update receipt (${activeDetail})`),
    desktopActive
      ? { actionId: "desktop-update.cancel", available: true, reason: "cancel and recover the exact durable desktop-update receipt" }
      : unavailable("desktop-update.cancel", `no cancellable desktop-update receipt (${activeDetail})`),
    unavailable("environment.switch", "environment switching is not yet migrated to the manager adapter"),
    unavailable("desktop-update.start", "desktop-update start is not yet migrated to the manager adapter"),
    ...
    unavailable("refresh.full", "full refresh requires dedicated receipt recovery proof before manager migration"),
