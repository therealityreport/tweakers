...
exports.PROMOTION_SURFACE_NAMES = exports.PROMOTION_HEALTH_REQUEST_MAX_AGE_MS = exports.PROMOTION_ORIGINAL_RENDERER_CLEANUP_BUDGET_MS = exports.PROMOTION_ORIGINAL_RENDERER_PRELOAD_TIMEOUT_MS = exports.PROMOTION_ORIGINAL_RENDERER_MOUNT_TIMEOUT_MS = exports.PROMOTION_ORIGINAL_RENDERER_LOAD_TIMEOUT_MS = exports.PROMOTION_ORIGINAL_RENDERER_STARTUP_TIMEOUT_MS = exports.PROMOTION_ORIGINAL_RENDERER_IPC_CHANNEL = exports.PROMOTION_ORIGINAL_RENDERER_AUTH_CHANNEL = exports.PROMOTION_ORIGINAL_RENDERER_URL = exports.PROMOTION_RENDERER_HOST = exports.PROMOTION_RENDERER_SCHEME = exports.PROMOTION_RENDERER_NONCE_QUERY = exports.PROMOTION_RENDERER_AUTH_CHANNEL = exports.PROMOTION_RENDERER_IPC_CHANNEL = void 0;
...
exports.disablePromotionOriginalRendererBackgroundThrottling = disablePromotionOriginalRendererBackgroundThrottling;
exports.verifyPromotionOriginalRendererBackgroundThrottlingDisabled = verifyPromotionOriginalRendererBackgroundThrottlingDisabled;
...
exports.validatePromotionOriginalRendererLoadObserved = validatePromotionOriginalRendererLoadObserved;
...
exports.PROMOTION_ORIGINAL_RENDERER_LOAD_TIMEOUT_MS = 75_000;
exports.PROMOTION_ORIGINAL_RENDERER_MOUNT_TIMEOUT_MS = 60_000;
exports.PROMOTION_ORIGINAL_RENDERER_PRELOAD_TIMEOUT_MS = 55_000;
exports.PROMOTION_ORIGINAL_RENDERER_CLEANUP_BUDGET_MS = 5_000;
exports.PROMOTION_HEALTH_REQUEST_MAX_AGE_MS = 200_000;
...
function createPromotionOriginalRendererDeadlineController(options) {
    ...
    const loadTimeoutMs = options.loadTimeoutMs ?? exports.PROMOTION_ORIGINAL_RENDERER_LOAD_TIMEOUT_MS;
    const mountTimeoutMs = options.mountTimeoutMs ?? exports.PROMOTION_ORIGINAL_RENDERER_MOUNT_TIMEOUT_MS;
    let phase = "startup";
    let handle = null;
...
function hasUniqueSandboxedPromotionRendererProcess(processMetrics, rendererProcessId) {
    if (typeof rendererProcessId !== "number"
    ...
    for (const metric of processMetrics) {
        if (!plainRecord(metric) || metric.pid !== rendererProcessId)
            continue;
...
function validatePromotionOriginalRendererHandshake(context, payload, nonce) {
    ...
    if (!context.authorizationConsumed)
        return { accepted: false, reason: "authorization required", observation: null };
    if (!context.loadObservedConsumed)
        return { accepted: false, reason: "renderer load observation required", observation: null };
    if (context.handshakeConsumed)
        return { accepted: false, reason: "handshake already consumed", observation: null };
    if (!plainRecord(payload))
        return { accepted: false, reason: "payload invalid", observation: null };
...
function validatePromotionOriginalRendererLoadObserved(context, payload, nonce) {
    ...
    if (!context.authorizationConsumed)
        return { accepted: false, reason: "authorization required", observation: null };
    if (context.loadObservedConsumed)
        return { accepted: false, reason: "load observation already consumed", observation: null };
    if (context.handshakeConsumed)
...
async function answerPromotionHealthRequest(userRoot, probes, options = {}) {
    ...
    try {
        ...
        if (!Number.isFinite(requestedAt)
            ...
            || now - requestedAt > (options.maxAgeMs ?? exports.PROMOTION_HEALTH_REQUEST_MAX_AGE_MS))
            return false;
