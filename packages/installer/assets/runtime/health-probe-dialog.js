...
exports.applyHealthProbeDialogSuppression = applyHealthProbeDialogSuppression;
function text(value) {
 ...
 */
function applyHealthProbeDialogSuppression(input) {
    if (!input.healthCheckOnly)
        return false;
    ...
    for (const [api, original] of Object.entries(originals)) {
        if (original !== undefined && dialog[api] === original) {
            throw new Error(`health-only Electron process could not suppress dialog.${api}`);
        }
    }
    return true;
}
//# sourceMappingURL=health-probe-dialog.js.map