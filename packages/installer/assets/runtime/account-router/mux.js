...
class AccountRouterMux {
    ...
    dispatchNewThread(request) {
        ...
        for (const account of this.options.config.accounts) {
            if (account.opaqueAccountId !== first.opaqueAccountId && this.options.store.snapshot().accountEligibility[account.opaqueAccountId] === "eligible")
                attempts.push(account.opaqueAccountId);
        }
    ...
    recordFanoutResponse(issued, response) {
        ...
        if (fanout.route === "fanout_initialize_intersection") {
            ...
            const capabilities = (0, protocol_1.intersectCapabilities)(results.map((result) => (0, types_1.isPlainRecord)(result) ? result.capabilities : null));
            const schema = results.map((result) => (0, types_1.isPlainRecord)(result) ? stableJson(result.serverInfo ?? null) : "");
            ...
            const result = (0, types_1.isPlainRecord)(primary.result) ? { ...primary.result, capabilities } : { capabilities };
    ...
    routeDesktopResponse(response) {
        ...
        if (terminal.method === "account/chatgptAuthTokens/refresh") {
            ...
            if (!refreshResponseMatches(response.result, child.opaqueAccountId, this.controlSecret)) {
                ...
                child.send((0, redaction_1.redactedRouterError)(terminal.originalId, "invalid_correlation"));
                return;
            }
        }
        try {
    ...
    reconcileTerminal(threadId, notification) {
