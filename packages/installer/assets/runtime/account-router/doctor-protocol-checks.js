...
exports.runDoctorProtocolAdapterChecks = runDoctorProtocolAdapterChecks;
const protocol_1 = require("./protocol");
/** Exercises production routing and correlation without an account, filesystem writes, or backend calls. */
function runDoctorProtocolAdapterChecks(contracts) {
    return contracts.map(({ method, direction }) => {
        try {
            ...
            const message = (0, protocol_1.parseJsonRpcLine)(JSON.stringify({ method, params, ...(direction === "notification" ? {} : { id: 7 }) }));
            if (!message || scoped && (0, protocol_1.threadIdFrom)(params) !== params.threadId)
                throw new Error("JSON-RPC parsing or thread ownership extraction failed");
            const route = direction === "client" ? (0, protocol_1.classifyClientMethod)(method, params)
                : direction === "notification" ? (0, protocol_1.classifyServerNotification)(method, params)
                    : (0, protocol_1.isKnownServerRequest)(method) ? "correlated_server_request" : "unknown";
            ...
            if (route !== expected)
                throw new Error(`Production route ${route} differs from expected ${expected}`);
            if (direction !== "notification") {
                const correlations = new protocol_1.CorrelationTable();
                const entry = correlations.create(direction === "client" ? "client_to_child" : "child_to_client", "ar_doctorsynthetic0001", 7, method);
                if (entry.originalId !== 7 || correlations.get(entry.internalId)?.method !== method)
                    throw new Error("Request correlation failed");
                if (!(0, protocol_1.parseJsonRpcLine)(JSON.stringify({ id: entry.internalId, result: {} })))
                    throw new Error("Response envelope rejected");
            }
            ...
            return { method, passed: false, summary: error instanceof Error ? error.message : String(error) };
        }
    });
}
//# sourceMappingURL=doctor-protocol-checks.js.map