...
exports.BrokerProcessChild = exports.AccountsBrokerOwnerV1 = exports.ENROLLMENT_MATERIALIZATION_JOURNAL_FILE = exports.ACCOUNTS_BROKER_APP_SERVER_MAX_FRAME_BYTES = exports.ACCOUNTS_BROKER_APP_SERVER_SOCKET_FILE = void 0;
exports.brokerChildEnvironment = brokerChildEnvironment;
exports.runAccountsBrokerOwnerCli = runAccountsBrokerOwnerCli;
exports.createBrokerStartupDiagnostics = createBrokerStartupDiagnostics;
exports.installBrokerTerminationDiagnostics = installBrokerTerminationDiagnostics;
exports.connectAccountsBrokerAppServerClient = connectAccountsBrokerAppServerClient;
...
const transfer_recovery_1 = require("./transfer-recovery");
...
const canonical_history_1 = require("./canonical-history");
...
const native_legacy_projects_1 = require("./native-legacy-projects");
const native_history_1 = require("./native-history");
exports.ACCOUNTS_BROKER_APP_SERVER_SOCKET_FILE = "accounts-broker-app.v1.sock";
...
exports.ACCOUNTS_BROKER_APP_SERVER_MAX_FRAME_BYTES = 64 * 1024 * 1024;
...
exports.ENROLLMENT_MATERIALIZATION_JOURNAL_FILE = "enrollment-materialization.v1.json";
const MAX_ENROLLMENT_MATERIALIZATION_JOURNAL_BYTES = 256 * 1024;
function isNativePointHistoryRead(method) {
    return method === "thread/read" || method === "thread/turns/list" || method === "thread/items/list";
}
function isPartialNativeHistoryRead(method) {
...
class AccountsBrokerOwnerV1 {
    ...
    pendingChild = new Map();
    pendingBrokerChild = new Map();
    pendingHistory = new Map();
    pendingHistoryByChild = new Map();
    historyCache = new Map();
    historyCursors = new Map();
    nativeMergedCursors = new Map();
    nativeLegacyProjectListCursors = new Map();
    historySections = new Map();
    /** Last successful native listing home per thread; bounded and never persisted as a private row mirror. */
    nativeSectionHomes = new Map();
    /** Account-local section availability is independent of the last merged public section row. */
    nativeSectionsByAccount = new Map();
    heldDesktopContinuations = new Map();
    taskRefsByThread = new Map();
    threadByTaskRef = new Map();
    enrollmentHelpers = new Map();
    connectionTargets = new Map();
    currentConversationByRenderer = new Map();
    /** One active balanced turn per native thread follows the canonical lease. */
    balanceReservationByThread = new Map();
    /** Bounded terminal correlation lets late official cumulative totals repay terminal debt. */
    settledBalanceReservationByThread = new Map();
    ...
    nativeHistory = null;
    nativeProjects = null;
    nativeLegacyProjects = null;
    nativeLegacyProjectsReady = false;
    nativeLegacyProjectRefresh = null;
    nativeHistoryDegraded = false;
    nonce = 0;
    closed = false;
    appServer = null;
    ...
    brokerClose = null;
    sweepTimer = null;
    enrollmentMaterializationFaultAt;
    onReservationAcquired;
    recoveryCompatibilityPreflight;
    initialized = false;
    desktopInitialization = null;
    initializingDesktopRequests = new Set();
    initializedDesktopClients = new Map();
    featureEnablements = new Map();
    featureRevision = 0;
    featureBroadcastTail = Promise.resolve();
    childFeatureRevision = new WeakMap();
    childFeatureReplay = new WeakMap();
    modelCheckedRequests = new WeakSet();
    inventoryCheckedRequests = new WeakSet();
    inventoryRefresh = null;
    inventoryFreshUntil = 0;
    inventoryFreshKey = "";
    modelEligibleRequests = new WeakMap();
    modelCatalogs = new models_1.AccountModelCatalogsV1((account) => this.readAccountModels(account));
    remoteRestoreStarted = false;
    startupStarted = node_perf_hooks_1.performance.now();
    onStartupStage;
    constructor(config, stateRoot, secret, command, args, options = {}) {
        this.config = config;
        ...
        this.recoveryCompatibilityPreflight = options.recoveryCompatibilityPreflight
            ?? (() => (0, transfer_recovery_1.verifyAccountsTransferRecovery)((0, node_path_1.resolve)(__dirname, "..")));
    }
    async initializeNativeRemote() {
        if (!this.nativeHistory)
            return;
        const binding = this.nativeHistory;
        this.nativeTransfer = new native_transfer_1.NativeTransferCoordinatorV1({
            ...
            db: new native_transfer_1.Sqlite3NativeCatalogDbV1(),
            capabilityProbe: () => (0, native_transfer_1.probeNativeTransferCapabilityV1)({ command: this.command, args: this.args, cwd: process.cwd() }),
            bindingPreflight: () => !this.closed && this.nativeHistory !== null && (0, native_history_1.nativeHistoryBindingSafeV1)(this.nativeHistory),
            writerCensus: () => this.nativeHistory !== null && (0, native_history_1.observeNativeHistoryWritersV1)(this.nativeHistory, this.nativeOwnedPids()).ok,
            accountOfflinePreflight: (account) => !this.closed && this.nativeHistory !== null
                ...
                && (0, native_history_1.observeNativeHistoryWritersV1)(this.nativeHistory, this.nativeOwnedPids()).ok,
            ...
            resumedWriterLockProof: (threadId, targetAccountId, identity) => {
                ...
                return Boolean(target && typeof pid === "number" && Number.isSafeInteger(pid) && pid > 0
                    && (0, native_thread_writer_lease_1.proveNativeThreadWriterLease)((0, node_path_1.join)(target.codexHome, "thread-writer-locks"), threadId, identity, pid));
            },
            catalogThreadCensus: async (threadIds) => new Map([...await (0, native_history_1.observeNativeThreadWritersV1)(binding, threadIds, this.nativeOwnedPids())]
        ...
        this.remoteController = new remote_controller_1.NativeRemoteControllerV1({
            secret: this.secret,
            request: async (account, method, params) => {
                const child = this.broker.acquireChild(account);
                if (!child)
                    throw new Error("native request outcome unavailable");
            ...
            gate: {
                beginEnable: async (account) => {
                    ...
                    if (this.nativeTransferHeldAccounts.has(account) || this.remoteModes.get(account) === "draining" || !this.accountIsIdle(account))
                        return "busy";
    ...
    async restoreNativeRemote() {
        if (this.remoteRestoreStarted || !this.desktopInitialization || !this.remoteController)
            return;
        ...
        for (const [account, mode] of [...this.remoteModes]) {
            if (mode === "enabled")
                await this.remoteController.enable(account);
            else if (mode === "draining")
                this.setRemoteMode(account, "disabled");
        }
    }
    recoverNativeWriterCommits() {
        if (!this.nativeTransfer)
            return;
        for (const operation of this.nativeTransfer.pendingWriterCommits()) {
            ...
            if (!conversationId)
                throw new Error("native transfer recovery conversation unavailable");
            this.store.update((state) => {
                const owner = state.threadOwners[operation.threadId];
                if (owner !== operation.sourceAccountId && owner !== operation.targetAccountId)
                    throw new Error("native transfer recovery owner mismatch");
    ...
    async recoverInterruptedNativeTransfers() {
        if (!this.nativeTransfer)
            return;
        ...
        for (const account of this.config.accounts) {
            const child = this.broker.acquireChild(account.opaqueAccountId);
            if (!child)
                return this.holdInterruptedNativeTransfers(pending);
    ...
    nativeRetirementRecovery = null;
    async recoverNativeSourceRetirements() {
        if (this.nativeRetirementRecovery)
            return this.nativeRetirementRecovery;
        ...
        try {
            ...
            if (this.nativeRetirementRecovery === pending)
                this.nativeRetirementRecovery = null;
        }
    }
    async performNativeSourceRetirementRecovery() {
        ...
        if (!heldHere.size)
            return;
        try {
            for (const account of heldHere)
                await this.quiesceIdleAccount(account);
    ...
    holdInterruptedNativeTransfers(operations) {
    ...
    readRemoteModes() {
        const path = (0, node_path_1.join)(this.stateRoot, "accounts-remote-modes.v1.json");
        try {
        ...
        (0, state_store_1.assertPrivateRegularFile)(path, 16_384);
        const value = JSON.parse((0, node_fs_1.readFileSync)(path, "utf8"));
        if (!(0, types_1.isPlainRecord)(value) || value.version !== 1 || Object.keys(value).sort().join("\0") !== "accounts\0version" || !Array.isArray(value.accounts))
            throw new Error("invalid remote mode state");
        for (const row of value.accounts) {
            if (!(0, types_1.isPlainRecord)(row) || Object.keys(row).sort().join("\0") !== "accountId\0mode" || !(0, types_1.isOpaqueAccountId)(row.accountId)
                || !this.config.accounts.some((account) => account.opaqueAccountId === row.accountId) || this.remoteModes.has(row.accountId)
                || (row.mode !== "disabled" && row.mode !== "draining" && row.mode !== "enabled"))
                throw new Error("invalid remote account mode");
            this.remoteModes.set(row.accountId, row.mode);
        }
    }
    setRemoteMode(account, mode) {
        const next = new Map(this.remoteModes);
        next.set(account, mode);
        (0, state_store_1.writePrivateJsonAtomic)(this.stateRoot, "accounts-remote-modes.v1.json", { version: 1, accounts: [...next].map(([accountId, mode]) => ({ accountId, mode })) });
        this.remoteModes.clear();
        for (const [id, state] of next)
            this.remoteModes.set(id, state);
    }
    remoteBlocksDesktop(account) {
        if (this.nativeTransferHeldAccounts.has(account))
            return true;
        const mode = this.remoteModes.get(account);
        return mode === "draining" || mode === "enabled";
    }
    accountIsIdle(account, ignoredHandoff) {
        return ![...this.heldDesktopContinuations.keys()].some((key) => key !== ignoredHandoff && this.broker.pendingHandoff(key)?.fromOpaqueAccountId === account)
    ...
    async quiesceIdleAccount(account, ignoredHandoff) {
        if (!this.accountIsIdle(account, ignoredHandoff))
            return false;
        const existing = this.children.get(account);
        if (!existing)
            return true;
        const released = this.broker.releaseIdleChild(account);
        if (!released || released !== existing)
            return false;
        let timer;
        try {
            return await Promise.race([released.whenClosed().then(() => true), new Promise((resolve) => { timer = setTimeout(() => resolve(false), 5_000); })]);
        }
        finally {
            if (timer)
                clearTimeout(timer);
    ...
    async loadedNativeThreads(account) {
        ...
        do {
            ...
            if (!response || response.error || !(0, types_1.isPlainRecord)(response.result) || !Array.isArray(response.result.data)
                || response.result.data.length > 16_384 || !response.result.data.every((id) => typeof id === "string" && /^[A-Za-z0-9_-]{1,128}$/.test(id)))
                return null;
            for (const id of response.result.data) {
                if (ids.has(id))
                    return null;
                ids.add(id);
            }
            ...
            if (next === null || next === undefined)
                return [...ids];
            if (typeof next !== "string" || next.length < 1 || next.length > 4096 || cursors.has(next))
                return null;
            cursors.add(next);
            cursor = next;
    ...
    async dispatchRemoteAction(command, accountId, deviceId) {
        if (!this.remoteController)
            throw new broker_1.BrokerCommandError("broker_unavailable", true);
        if (command === "remote.enable" && this.remoteModes.get(accountId) !== "enabled" && !this.accountIsIdle(accountId))
            throw new broker_1.BrokerCommandError("account_history_busy", true);
        switch (command) {
            ...
            case "remote.pairing.start": {
                if (this.remoteModes.get(accountId) !== "enabled") {
                    ...
                    if (!enabled.enabled || enabled.state === "unavailable")
                        return enabled;
                }
            ...
            case "remote.pairing.close":
                this.remoteController.closePairing(accountId);
                return this.remoteController.status(accountId);
    ...
    async reconcileRemoteActivity() {
        if (this.closed || !this.nativeTransfer || this.remoteReconciliationPending)
            return;
        ...
        try {
            if (this.nativeTransfer.preflightSharedWriterLocks().state !== "ready") {
                for (const [account, mode] of this.remoteModes)
                    if (mode === "enabled")
                        await this.remoteController?.disable(account);
                return;
            }
            ...
            for (const threadId of reconciled.updatedThreadIds) {
                ...
                if (conversation && this.canonicalHistory.hasActiveTurn(conversation))
                    continue;
                if (conversation)
                    this.canonicalHistory.reconcileNativeWriter(conversation, owner, threadId);
                this.store.update((state) => {
                    if (previous)
                        state.ledger[previous].assignedThreadCount = Math.max(0, state.ledger[previous].assignedThreadCount - 1);
                    state.threadOwners[threadId] = owner;
                    state.ledger[owner].assignedThreadCount += 1;
                });
                const task = this.taskRefsByThread.get(threadId);
                if (task && previous)
                    this.broker.transferNativeTask(task, previous, owner);
            ...
            this.remoteReconciliationPending = false;
        }
    }
    /**
    ...
    */
    initializeAfterElection() {
        ...
        if (!(0, app_server_mux_1.preflightRouterHomes)(this.config, this.stateRoot)) {
        ...
        this.tokenBalance = new token_balance_1.TokenBalanceLedger({
            root: this.stateRoot,
        ...
        const store = new state_store_1.RouterStateStore(this.stateRoot, this.config);
        this.store = store;
        ...
        this.canonicalHistory = new canonical_history_1.CanonicalHistoryStoreV1(this.stateRoot, Date.now, node_crypto_1.randomBytes, (nativeThreadId) => `lh_${(0, node_crypto_1.createHmac)("sha256", this.secret).update(`canonical-history:v1:${nativeThreadId}`, "utf8").digest("base64url")}`);
        ...
        this.preferences = new preferences_1.AccountsPreferencesStore(this.stateRoot);
        this.profileStatistics = new profile_statistics_1.NativeProfileStatisticsV1({ secret: this.secret, accounts: () => this.config.accounts.map((account) => ({
                accountId: account.opaqueAccountId, enabled: account.included,
                codexHome: this.nativeHistory ? this.isolatedAuthHome(account.opaqueAccountId) ?? this.nativeAccountBinding(account.opaqueAccountId).codexHome : (0, node_path_1.join)(this.stateRoot, "accounts", account.opaqueAccountId, "codex-home"),
            })) });
        this.broker = new broker_1.AccountsBrokerV1({
            accounts: (0, broker_1.accountPoolSeedsFromRouterConfigV3)(this.config, store.snapshot().ledger),
            secret: this.secret,
            childFactory: {
            ...
            onEnrollmentSettled: (enrollment) => {
                if ((enrollment.kind === "enrollment" && enrollment.opaqueAccountId === null) || this.enrollmentHelpers.get(enrollment.enrollmentRef)?.isolatedReconnect)
                    this.retireEnrollmentHelper(enrollment.enrollmentRef);
            },
            onAccountSettingsChanged: (change) => {
                const saved = this.persistAccountSettings(change);
                if (saved && change.enabled === true)
                    queueMicrotask(() => { void this.warmEnabledChildren(); });
                return saved;
            },
            ...
            onPreferencesUpdate: (patch) => {
                if (patch.unifiedCatalogEnabled === true && this.nativeTransfer?.preflightSharedWriterLocks().state !== "ready")
                    throw new broker_1.BrokerCommandError("broker_unavailable", true);
                const preferences = this.preferences.update(patch);
                if (preferences.unifiedCatalogEnabled)
        ...
        if (this.nativeHistory) {
            ...
            this.nativeProjects = new native_projects_1.NativeProjectLinksV1(this.stateRoot, this.nativeHistory.source.metadataAccountId, this.secret, (account, method, params) => this.requestNativeProject(account, method, params));
            const metadataSource = this.nativeAccountBinding(this.nativeHistory.source.metadataAccountId);
            if (!metadataSource)
                throw new Error("native history metadata source is unavailable");
            this.nativeLegacyProjects = new native_legacy_projects_1.NativeLegacyProjectsV1(this.stateRoot, metadataSource.codexHome, this.nativeHistory.source.metadataAccountId);
        }
    ...
    remoteModes = new Map();
    remoteReconciliationPending = false;
    nativeTransferHeldAccounts = new Set();
    nativeRetirementHandoffs = new Map();
    nativeContinuityDeferred = new Set();
    nativeContinuityReasons = new Map();
    async start() {
        if (this.closed)
            throw new Error("accounts broker owner is closed");
        ...
        const reservation = await (0, broker_socket_1.reserveAccountsBrokerSocket)({ root: this.stateRoot, secret: this.secret }).catch((error) => {
            this.reportStartup("election", "unavailable");
            throw error;
        });
        this.reportStartup("election", "ready");
        this.brokerClose = () => reservation.close();
        let startupPhase = "recovery";
        try {
            ...
            this.sweepTimer = setInterval(() => {
                this.broker.sweep();
                if (this.nativeHistory && this.children.size > 0)
                    this.nativeHistoryWritersSafe();
                if (this.nativeHistory && this.children.size > 0 && this.desktopInitialization)
                    void this.ensureNativePluginInventory();
                void this.reconcileRemoteActivity();
            }, OWNER_IDLE_SWEEP_MS);
    ...
    reportStartup(stage, code) {
        try {
            this.onStartupStage?.({ stage, code, elapsedMs: Math.max(0, Math.min(600_000, Math.round(node_perf_hooks_1.performance.now() - this.startupStarted))) });
        }
    ...
    async close() {
        ...
        this.clients.clear();
        for (const socket of this.appConnections)
            socket.destroy();
        ...
        for (const pending of this.pendingDesktop.values())
            clearTimeout(pending.timer);
        this.pendingDesktop.clear();
        for (const helper of this.enrollmentHelpers.values()) {
            if (helper.isolatedReconnect) {
                clearTimeout(helper.isolatedReconnect.timer);
                helper.isolatedReconnect.preimage.fill(0);
                await helper.child.terminateAndWait();
            }
            else
                try {
                    helper.child.terminate("shutdown");
                }
                catch { }
        }
        ...
        if (this.appServer?.listening) {
            ...
            await new Promise((resolvePromise) => this.appServer.close(() => resolvePromise()));
        }
        this.appServer = null;
    ...
    mapNativeTargets(rendererRef, request) {
        if (!this.clients.has(rendererRef) || !isNativeTargetMapRequest(request))
        ...
        if (composerConversation !== conversationId)
            return { version: 1, status: "unavailable" };
        const turns = this.canonicalHistory.publicTurnIdsForConversation(conversationId, request.assistantTurnNativeIds);
        if (!turns || turns.size !== request.assistantTurnNativeIds.length)
            return { version: 1, status: "unavailable" };
        return {
            version: 1,
            status: "mapped",
            conversationId: `conversation_${(0, node_crypto_1.createHmac)("sha256", this.secret).update(`renderer-conversation:v1:${conversationId}`, "utf8").digest("base64url")}`,
            turnIds: request.assistantTurnNativeIds.map((nativeTurnId) => `turn_${(0, node_crypto_1.createHmac)("sha256", this.secret).update(`renderer-turn:v1:${turns.get(nativeTurnId)}`, "utf8").digest("base64url")}`),
        };
    }
    /** Main-only selected-home context; no credential or account identity leaves the owner. */
    async resolveNativeBrowserContext(rendererRef, opaqueAccountId) {
        if (!(0, types_1.isOpaqueAccountId)(opaqueAccountId) || !this.clients.has(rendererRef) || !this.broker.hasAuthenticatedRenderer(rendererRef)) {
            return { version: 1, status: "unavailable" };
        }
        ...
        const child = this.broker.acquireChild(opaqueAccountId);
        if (!child || !await this.initializeChild(child) || !(0, types_1.isPlainRecord)(child.initializeResult)
            || typeof child.initializeResult.userAgent !== "string")
            return { version: 1, status: "unavailable" };
        ...
        return { version: 1, status: "ready", opaqueAccountId, codexHome: binding.codexHome,
            configFile: (0, node_path_1.join)(binding.codexHome, "config.toml"), appServerVersion: version };
    }
    async invokeNativeBrowserRequest(rendererRef, opaqueAccountId, method, params) {
        const request = (0, native_request_1.parseNativeBrowserChildRequestV1)(method, params);
        if (!request || !(0, types_1.isOpaqueAccountId)(opaqueAccountId) || !this.clients.has(rendererRef)
            || !this.broker.hasAuthenticatedRenderer(rendererRef) || !this.nativeAccountBinding(opaqueAccountId))
            return null;
        const pool = this.broker.pool().accounts.find((account) => account.opaqueAccountId === opaqueAccountId);
        if (!pool || !pool.enabled || pool.state === "disabled" || pool.state === "reauth_required" || pool.state === "unhealthy")
            return null;
        const child = this.broker.acquireChild(opaqueAccountId);
        ...
        return response && !response.error && (0, native_request_1.isBoundedNativeResultV1)(response.result, "plugins") ? response.result : null;
    }
    /**
    ...
    persistPendingHandoff(handoff) {
        this.store.update((state) => {
            const handoffs = state.pendingHandoffs ?? (state.pendingHandoffs = {});
            if (Object.keys(handoffs).length >= 64)
                throw new Error("accounts broker handoff metadata capacity reached");
            if (handoffs[handoff.handoffRef])
                throw new Error("duplicate accounts broker handoff receipt");
    ...
    persistRetargetedHandoff(handoff) {
        this.store.update((state) => {
            ...
            if (!receipt || receipt.state !== "pending" || receipt.fromOpaqueAccountId !== handoff.fromOpaqueAccountId
                || receipt.conversationId !== handoff.conversationId)
                throw new Error("pending handoff receipt unavailable");
            receipt.toOpaqueAccountId = handoff.toOpaqueAccountId;
        });
    }
    persistHandoffSettlement(handoff, state) {
        this.store.update((record) => {
            ...
            if (existing)
                ...
                handoffs[handoff.handoffRef] = persistentHandoff(handoff, "ambiguous");
        });
    }
    settleHandoff(handoff, state) {
        ...
        if (held.automaticCapacityRefreshKey)
            this.automaticCapacityRefreshRequests.delete(held.automaticCapacityRefreshKey);
        if (held.forwardChildId)
            this.removePendingDesktop(held.forwardChildId);
        held.client.outstanding = Math.max(0, held.client.outstanding - 1);
        this.sendDesktop(held.client, (0, redaction_1.redactedRouterError)(held.desktopId, state === "ambiguous" ? "ambiguous_dispatch"
     ...
     */
    persistAccountSettings(change) {
        const previousConfig = this.config;
        if (change.enabled === false && this.remoteBlocksDesktop(change.opaqueAccountId))
            throw new broker_1.BrokerCommandError("account_history_busy", true);
        const current = this.config.accounts.find((account) => account.opaqueAccountId === change.opaqueAccountId);
        if (!current || (change.label === undefined && change.enabled === undefined))
            return false;
        ...
        const next = { ...draft, fingerprint: (0, config_1.routerConfigFingerprint)(draft) };
        const existingState = this.store.snapshot();
        if (!(0, state_store_1.validateRouterState)(existingState, next))
            return false;
        try {
            ...
            try {
                (0, state_store_1.writePrivateJsonAtomic)(this.stateRoot, config_1.ACCOUNT_ROUTER_CONFIG_FILE, previousConfig);
            }
            catch { }
            return false;
        }
     ...
     * setting change.
     */
    persistBalanceSetting(enabled) {
        const previousConfig = this.config;
        if (!enabled && previousConfig.mode === "manual")
            return true;
        const draft = enabled
            ? {
                ...previousConfig,
        ...
        const next = { ...draft, fingerprint: (0, config_1.routerConfigFingerprint)(draft) };
        if (!(0, state_store_1.validateRouterState)(this.store.snapshot(), next))
            return false;
        try {
            (0, state_store_1.writePrivateJsonAtomic)(this.stateRoot, config_1.ACCOUNT_ROUTER_CONFIG_FILE, next);
            this.config = next;
    ...
    eligibleAutomaticAccounts(exclude = null) {
     ...
     */
    deferForAutomaticCapacityRefresh(client, request) {
        if (this.config.mode !== "quota_aware" || (request.method !== "thread/start" && request.method !== "turn/start"))
            return false;
        ...
        if (this.automaticCapacityRefreshRequests.has(key)) {
            ...
            this.sendDesktop(client, (0, redaction_1.redactedRouterError)(request.id, "invalid_correlation"));
            return true;
        }
        const quotas = new Map(this.broker.quota().map((quota) => [quota.opaqueAccountId, quota]));
    ...
    refreshAutomaticCapacity(account) {
        ...
        const refresh = (async () => {
            try {
                ...
                if (projection && this.automaticAccountAuthenticated.get(account) === true)
                    this.broker.updateQuota({ opaqueAccountId: account, ...projection });
                else {
                    const previous = this.broker.quota().find((quota) => quota.opaqueAccountId === account);
                    if (previous)
                        this.broker.updateQuota({ ...previous, freshness: "stale" });
                }
            }
    ...
    readBalance() {
        ...
        const policy = this.config.mode === "manual"
            ? "manual"
        ...
        const degradedReason = policy !== "balanced_tokens_v1"
            ? null
    ...
    async dispatchDeviceAction(action) {
        ...
        if (action.kind === "device.start") {
            ...
            if (!response || response.error || !(0, types_1.isPlainRecord)(response.result) || !isDeviceStartResult(response.result)) {
                if (helper.isolatedReconnect)
        ...
        if (action.kind === "device.status") {
            ...
            if (!helper || helper.loginId !== action.loginId)
                return { outcome: "rejected" };
        ...
        if (action.kind === "native.request" && action.surface === "plugins" && !await this.ensureNativePluginInventory())
            return { outcome: "rejected" };
        const child = this.broker.acquireChild(action.opaqueAccountId);
        if (!child)
            return { outcome: "rejected" };
        if (action.kind === "profile.email") {
            const response = await this.requestBrokerChild(action.opaqueAccountId, child, "account/read", {});
            const result = response && !response.error && (0, types_1.isPlainRecord)(response.result) ? response.result : null;
            const email = result && (0, types_1.isPlainRecord)(result.account) ? result.account.email : null;
        ...
        if (action.kind === "native.request") {
            if (this.remoteBlocksDesktop(action.opaqueAccountId))
                throw new broker_1.BrokerCommandError("account_history_busy", true);
            const scopedThreadId = (0, types_1.isPlainRecord)(action.params) && typeof action.params.threadId === "string" ? action.params.threadId : null;
            if (scopedThreadId && this.store.snapshot().threadOwners[scopedThreadId] !== action.opaqueAccountId)
                return { outcome: "rejected" };
            if ((action.method === "config/value/write" || action.method === "config/batchWrite") && (0, types_1.isPlainRecord)(action.params)
                ...
                const activeConfig = mode?.state === "ready" ? (0, node_path_1.join)(mode.document.overlay.path, "config.toml") : binding ? (0, node_path_1.join)(binding.codexHome, "config.toml") : null;
                if (!binding || typeof action.params.filePath !== "string"
                    || mode?.state === "blocked" || (0, node_path_1.resolve)(action.params.filePath) !== activeConfig)
                    return { outcome: "rejected" };
            }
            try {
                let value;
                if (action.method === "http.request" && (action.surface === "apps" || action.surface === "plugins")) {
                    ...
                    if (action.surface === "plugins" && this.sharedNativeMode()?.state === "ready"
                        && (0, types_1.isPlainRecord)(action.params) && action.params.verb === "POST"
                        && typeof action.params.path === "string" && /^\/ps\/plugins\/\{plugin_id\}\/(install|uninstall|enable|disable)$/.test(action.params.path))
                        return { outcome: "rejected" };
                    const binding = this.nativeAccountBinding(action.opaqueAccountId);
                    if (!binding)
                        return { outcome: "rejected" };
                    if (this.isolatedAuthHome(action.opaqueAccountId) && !await this.refreshIsolatedAuth(action.opaqueAccountId))
                        return { outcome: "rejected" };
                    value = await (0, native_request_1.requestNativeHttpV1)(this.isolatedAuthHome(action.opaqueAccountId) ?? binding.codexHome, action.surface, action.params);
                }
                    ...
                    if (this.isolatedAuthHome(action.opaqueAccountId) && !await this.refreshIsolatedAuth(action.opaqueAccountId))
                        return { outcome: "rejected" };
                    value = await (0, native_request_1.requestNativeUsageCreditsV1)(this.isolatedAuthHome(action.opaqueAccountId) ?? binding.codexHome, action.method, action.params);
                }
                else {
                    if (!await this.initializeChild(child))
                        return { outcome: "rejected" };
                    const response = await this.requestBrokerChild(action.opaqueAccountId, child, action.method, action.params);
                    if (!response || response.error)
                        return { outcome: response ? "rejected" : "ambiguous" };
                    value = response.result;
                    await this.refreshSharedNativeRuntimes(action.method);
                }
                return (0, native_request_1.isBoundedNativeResultV1)(value, action.surface) ? { outcome: "accepted", value } : { outcome: "rejected" };
            }
            catch {
                return { outcome: "ambiguous" };
            }
        }
        if ((action.kind === "resetCredit.consume" || action.kind === "connection.authorize") && this.remoteBlocksDesktop(action.opaqueAccountId))
            throw new broker_1.BrokerCommandError("account_history_busy", true);
        if (action.kind === "resetCredit.consume") {
            ...
            return response && !response.error && (0, types_1.isPlainRecord)(response.result) && typeof response.result.outcome === "string"
    ...
    createEnrollmentHelper(enrollmentRef) {
        const existing = this.enrollmentHelpers.get(enrollmentRef);
        if (existing)
            return existing;
        const synthetic = `ar_${(0, node_crypto_1.createHmac)("sha256", this.secret).update(`enrollment-helper:${enrollmentRef}`, "utf8").digest("base64url")}`;
        const root = (0, node_path_1.join)(this.stateRoot, "enrollments", enrollmentRef);
        try {
            ...
            (0, node_fs_1.mkdirSync)((0, node_path_1.join)(root, "codex-home"), { recursive: true, mode: 0o700 });
            (0, node_fs_1.mkdirSync)((0, node_path_1.join)(root, "sqlite-home"), { recursive: true, mode: 0o700 });
            const codexHome = (0, node_path_1.join)(root, "codex-home");
            if (this.nativeHistory)
                (0, node_fs_1.writeFileSync)((0, node_path_1.join)(codexHome, "config.toml"), "", { flag: "wx", mode: 0o600 });
            else if (!(0, app_server_mux_1.materializeSharedSkillsIntoAccount)(this.stateRoot, codexHome) || !(0, app_server_mux_1.materializeSharedPluginsIntoAccount)(this.stateRoot, codexHome))
                return null;
            const childArgs = this.nativeHistory ? this.args : (0, app_server_mux_1.sharedPluginChildArgs)(this.stateRoot, this.args);
            if (!childArgs)
                return null;
            const childProcess = (0, node_child_process_1.spawn)(this.command, childArgs, {
                cwd: process.cwd(),
                env: brokerChildEnvironment((0, node_path_1.join)(root, "codex-home"), (0, node_path_1.join)(root, "sqlite-home")),
            ...
            helper.child = new BrokerProcessChild(synthetic, childProcess, (message) => this.receiveChild(synthetic, helper.child, message), () => { if (this.enrollmentHelpers.get(enrollmentRef) === helper)
    ...
    reconnectHelper(enrollmentRef, opaqueAccountId) {
        ...
        if (authHome) {
            if ((this.authHelperQueued.get(opaqueAccountId) ?? 0) > 0 || this.activeAuthHelpers.has(opaqueAccountId) || [...this.enrollmentHelpers.values()].some((helper) => helper.opaqueAccountId === opaqueAccountId && helper.isolatedReconnect))
                return null;
            const root = (0, node_fs_1.mkdtempSync)((0, node_path_1.join)(authHome, ".reconnect-"));
            (0, node_fs_1.chmodSync)(root, 0o700);
            (0, node_fs_1.writeFileSync)((0, node_path_1.join)(root, "config.toml"), "", { flag: "wx", mode: 0o600 });
            const preimage = (0, native_auth_binding_1.readNativeAuthPrivateFileV1)((0, node_path_1.join)(authHome, "auth.json"));
            const synthetic = `ar_${(0, node_crypto_1.createHmac)("sha256", this.secret).update(`reconnect-helper:${enrollmentRef}`).digest("base64url")}`;
            const child = new BrokerProcessChild(synthetic, (0, node_child_process_1.spawn)(this.command, credentialStoreArgs(this.args, "file"), {
                cwd: root, env: brokerChildEnvironment(root, root), stdio: ["pipe", "pipe", "ignore"],
            }), (message) => this.receiveChild(synthetic, child, message), () => { });
            const timer = setTimeout(() => this.retireEnrollmentHelper(enrollmentRef), 15 * 60_000);
            timer.unref();
            const helper = { enrollmentRef, child, transportAccountId: synthetic, root, opaqueAccountId, loginId: null, completion: null,
        ...
        if (!child)
            return null;
        const helper = { enrollmentRef, child, transportAccountId: opaqueAccountId, root: null, opaqueAccountId, loginId: null, completion: null };
        this.enrollmentHelpers.set(enrollmentRef, helper);
        return helper;
    }
    /** The native base's current credentials select the inventory actor, independently of history/routing ownership. */
    nativeInventoryBinding() {
        try {
            const mode = this.sharedNativeMode();
            if (mode?.state !== "ready")
                return null;
            const raw = (0, app_server_mux_1.readOwnerPrivateAuthAccountId)(mode.document.nativeBase.path);
            if (!raw)
                return null;
            const actor = `ar_${(0, node_crypto_1.createHmac)("sha256", this.secret).update(`account-router:v1:${raw}`).digest("base64url")}`;
            ...
            if (matches.length !== 1 || !entry || entry.authIdentityHmac !== (0, native_history_1.nativeHistoryAuthIdentityHmacV1)(raw, this.secret))
                return null;
            return { actor, mode, key: `${mode.fingerprint}:${actor}`, directory: (0, node_path_1.join)(mode.document.overlay.path, "plugins") };
        }
        catch {
            return null;
        }
    ...
    cachedNativeInventory(binding) {
        try {
            const directory = (0, node_fs_1.lstatSync)(binding.directory);
            if (!directory.isDirectory() || directory.isSymbolicLink() || directory.uid !== process.getuid?.() || (directory.mode & 0o077) !== 0)
                return null;
            const inventory = JSON.parse((0, native_auth_binding_1.readNativeAuthPrivateFileV1)((0, node_path_1.join)(binding.directory, NATIVE_INVENTORY_FILE), NATIVE_INVENTORY_MAX_BYTES).toString("utf8"));
            const provenance = JSON.parse((0, native_auth_binding_1.readNativeAuthPrivateFileV1)((0, node_path_1.join)(this.stateRoot, NATIVE_INVENTORY_PROVENANCE_FILE), 4096).toString("utf8"));
            if (!isNativePluginInventory(inventory, binding.mode.document.nativeBase.path) || !(0, types_1.isPlainRecord)(provenance)
                ...
                || provenance.inventoryDigest !== durableDigest(inventory))
                return null;
            return inventory;
        }
        catch {
    ...
    async ensureNativePluginInventory(force = false, reloadChanged = true) {
        const mode = this.sharedNativeMode();
        if (!mode || mode.state === "absent")
            return true;
        if (mode.state !== "ready" || this.closed)
            return false;
        const binding = this.nativeInventoryBinding();
        if (!binding)
            return false;
        const cached = this.cachedNativeInventory(binding);
        if (!force && cached && this.inventoryFreshKey === binding.key && Date.now() < this.inventoryFreshUntil)
            return true;
        ...
        const work = (async () => {
            try {
                const child = this.broker.acquireChild(binding.actor);
                if (!child || !await this.initializeChild(child))
                    throw new Error("native inventory actor unavailable");
                ...
                if (this.closed || this.nativeInventoryBinding()?.key !== binding.key)
                    return false;
                if (!inventory)
                    return this.cachedNativeInventory(binding) !== null;
                ...
                if (changed) {
                    ...
                    if (!(0, node_fs_1.existsSync)(binding.directory))
                        (0, node_fs_1.mkdirSync)(binding.directory, { mode: 0o700 });
                    const fd = (0, node_fs_1.openSync)(binding.directory, node_fs_1.constants.O_RDONLY | node_fs_1.constants.O_NOFOLLOW);
                    try {
                        const stat = (0, node_fs_1.fstatSync)(fd);
                        if (!stat.isDirectory() || stat.uid !== process.getuid?.() || (stat.mode & 0o022) !== 0)
                            throw new Error("unsafe native inventory directory");
                        (0, node_fs_1.fchmodSync)(fd, 0o700);
                        const current = (0, node_fs_1.lstatSync)(binding.directory);
                        if (current.isSymbolicLink() || current.dev !== stat.dev || current.ino !== stat.ino)
                            throw new Error("native inventory directory changed");
                    }
                    finally {
                        (0, node_fs_1.closeSync)(fd);
                    }
                    (0, state_store_1.writePrivateJsonAtomicBounded)(binding.directory, NATIVE_INVENTORY_FILE, inventory, NATIVE_INVENTORY_MAX_BYTES);
                }
                if (changed || !cached)
                    (0, state_store_1.writePrivateJsonAtomicBounded)(this.stateRoot, NATIVE_INVENTORY_PROVENANCE_FILE, { version: 1, actor: binding.actor, modeFingerprint: binding.mode.fingerprint, inventoryDigest: digest }, 4096);
                if (changed && reloadChanged)
                    await this.reloadSharedNativeChildren();
        ...
        try {
            ...
            if (this.inventoryRefresh?.promise === work)
                this.inventoryRefresh = null;
        }
    }
    deferForNativePluginInventory(client, request) {
        if (!nativeInventoryConsumer(request.method) || this.inventoryCheckedRequests.has(request) || !this.nativeHistory)
            return false;
        const mode = this.sharedNativeMode();
        if (!mode || mode.state === "absent")
            return false;
    ...
    async reloadSharedNativeChildren() {
    ...
    async refreshSharedNativeRuntimes(method) {
        if (!["config/value/write", "config/batchWrite", "plugin/install", "plugin/uninstall", "marketplace/add", "marketplace/remove", "marketplace/upgrade", "skills/config/write"].includes(method)
            || this.sharedNativeMode()?.state !== "ready")
            return;
        if (await this.ensureNativePluginInventory(true, false))
            await this.reloadSharedNativeChildren();
    }
    async requestBrokerChild(account, child, method, params, deadlineAt) {
        if (nativeInventoryConsumer(method) && !await this.ensureNativePluginInventory())
            return null;
        if (method === "account/read" && this.children.get(account) === child && this.isolatedAuthHome(account)) {
            return this.withAuthHelper(account, async (helper, home) => {
                ...
                if (!entry)
                    return null;
                (0, native_auth_binding_1.readNativeExternalTokensV1)(home, entry, this.secret);
                const response = await helper.requestPrivate("account/read", { refreshToken: false });
                if (!response || response.error || !providerAuthenticated(response.result) || this.isolatedAuthHome(account) !== home)
                    return null;
                (0, native_auth_binding_1.readNativeExternalTokensV1)(home, entry, this.secret);
                return response;
            });
    ...
    async requestInitializedBrokerChild(account, child, method, params, deadlineAt) {
        if (!child.ready || this.closed || !this.nativeHistoryWritersSafe())
            return null;
        ...
        if (desktopWrite && this.remoteBlocksDesktop(account))
            return null;
        if (this.nativeHistory && method === "mcpServer/oauth/login" && !this.nativeAccountOperationSafe(account, child))
            return null;
        const threadId = (0, protocol_1.threadIdFrom)(params);
        if (this.nativeHistory && threadId && !isReadOnlyNativeAccountMethod(method) && this.nativeThreadConflict(threadId) !== "clear")
            return null;
     ...
     */
    async requestNativeProject(account, method, params) {
        if (!this.nativeHistory || !this.nativeAccountBinding(account) || !this.nativeHistoryWritersSafe())
            return null;
        const child = this.broker.acquireChild(account);
        if (!child)
            return null;
        if (nativeProjectWriteMethod(method) && (this.remoteBlocksDesktop(account) || !this.nativeHistoryWritersSafe()))
            return null;
        const response = await this.requestBrokerChild(account, child, method, params);
     ...
     */
    refreshNativeLegacyProjects() {
        if (this.nativeLegacyProjectsReady)
            return Promise.resolve(true);
        const active = this.nativeLegacyProjectRefresh;
        if (active)
            return active;
        const work = (async () => {
            if (!this.nativeHistory || !this.nativeLegacyProjects || !this.nativeHistoryWritersSafe())
                return false;
            ...
            if (!response || response.error || !(0, types_1.isPlainRecord)(response.result)
    ...
    async readProviderConnections(opaqueAccountId, kind, child) {
        ...
        for (let page = 0; page < 32; page += 1) {
            ...
            for (let attempt = 0; attempt < attempts; attempt += 1) {
                ...
                if (attempt + 1 < attempts)
                    await new Promise((resolvePromise) => setTimeout(resolvePromise, PLUGIN_CONNECTION_RETRY_MS));
            }
            if (!response || response.error)
                return null;
            const entries = providerConnectionRows(response.result, kind);
            if (!entries || rows.length + entries.length > 2048)
                return null;
            ...
            const next = (0, types_1.isPlainRecord)(response.result) ? response.result.nextCursor : undefined;
            if (kind === "plugin" || next === undefined || next === null)
                break;
            if (typeof next !== "string" || next.length === 0 || next.length > 4096 || seenCursors.has(next) || page === 31)
                return null;
            seenCursors.add(next);
            cursor = next;
        }
        const projected = rows.map((row) => {
            const definitionRef = `bd_${(0, node_crypto_1.createHmac)("sha256", this.secret).update(`provider-connection:${opaqueAccountId}:${kind}:${row.name}`, "utf8").digest("base64url")}`;
    ...
    async materializeEnrollment(enrollment) {
        if (enrollment.kind !== "enrollment" || enrollment.opaqueAccountId !== null)
            return null;
        const helper = this.enrollmentHelpers.get(enrollment.enrollmentRef);
        if (!helper?.root || !helper.completion?.success)
            return null;
        ...
        const rawAccountId = (0, app_server_mux_1.readOwnerPrivateAuthAccountId)((0, node_path_1.join)(source, "codex-home"));
        if (!rawAccountId)
            return null;
        const opaqueAccountId = `ar_${(0, node_crypto_1.createHmac)("sha256", this.secret).update(`account-router:v1:${rawAccountId}`, "utf8").digest("base64url")}`;
        if (this.config.accounts.some((account) => account.opaqueAccountId === opaqueAccountId))
            return null;
        const target = (0, node_path_1.join)(this.stateRoot, "accounts", opaqueAccountId);
        ...
        if (!safeProfile || !quota || quota.error || !providerQuotaProjection(quota.result))
            return null;
        helper.child.terminate("capacity");
        await helper.child.whenClosed();
        if (this.config.accounts.some((account) => account.opaqueAccountId === opaqueAccountId) || (0, node_fs_1.existsSync)(target))
            return null;
        ...
        const next = { ...nextWithoutFingerprint, fingerprint: (0, config_1.routerConfigFingerprint)(nextWithoutFingerprint) };
        ...
        const migratedState = (0, state_store_1.migrateIdleRouterStateV3)(priorState, next);
        if (!migratedState)
            return null;
        ...
        try {
            ...
            (0, state_store_1.ensurePrivateDirectory)((0, node_path_1.join)(this.stateRoot, "accounts"));
            assertPrivateEnrollmentDirectory(source);
            if ((0, node_fs_1.existsSync)(target))
                throw new Error("enrollment target already exists");
            ...
            (0, state_store_1.writePrivateJsonAtomic)(this.stateRoot, "router-state.json", migratedState);
            ...
            if (journal.nativeExtension) {
                ...
                if (native.state !== "ready")
                    throw new Error("new native account union unavailable");
                this.nativeHistory = native.binding;
                if (this.nativeTransfer?.updateAccounts(native.binding.accounts.map((account) => ({ accountId: account.opaqueAccountId, codexHome: account.codexHome, sqliteHome: account.sqliteHome }))).state !== "ready")
                    throw new Error("native account coordinator admission failed");
                this.nativeTransfer.provisionSharedWriterLocks();
            }
            this.syncTokenBalanceAccounts();
            ...
            if (!(error instanceof EnrollmentMaterializationFault)) {
                try {
                    recoverEnrollmentMaterialization(this.stateRoot, priorConfig, this.secret);
                }
                catch { /* restart remains the recovery boundary */ }
            }
            return null;
        }
    }
    injectEnrollmentMaterializationFault(point) {
        if (this.enrollmentMaterializationFaultAt === point)
            throw new EnrollmentMaterializationFault(point);
    }
    retireEnrollmentHelper(enrollmentRef) {
        ...
        if (helper.isolatedReconnect) {
            clearTimeout(helper.isolatedReconnect.timer);
            helper.isolatedReconnect.preimage.fill(0);
            void helper.child.terminateAndWait();
            return;
        }
        try {
            helper.child.terminate("capacity");
        ...
        if (!helper.root || !(0, node_fs_1.existsSync)(helper.root))
            return;
        ...
        const retiredRoot = (0, node_path_1.join)(this.stateRoot, "enrollment-retired");
        try {
            (0, node_fs_1.mkdirSync)(retiredRoot, { recursive: true, mode: 0o700 });
            (0, node_fs_1.renameSync)(helper.root, (0, node_path_1.join)(retiredRoot, `${enrollmentRef}.${Date.now()}`));
        }
        catch { /* a private orphan is safer than broad deletion */ }
    }
    async startAppServerSocket() {
        const path = (0, broker_socket_1.accountsBrokerSocketPath)(this.stateRoot, exports.ACCOUNTS_BROKER_APP_SERVER_SOCKET_FILE);
    ...
    serveAppClient(socket) {
        ...
        socket.once("close", () => {
            ...
            if (client && this.clients.get(client.rendererRef) === client)
                this.disconnectAppClient(client);
        });
        socket.setEncoding("utf8");
        socket.on("data", (chunk) => {
            byteLength += Buffer.byteLength(chunk, "utf8");
            if (byteLength > exports.ACCOUNTS_BROKER_APP_SERVER_MAX_FRAME_BYTES * 2) {
                socket.destroy();
                return;
            }
            buffered += chunk;
            while (true) {
                ...
                if (Buffer.byteLength(line, "utf8") > exports.ACCOUNTS_BROKER_APP_SERVER_MAX_FRAME_BYTES) {
                    socket.destroy();
                    return;
                }
                let frame;
                ...
                if (!client) {
                    if (!isAppHandshake(frame)) {
                        socket.destroy();
                        return;
                    }
                    const result = this.broker.handshake(frame.handshake);
                    if (!result.ok) {
                        writeAppFrame(socket, { version: 1, kind: "handshake", ok: false }, exports.ACCOUNTS_BROKER_APP_SERVER_MAX_FRAME_BYTES);
                        socket.end();
                        return;
                    }
                    clearTimeout(handshakeTimer);
                    ...
                    if (!writeAppFrame(socket, { version: 1, kind: "handshake", ok: true }, exports.ACCOUNTS_BROKER_APP_SERVER_MAX_FRAME_BYTES))
                        socket.destroy();
                    continue;
                }
                if (!isAppMessage(frame)) {
                    socket.destroy();
                    return;
                }
    ...
    receiveDesktop(client, message, resumedAutomaticCapacityKey = null, nativeProvenOwner = null) {
        ...
        if (request.method === "initialize") {
            if (!(0, types_1.isPlainRecord)(request.params) || !(0, types_1.isPlainRecord)(request.params.clientInfo)
            ...
            for (const existing of [...this.children.values(), ...[...this.enrollmentHelpers.values()].map((helper) => helper.child)]) {
                if (existing.ready && existing.initializationClient && this.clients.get(existing.initializationClient.rendererRef) !== existing.initializationClient)
                    existing.initializationClient = client;
            }
            const child = this.broker.acquireChild(this.config.primaryOpaqueAccountId);
            if (!child) {
                this.sendDesktop(client, (0, redaction_1.redactedRouterError)(request.id, "pool_depleted"));
                return;
            }
        ...
        if (this.deferForNativePluginInventory(client, request))
            return;
        ...
        if (this.deferForModelCatalog(client, request))
            return;
        if (request.method === "account/rateLimits/read") {
            ...
            void Promise.allSettled(this.config.accounts.filter((account) => account.included).map((account) => this.refreshAutomaticCapacity(account.opaqueAccountId))).then(() => {
                if (!this.closed && this.clients.get(client.rendererRef) === client)
                    this.sendDesktop(client, { jsonrpc: "2.0", id: request.id, result: (0, pooled_quota_1.pooledNativeQuotaV1)(this.broker.pool().accounts, this.broker.quota()) });
        ...
        if (this.nativeHistory && this.routeNativeProjectRequest(client, request)) {
            if (resumedAutomaticCapacityKey)
                this.automaticCapacityRefreshRequests.delete(resumedAutomaticCapacityKey);
            return;
        }
            ...
            if (root) {
                ...
                if (durableOwner) {
                    if (!this.nativeHistoryWritersSafe()) {
                        if (resumedAutomaticCapacityKey)
                            this.automaticCapacityRefreshRequests.delete(resumedAutomaticCapacityKey);
                        this.sendDesktop(client, (0, redaction_1.redactedRouterError)(request.id, "unknown_thread_owner"));
                        return;
                    }
        ...
        if (!resumedAutomaticCapacityKey && this.deferForAutomaticCapacityRefresh(client, request))
            return;
        const requestedThreadId = (0, protocol_1.threadIdFrom)(request.params);
        ...
        if (modelEligible && !modelEligible.has(ownerAccount)) {
            if (resumedAutomaticCapacityKey)
                this.automaticCapacityRefreshRequests.delete(resumedAutomaticCapacityKey);
            this.sendDesktop(client, (0, redaction_1.redactedRouterError)(request.id, "capability_mismatch"));
            return;
        }
        ...
        if (this.nativeHistory && request.method === "thread/section/move" && (0, types_1.isPlainRecord)(request.params)
        ...
        if (this.remoteBlocksDesktop(account) && !isReadOnlyNativeAccountMethod(request.method)) {
            if (resumedAutomaticCapacityKey)
                this.automaticCapacityRefreshRequests.delete(resumedAutomaticCapacityKey);
            this.sendDesktop(client, (0, redaction_1.redactedRouterError)(request.id, "account_history_busy"));
            return;
        }
        ...
        if (this.nativeHistory && request.method === "thread/start")
            routedRequest = { ...routedRequest, params: { ...((0, types_1.isPlainRecord)(routedRequest.params) ? routedRequest.params : {}), historyMode: "paginated" } };
        ...
        if (this.nativeHistory && threadId && !isReadOnlyNativeAccountMethod(request.method)) {
            ...
            if (conflict !== "clear") {
                ...
                if (this.nativeTransfer?.hasPendingSourceRetirement(threadId))
                    void this.recoverNativeSourceRetirements().catch(() => { });
                if (resumedAutomaticCapacityKey)
                    this.automaticCapacityRefreshRequests.delete(resumedAutomaticCapacityKey);
                this.sendDesktop(client, (0, redaction_1.redactedRouterError)(request.id, conflict === "conflict" ? "account_history_busy" : "post_start_failure"));
                return;
            }
        }
        ...
        if (threadId && !taskRef) {
            if (resumedAutomaticCapacityKey)
                this.automaticCapacityRefreshRequests.delete(resumedAutomaticCapacityKey);
            this.sendDesktop(client, (0, redaction_1.redactedRouterError)(request.id, conversationId && this.canonicalHistory.hasActiveTurn(conversationId) ? "conversation_busy" : "unknown_thread_owner"));
            return;
        }
        ...
        if (taskRef && (unavailableContinuation || balanceContinuation)) {
            ...
            if (handoff) {
                ...
                if (request.method === "turn/start" && this.preferences.snapshot().failoverMode === "automatic"
                    && (0, quota_1.hasConfirmedQuotaDepletion)(this.broker.quota().find((quota) => quota.opaqueAccountId === account))
        ...
        if (this.deferUntilChildReady(client, request, child, () => this.receiveDesktop(client, request, resumedAutomaticCapacityKey)))
            return;
        let logicalTurnId = null;
        let balanceReservation = null;
        if (taskRef && request.method === "turn/start" && conversationId && threadId) {
            if (this.canonicalHistory.hasActiveTurn(conversationId)) {
                if (resumedAutomaticCapacityKey)
                    this.automaticCapacityRefreshRequests.delete(resumedAutomaticCapacityKey);
                this.sendDesktop(client, (0, redaction_1.redactedRouterError)(request.id, "conversation_busy"));
                return;
            }
            try {
        ...
        this.addPendingDesktop(childId, {
            ...
            ...(this.nativeHistory && request.method === "thread/section/move" && (0, types_1.isPlainRecord)(request.params)
                ? { nativeSectionMove: this.nativeSectionMoveMetadata(request.params) } : {}),
        });
        if (resumedAutomaticCapacityKey)
            this.automaticCapacityRefreshByChild.set(childId, resumedAutomaticCapacityKey);
        client.outstanding += 1;
        let balanceDispatched = false;
        let dispatchAttempted = false;
        let preDispatchConflict = "clear";
        try {
            ...
            if (this.nativeHistory && threadId && !isReadOnlyNativeAccountMethod(request.method))
                preDispatchConflict = this.nativeThreadConflict(threadId);
            if (this.nativeHistory && ((0, account_continuity_1.isAccountScopedCapabilityMutationV1)(request.method) || request.method === "mcpServer/oauth/login")) {
                if (!this.nativeAccountOperationSafe(account, child))
                    preDispatchConflict = "conflict";
            }
            if (!this.nativeHistoryWritersSafe() || preDispatchConflict !== "clear")
                throw new Error("native history writer conflict");
            ...
            if (taskRef && isContinuationRequestMethod(request.method))
                this.broker.finishRun(taskRef);
            this.sendDesktop(client, (0, redaction_1.redactedRouterError)(request.id, preDispatchConflict === "conflict" ? "account_history_busy" : dispatchAttempted ? "ambiguous_dispatch" : "post_start_failure"));
        }
    }
    /** Main/preload-only aggregate read; account and source home come from the signed owner binding. */
    readDesktopProjects(client, request) {
        if (!(0, types_1.isPlainRecord)(request.params) || Object.keys(request.params).length !== 0) {
            this.sendDesktop(client, { jsonrpc: "2.0", id: request.id, error: { code: -32602, message: "Desktop project read takes no parameters." } });
            return;
        }
        if (!this.nativeHistory || !this.nativeLegacyProjects || !(0, native_history_1.nativeHistoryBindingSafeV1)(this.nativeHistory)) {
        ...
        void (async () => {
            ...
            if (!binding || !helper || !(0, native_history_1.nativeHistoryBindingSafeV1)(binding))
                return null;
            const account = binding.source.metadataAccountId;
            const child = this.broker.acquireChild(account);
            if (!child?.ready || !(0, native_history_1.nativeHistoryBindingSafeV1)(binding))
                return null;
            const response = await this.requestBrokerChild(account, child, "project/list", { cursor: null, limit: 512 });
            if (this.closed || this.clients.get(client.rendererRef) !== client || this.nativeHistory !== binding
                || this.nativeLegacyProjects !== helper || this.children.get(account) !== child)
                return null;
            if (!response || response.error || !(0, types_1.isPlainRecord)(response.result)
                || (response.result.nextCursor !== undefined && response.result.nextCursor !== null)
                || !Array.isArray(response.result.data))
            ...
            for (const project of response.result.data) {
                if (!(0, types_1.isPlainRecord)(project) || !validNativeProjectId(project.id))
                    return null;
                ids.push(project.id);
            }
            if (new Set(ids).size !== ids.length || !(0, native_history_1.nativeHistoryBindingSafeV1)(binding))
                return null;
            ...
            if (!result)
                ...
                this.sendDesktop(client, { jsonrpc: "2.0", id: request.id, result });
        }).catch(() => {
            this.sendDesktop(client, { jsonrpc: "2.0", id: request.id, error: { code: -32603, message: "Desktop project projection is unavailable." } });
        }).finally(() => { client.outstanding = Math.max(0, client.outstanding - 1); });
    }
    /** Convert only a known logical alias back to its sealed native root. */
    nativeRootThreadId(request) {
        const requested = (0, protocol_1.threadIdFrom)(request.params);
        if (!requested)
            return null;
        const conversationId = this.canonicalHistory.conversationForPublicThreadId(requested);
        if (conversationId)
            return this.canonicalHistory.rootNativeThreadId(conversationId);
     ...
     */
    proveNativeOwnerAndRetry(client, request, rootThreadId, resumedAutomaticCapacityKey) {
        client.outstanding += 1;
    ...
    proveNativeOwner(rootThreadId) {
        const current = this.nativeOwnerProofs.get(rootThreadId);
        if (current)
            return current;
        const proof = this.probeNativeOwner(rootThreadId).finally(() => {
            if (this.nativeOwnerProofs.get(rootThreadId) === proof)
                this.nativeOwnerProofs.delete(rootThreadId);
        });
        this.nativeOwnerProofs.set(rootThreadId, proof);
        return proof;
    }
    async probeNativeOwner(rootThreadId) {
        if (!this.nativeHistory || !this.nativeHistoryWritersSafe())
            return { state: "unavailable" };
        const outcomes = await Promise.all(this.nativeHistory.accounts.map(async (source) => {
            let child = null;
            try {
                child = this.broker.acquireChild(source.opaqueAccountId);
            }
            catch {
                return { account: source.opaqueAccountId, state: "unavailable" };
            }
            if (!child)
                return { account: source.opaqueAccountId, state: "unavailable" };
            ...
            if (!response)
                return { account: source.opaqueAccountId, state: "unavailable" };
            if (response.error)
                return { account: source.opaqueAccountId, state: nativeThreadNotFound(response, rootThreadId) ? "absent" : "unavailable" };
            return { account: source.opaqueAccountId, state: nativeThreadReadMatches(response.result, rootThreadId) ? "matched" : "unavailable" };
        }));
    ...
    disconnectAppClient(client) {
        if (this.clients.get(client.rendererRef) !== client)
            return;
        ...
        for (const [desktopId, pending] of [...this.pendingChild]) {
            ...
            try {
                pending.child.send((0, redaction_1.redactedRouterError)(pending.childRequestId, "post_start_failure"));
            }
            catch { }
        }
        this.broker.disconnectRenderer(client.rendererRef);
    }
    settleUncertainPending(pending) {
        if (!pending.conversationId || !pending.taskRef)
            return;
    ...
    reserveBalancedTurn(account, nativeThreadId, params) {
        if (!this.isTokenBalancingEnabled())
            return null;
        if (this.balanceReservationByThread.has(nativeThreadId))
            throw new Error("balanced turn already has a reservation");
    ...
    releasePreDispatchBalance(reservation, nativeThreadId) {
        if (!reservation)
            return;
        try {
            this.tokenBalance.releasePreDispatch(reservation.reservationId);
        }
        catch {
            return;
        }
    ...
    seedProvenNewThreadBalance(account, nativeThreadId) {
        if (!this.isTokenBalancingEnabled())
            return;
        try {
            this.tokenBalance.seedThreadBaseline({
    ...
    bindBalanceTurn(reservationId, nativeThreadId, nativeTurnId) {
        if (!reservationId || !nativeThreadId)
            return;
        try {
    ...
    observeBalancedTokenUsage(account, nativeThreadId, nativeTurnId, tokenUsage) {
        const total = totalTokenUsage(tokenUsage);
        if (total !== null) {
            this.tokenUsageByThread.set(nativeThreadId, total);
            while (this.tokenUsageByThread.size > 256)
                this.tokenUsageByThread.delete(this.tokenUsageByThread.keys().next().value);
        }
        ...
        if (activeReservationId === reservationId)
            this.bindBalanceTurn(reservationId, nativeThreadId, nativeTurnId);
        try {
    ...
    settleBalancedTurn(account, nativeThreadId, nativeTurnId, tokenUsage) {
        if (!nativeThreadId)
            return;
        const activeReservationId = this.balanceReservationByThread.get(nativeThreadId) ?? null;
        ...
        try {
            ...
            if (this.balanceReservationByThread.get(nativeThreadId) === knownReservationId)
                this.balanceReservationByThread.delete(nativeThreadId);
    ...
    rememberSettledBalanceReservation(account, nativeThreadId, reservationId, turnId) {
    ...
    settledBalanceReservationFor(account, nativeThreadId, turnId) {
        const entries = this.settledBalanceReservationByThread.get(balanceThreadKey(account, nativeThreadId)) ?? [];
        if (turnId)
            return entries.find((entry) => entry.turnId === turnId)?.reservationId ?? null;
        return entries.length === 1 ? entries[0].reservationId : null;
    }
    addPendingDesktop(id, pending) {
    ...
    removePendingDesktop(id) {
        ...
        if (automaticKey)
            this.automaticCapacityRefreshRequests.delete(automaticKey);
        return pending;
    }
    expirePendingDesktop(id) {
        ...
        if (this.clients.get(pending.client.rendererRef) === pending.client) {
            this.sendDesktop(pending.client, (0, redaction_1.redactedRouterError)(pending.desktopId, "post_start_failure"));
        }
    }
    /**
     ...
     */
    routeHistoryRead(client, request, nativeProvenOwner = null) {
        if (!HISTORY_READ_METHODS.has(request.method))
            return false;
        if (this.nativeHistory && this.routeNativePointHistoryRead(client, request, nativeProvenOwner))
            return true;
        if (!this.nativeHistory && this.config.schemaVersion === 3)
            return this.routeCanonicalHistoryRead(client, request);
        ...
        if (request.method === "thread/list" && (0, types_1.isPlainRecord)(request.params) && typeof request.params.sectionId === "string") {
    ...
    routeNativePointHistoryRead(client, request, nativeProvenOwner) {
        if (!this.nativeHistory || !["thread/read", "thread/turns/list", "thread/items/list"].includes(request.method))
            return false;
        ...
        const child = this.broker.acquireChild(nativeProvenOwner);
        if (!child || !this.nativeHistoryWritersSafe()) {
            this.sendDesktop(client, (0, redaction_1.redactedRouterError)(request.id, "pool_depleted"));
            return true;
        }
        if (this.deferUntilChildReady(client, request, child, () => this.receiveDesktop(client, request)))
            return true;
        ...
        client.outstanding += 1;
        try {
            child.send({ ...withThreadId(request, rootThreadId), id: childId });
        }
        catch {
            this.removePendingDesktop(childId);
            client.outstanding = Math.max(0, client.outstanding - 1);
     ...
     */
    routeTransientNativePointHistoryRead(client, request, rootThreadId, account) {
        if (!this.nativeHistory || !this.nativeAccountBinding(account) || !this.nativeHistoryWritersSafe()) {
            this.sendDesktop(client, (0, redaction_1.redactedRouterError)(request.id, "unknown_thread_owner"));
            return;
        }
    ...
    routeMergedNativePointHistoryRead(client, request, rootThreadId, _conversationId, segments) {
        if (!this.nativeHistory || !this.nativeHistoryWritersSafe()) {
            this.sendDesktop(client, (0, redaction_1.redactedRouterError)(request.id, "unknown_thread_owner"));
            return;
        }
        ...
        void this.readNativeThreadSnapshots(segments).then((parts) => {
            ...
            const page = combinedNativeHistoryResult(request.method, cursor.params, rootThreadId, complete.map((part) => ({
                ...part.snapshot,
                handoffContextMarker: this.nativeHandoffContextMarker(part.opaqueAccountId, part.nativeThreadId),
            })), cursor.offset, exports.ACCOUNTS_BROKER_APP_SERVER_MAX_FRAME_BYTES - 4 * 1024);
            if (!page) {
                this.sendDesktop(client, (0, redaction_1.redactedRouterError)(request.id, "unknown_thread_owner"));
                return;
            }
            if (page.nextOffset !== null)
                page.result.nextCursor = this.createNativeMergedCursor(request.method, rootThreadId, cursor.queryFingerprint, page.nextOffset);
    ...
    async readNativeThreadSnapshots(segments) {
        if (!this.nativeHistory || segments.length < 1 || segments.length > 64 || !this.nativeHistoryWritersSafe())
            return null;
        const parts = await Promise.all(segments.map(async (segment) => {
            ...
            if (!this.nativeHistory || !this.nativeAccountBinding(segment.opaqueAccountId))
                return null;
            const child = this.broker.acquireChild(segment.opaqueAccountId);
            if (!child)
                return null;
    ...
    routeNativeProjectRequest(client, request) {
        if (!this.nativeHistory || !request.method.startsWith("project/"))
            return false;
        const account = this.nativeHistory.source.metadataAccountId;
        if (!this.nativeHistoryWritersSafe()) {
            this.sendDesktop(client, (0, redaction_1.redactedRouterError)(request.id, "post_start_failure"));
    ...
    ensureNativeProjectAndRetry(client, request, sourceProjectId, targetAccount, resumedAutomaticCapacityKey) {
    ...
    async nativeContinuationContext(sourceAccount, sourceThreadId, conversationId) {
        if (!this.nativeHistory || !this.nativeAccountBinding(sourceAccount) || !this.nativeHistoryWritersSafe())
            return null;
        const segments = this.canonicalHistory.orderedNativeSegments(conversationId);
        if (!segments || !segments.some((segment) => segment.opaqueAccountId === sourceAccount && segment.nativeThreadId === sourceThreadId))
            return null;
        ...
        for (const part of snapshots) {
            const visibleTurns = part.snapshot.turns.map((turn) => nativeVisibleHistoryTurn(turn, this.nativeHandoffContextMarker(part.opaqueAccountId, part.nativeThreadId)));
            const parsed = (0, native_history_1.nativeHistoryThreadReadContextV1)({ thread: { ...part.snapshot.thread, turns: visibleTurns } }, part.nativeThreadId);
            ...
            for (const turn of parsed.turns) {
                ...
                if (itemCount > 256 || inputBytes > 96 * 1024)
                    return null;
            }
        ...
        return (0, native_history_1.renderNativeHistoryContextV1)(turns);
    }
    /** Exact secret-derived marker identifies only broker-written handoff context. */
    nativeHandoffContextMarker(account, nativeThreadId) {
        return `account-router-handoff-context-v1:${(0, node_crypto_1.createHmac)("sha256", this.secret)
            .update(`native-handoff-context:v1\0${account}\0${nativeThreadId}`, "utf8")
            .digest("base64url")}`;
    }
    routeCanonicalHistoryRead(client, request) {
        ...
        if (request.method !== "thread/read" || !(0, types_1.isPlainRecord)(request.params) || Object.keys(request.params).some((key) => !["includeTurns", "threadId"].includes(key))
            || typeof request.params.threadId !== "string") {
            this.sendDesktop(client, (0, redaction_1.redactedRouterError)(request.id, "unknown_thread_owner"));
            return true;
        }
    ...
    dispatchNextHistoryFanout(fanout) {
        if (this.closed || this.pendingHistory.get(fanout.key) !== fanout)
            return;
        ...
        if (this.nativeHistory && request.method === "thread/list" && (0, types_1.isPlainRecord)(request.params)
        ...
        if (!child.ready || this.childFeatureRevision.get(child) !== this.featureRevision) {
            void this.initializeChild(child).then((ready) => {
                if (this.pendingHistory.get(fanout.key) !== fanout)
                    return;
                if (!ready) {
                    this.failHistoryFanout(fanout.key, "post_start_failure");
                    return;
     ...
     */
    dispatchNativeLegacyProjectRows(fanout, account, request, projectId) {
        const helper = this.nativeLegacyProjects;
        ...
        if (state.legacyOffset < ids.length) {
            ...
            void Promise.all(selected.map(async (threadId) => {
                ...
                if (this.pendingHistory.get(fanout.key) !== fanout)
                    return;
                const accepted = rows.filter((row) => row !== null);
    ...
    resolveHistoryFanout(account, response) {
        ...
        if (response.error)
            ...
            const cacheKey = historyCacheKey(fanout.request, account);
            if (cacheKey)
                this.cacheHistoryResponse(cacheKey, publicResponse.result);
        }
    ...
    completeHistoryFanout(fanout) {
        ...
        if (this.nativeHistory && fanout.partial && !partialNativeRead) {
            this.sendDesktop(fanout.client, (0, redaction_1.redactedRouterError)(fanout.desktopId, "unknown_thread_owner"));
            return;
        }
        const result = mergeHistoryResponses(fanout.request.method, fanout.responses, (threadId, account) => partialNativeRead ? this.bindNativeReadHint(account, threadId) : this.bindHistoryOwnership(account, threadId), (threadId) => this.store.snapshot().threadOwners[threadId] ?? null, (account, localId) => this.nativeHistory ? this.nativeHistorySectionId(account, localId) : this.historySectionId(account, localId), this.nativeHistory !== null, this.nativeHistory ? (account, threadId) => this.nativeHistoryRowVisible(account, threadId) : undefined);
        if (result === null) {
            this.sendDesktop(fanout.client, (0, redaction_1.redactedRouterError)(fanout.desktopId, "unknown_thread_owner"));
            return;
        }
        if (fanout.request.method === "thread/list" && (0, types_1.isPlainRecord)(fanout.request.params)
            ...
            result.data = result.data.map((row, index) => ({ row, index, position: (0, types_1.isPlainRecord)(row) && typeof row.id === "string" ? position.get(row.id) : undefined }))
        ...
        for (const { account, response } of fanout.responses) {
            ...
            if (cursor)
                nextCursors.set(account, cursor);
        }
    ...
    failHistoryFanout(key, code) {
        const fanout = this.pendingHistory.get(key);
        if (!fanout)
            return;
        this.pendingHistory.delete(key);
        clearTimeout(fanout.timer);
        if (fanout.currentChildId)
            this.pendingHistoryByChild.delete(fanout.currentChildId);
        fanout.client.outstanding = Math.max(0, fanout.client.outstanding - 1);
        this.sendDesktop(fanout.client, (0, redaction_1.redactedRouterError)(fanout.desktopId, code));
    }
    historySectionId(account, localId) {
        const id = `section_${(0, node_crypto_1.createHmac)("sha256", this.secret).update(`history-section:v1:${account}:${localId}`, "utf8").digest("base64url")}`;
        this.historySections.set(id, { account, localId, expiresAt: Date.now() + HISTORY_SECTION_BINDING_TTL_MS });
        while (this.historySections.size > 512)
            this.historySections.delete(this.historySections.keys().next().value);
        return id;
    }
    /** Native section ids remain stable; availability is retained for each contributing account. */
    nativeHistorySectionId(account, localId) {
        ...
        while (this.nativeSectionsByAccount.size > 4_096)
            this.nativeSectionsByAccount.delete(this.nativeSectionsByAccount.keys().next().value);
        return localId;
    }
    /** A canonical continuation segment is one logical native root, not a second task row. */
    nativeHistoryRowVisible(account, nativeThreadId) {
        const nativeOwner = this.nativeTransfer?.ownerForThread(nativeThreadId);
        if (nativeOwner && this.nativeTransfer.isCommittedProjection(nativeThreadId, account) && nativeOwner !== account)
            return false;
    ...
    bindNativeReadHint(account, threadId) {
        const existing = this.nativeReadHints.get(threadId);
        if (existing && existing.expiresAt > Date.now() && existing.opaqueAccountId !== account)
            return false;
        this.nativeReadHints.set(threadId, { opaqueAccountId: account, expiresAt: Date.now() + NATIVE_READ_HINT_TTL_MS });
        while (this.nativeReadHints.size > NATIVE_READ_HINT_MAX_ENTRIES)
            this.nativeReadHints.delete(this.nativeReadHints.keys().next().value);
        return true;
    }
    nativeReadHint(threadId) {
    ...
    withNativePublicProjectIds(account, response) {
        if (!this.nativeProjects || !(0, types_1.isPlainRecord)(response.result))
            return response;
    ...
    withNativeHistoryProjectIds(account, response) {
        const linked = this.withNativePublicProjectIds(account, response);
        const legacy = this.nativeLegacyProjects;
        if (!this.nativeHistory || !legacy || !this.nativeLegacyProjectsReady || account !== this.nativeHistory.source.metadataAccountId || !(0, types_1.isPlainRecord)(linked.result))
            return linked;
    ...
    refreshNativeLegacyProjectsFromResult(result) {
        if (!this.nativeLegacyProjects || !(0, types_1.isPlainRecord)(result) || (result.nextCursor !== undefined && result.nextCursor !== null) || !Array.isArray(result.data))
            return false;
        const ids = [];
        ...
        if (new Set(ids).size !== ids.length || !this.nativeHistoryWritersSafe())
            return false;
        const ready = this.nativeLegacyProjects.refresh(ids);
        this.nativeLegacyProjectsReady = ready;
        return ready;
    }
    withNativePublicProjectIdsNotification(account, notification) {
        if (!this.nativeProjects || !(0, protocol_1.isNotification)(notification) || !(0, types_1.isPlainRecord)(notification.params))
            return notification;
    ...
    resolveHistoryCursorInput(request) {
        ...
        if (base.publicCursor === null)
            return { providerCursors: null, queryFingerprint };
        const state = this.historyCursors.get(base.publicCursor);
        if (!state || state.expiresAt <= Date.now()) {
            this.historyCursors.delete(base.publicCursor);
            return null;
        }
        if (state.method !== request.method || state.queryFingerprint !== queryFingerprint)
            return null;
        return { providerCursors: state.providerCursors, queryFingerprint };
    }
    createHistoryCursor(method, queryFingerprint, providerCursors) {
        const id = `hc_${(0, node_crypto_1.createHmac)("sha256", this.secret).update(`history-cursor:v1:${++this.nonce}:${Date.now()}:${method}:${queryFingerprint}`, "utf8").digest("base64url")}`;
        ...
        while (this.historyCursors.size > HISTORY_CURSOR_MAX_ENTRIES)
            this.historyCursors.delete(this.historyCursors.keys().next().value);
        return id;
    }
    /** Resolve a cursor that is meaningful only while this owner lives. */
    resolveNativeMergedCursorInput(request, rootThreadId) {
        const base = nativeMergedBaseParams(request.params);
        if (!base)
            return null;
        const queryFingerprint = historyQueryFingerprint(this.secret, request.method, base.params);
        if (base.publicCursor === null)
            return { params: base.params, queryFingerprint, offset: 0 };
        ...
        if (cursor.method !== request.method || cursor.rootThreadId !== rootThreadId || cursor.queryFingerprint !== queryFingerprint)
            return null;
        return { params: base.params, queryFingerprint, offset: cursor.offset };
    }
    createNativeMergedCursor(method, rootThreadId, queryFingerprint, offset) {
        if (!Number.isSafeInteger(offset) || offset < 0)
            throw new Error("invalid native merged cursor offset");
        const id = `nmc_${(0, node_crypto_1.createHmac)("sha256", this.secret).update(`native-merged-cursor:v1:${++this.nonce}:${Date.now()}:${method}:${rootThreadId}:${queryFingerprint}:${offset}`, "utf8").digest("base64url")}`;
        ...
        while (this.nativeMergedCursors.size > HISTORY_CURSOR_MAX_ENTRIES)
            this.nativeMergedCursors.delete(this.nativeMergedCursors.keys().next().value);
        return id;
    }
    resolveNativeLegacyProjectListCursor(cursor, projectId) {
        if (cursor === null)
            return { expiresAt: Date.now() + HISTORY_CURSOR_TTL_MS, projectId, legacyOffset: 0, nativeStarted: false, nativeCursor: null };
    ...
    createNativeLegacyProjectListCursor(state) {
        ...
        const id = `nlp_${(0, node_crypto_1.createHmac)("sha256", this.secret).update(`native-legacy-project-list:v1:${++this.nonce}:${Date.now()}:${state.projectId}:${state.legacyOffset}:${state.nativeStarted}:${state.nativeCursor ?? ""}`, "utf8").digest("base64url")}`;
        this.nativeLegacyProjectListCursors.set(id, { ...state, expiresAt: Date.now() + HISTORY_CURSOR_TTL_MS });
        while (this.nativeLegacyProjectListCursors.size > HISTORY_CURSOR_MAX_ENTRIES)
            this.nativeLegacyProjectListCursors.delete(this.nativeLegacyProjectListCursors.keys().next().value);
        return id;
    }
    accountForRequest(message) {
        const threadId = (0, protocol_1.threadIdFrom)(message.params);
        if (threadId)
            return this.store.snapshot().threadOwners[threadId] ?? null;
        if (this.nativeHistory && OWNERLESS_NATIVE_THREAD_READ_METHODS.has(message.method))
        ...
        if (this.nativeHistory && /^(?:thread\/|turn\/)/.test(message.method) && message.method !== "thread/start")
            return null;
        if (message.method === "thread/start") {
            ...
            if (this.config.mode === "manual")
                return this.config.primaryOpaqueAccountId;
            if (this.isTokenBalancingEnabled()) {
                if (this.balanceConfiguredAccounts().length !== 2)
                    return null;
            ...
            candidates.sort((left, right) => compareNewWorkCapacity(quotas.get(left.opaqueAccountId), left.assignedTaskCount, left.opaqueAccountId, quotas.get(right.opaqueAccountId), right.assignedTaskCount, right.opaqueAccountId, this.config.accounts.findIndex((account) => account.opaqueAccountId === left.opaqueAccountId), this.config.accounts.findIndex((account) => account.opaqueAccountId === right.opaqueAccountId)));
            return candidates[0]?.opaqueAccountId ?? null;
        }
        return this.config.primaryOpaqueAccountId;
    }
    accountNeedsContinuationHandoff(account) {
    ...
    selectContinuationTarget(exclude) {
        if (this.isTokenBalancingEnabled()) {
     ...
     */
    shouldProposeBalanceHandoff(account, request, conversationId) {
        if (!this.isTokenBalancingEnabled() || request.method !== "turn/start" || !conversationId
            ...
            return false;
        const configured = this.balanceConfiguredAccounts();
        if (configured.length !== 2 || !configured.includes(account))
            return false;
        const other = configured.find((candidate) => candidate !== account) ?? null;
        if (!other || !this.eligibleAutomaticAccounts(account).includes(other))
            return false;
    ...
    bindHistoryOwnership(account, threadId) {
        const current = this.store.snapshot().threadOwners[threadId];
        if (current)
            return current === account;
        try {
            this.store.update((state) => {
                if (state.threadOwners[threadId] && state.threadOwners[threadId] !== account)
                    throw new Error("history owner collision");
                if (!state.threadOwners[threadId]) {
                    state.threadOwners[threadId] = account;
                    const ledger = state.ledger[account];
                    if (!ledger)
                        throw new Error("history ledger account unavailable");
    ...
    ensureTask(client, account, threadId) {
        const current = this.store.snapshot().threadOwners[threadId];
        ...
        if (registered)
            this.currentConversationByRenderer.set(client.rendererRef, conversationId);
        return registered ? taskRef : null;
    }
    /** Map a stable origin native thread to the currently active immutable segment. */
    logicalRoute(account, requestedThreadId) {
    ...
    nativeOutboundThreadId(conversationId) {
    ...
    publishConversation(conversationId) {
        ...
        if (projection)
            this.broker.publishLogicalConversation(projection);
    }
    publishCommittedTurn(conversationId, turnId) {
        ...
        if (projection)
            this.broker.publishLogicalTurn(projection);
    }
    readCurrentLogicalHistory(rendererRef) {
     ...
     */
    publishPeerCommittedTranscript(conversationId, turnId, originRendererRef) {
        ...
        for (const [rendererRef, boundConversationId] of this.currentConversationByRenderer) {
            if (rendererRef === originRendererRef || boundConversationId !== conversationId)
                continue;
    ...
    projectConversation(conversationId) {
        ...
        if (currentProjection.availability !== "complete")
    ...
    withCurrentAvailability(conversation) {
    ...
    taskRef(threadId) {
        ...
        if (cached)
            return cached;
        const taskRef = `bt_${(0, node_crypto_1.createHmac)("sha256", this.secret).update(`broker-task:v1:${threadId}`, "utf8").digest("base64url")}`;
    ...
    nativeAccountBinding(account) {
        return this.nativeHistory?.accounts.find((entry) => entry.opaqueAccountId === account) ?? null;
    }
    nativeHistoryWritersSafe() {
        return !this.nativeHistory || (0, native_history_1.nativeHistoryBindingSafeV1)(this.nativeHistory);
    }
    sharedNativeMode() {
        return this.nativeHistory
            ? (0, shared_native_mode_1.readSharedNativeModeV1)({ stateRoot: this.stateRoot, binding: this.nativeHistory, secret: this.secret })
            : null;
    }
    nativeAccountOperationSafe(account, child) {
        if (!this.nativeHistory)
            return true;
        if (this.children.get(account) !== child || typeof child.pid !== "number")
            return false;
        return (0, native_history_1.observeNativeAccountOperationWritersV1)(this.nativeHistory, account, child.pid).ok;
    }
    nativeThreadConflict(threadId) {
        if (!this.nativeHistory)
            return "clear";
        if (this.nativeTransfer?.hasPendingSourceRetirement(threadId))
            return "conflict";
        return (0, native_history_1.observeNativeThreadWriterV1)(this.nativeHistory, threadId, this.nativeOwnedPids()).state;
    }
    /** Synchronous account lease: no callback can interleave capture, preparation and spawn. */
    prepareAccountContinuity(account, codexHome, captureOnly = false) {
        const mode = this.sharedNativeMode();
        if (mode?.state === "blocked")
            return false;
        if (mode?.state === "ready") {
            if (this.nativeAccountBinding(account)?.codexHome !== codexHome)
                return false;
            this.nativeContinuityDeferred.delete(account);
            this.nativeContinuityReasons.delete(account);
            return true;
        }
        const sharedPath = (0, node_path_1.join)(this.stateRoot, "shared-account-config", "base.v1.json");
        if (!(0, node_fs_1.existsSync)(sharedPath))
            return true;
        try {
            let shared = (0, account_continuity_1.loadSharedAccountBase)(this.stateRoot);
            let plugins = (0, account_continuity_1.loadSharedPluginsManifestV1)(this.stateRoot);
            if (!shared || !plugins || this.children.has(account))
                return false;
            ...
            if (this.nativeHistory) {
                ...
                const provenance = (0, account_continuity_1.loadAccountContinuitySharedSourceProvenanceV1)(this.stateRoot);
                if (!source || source.codexHome !== codexHome || !(0, native_history_1.nativeHistoryBindingSafeV1)(this.nativeHistory)
            ...
            const writeEvidence = {
                ...
                ...(this.nativeHistory ? { nativeWriterCensus: () => {
                        const observation = (0, native_history_1.observeNativeAccountWritersV1)(this.nativeHistory, account, []);
            ...
            const input = () => ({ stateRoot: this.stateRoot, account: continuityAccount, shared: shared,
                schema: account_continuity_1.DEFAULT_ACCOUNT_CONTINUITY_SCHEMA_V1, plugins: plugins, writeEvidence });
            if (!captureOnly && (0, account_continuity_1.ensureAccountContinuityEnrollment)({ ...input(), apply: true }).state === "blocked")
                return false;
            let capturedUnmaterialized = false;
            if (this.nativeHistory) {
                const source = this.nativeAccountBinding(account);
                if (!source || source.codexHome !== codexHome)
                    return false;
                const observed = (0, account_continuity_1.observeExistingNativeAccountContinuity)({ ...input(), apply: true,
                    nativeHomeIdentity: source.codexHomeIdentity,
                    nativeBindingPreflight: () => this.nativeHistory !== null && (0, native_history_1.nativeHistoryBindingSafeV1)(this.nativeHistory) });
                const writers = (0, native_history_1.observeNativeAccountWritersV1)(this.nativeHistory, account, []);
                if (!writers.ok) {
                    ...
                    if (captureOnly || writers.foreignPids.length === 0 || observed.state === "blocked")
                        return false;
                ...
                if (!(0, node_fs_1.existsSync)((0, node_path_1.join)(this.stateRoot, "accounts", account, "config-materialization.v1.json"))) {
                    const captured = (0, account_continuity_1.captureUnmaterializedNativeChangesBeforeSpawn)({ ...input(), apply: true,
                        nativeHomeIdentity: source.codexHomeIdentity,
                        nativeBindingPreflight: () => this.nativeHistory !== null && (0, native_history_1.nativeHistoryBindingSafeV1)(this.nativeHistory) });
                    if (captured.state === "blocked")
                        return false;
                    capturedUnmaterialized = true;
                }
            }
            const preview = captureOnly ? null : (0, account_continuity_1.prepareAccountConfigBeforeSpawn)(input());
            if (!capturedUnmaterialized && (captureOnly || preview?.state === "blocked" || account === sharedSourceAccount)) {
                const captured = (0, account_continuity_1.captureIdleAccountChangesBeforeSpawn)({ ...input(), primary: account === sharedSourceAccount, apply: true });
                if (captured.state === "blocked")
                    return false;
                if (captured.proposedSharedConfig && captured.proposedSharedCapabilities) {
                    const published = (0, account_continuity_1.publishPrimarySharedBaseAfterExit)({ stateRoot: this.stateRoot, prior: shared,
                        proposedConfig: captured.proposedSharedConfig, proposedCapabilities: captured.proposedSharedCapabilities,
                        schema: account_continuity_1.DEFAULT_ACCOUNT_CONTINUITY_SCHEMA_V1, apply: true });
                    if (published.state !== "published" || !published.shared)
                        return false;
                    shared = published.shared;
                }
            }
            if (!capturedUnmaterialized && account === sharedSourceAccount) {
                plugins = (0, account_continuity_1.publishPrimaryPluginInventoryAfterExit)({ stateRoot: this.stateRoot, account: continuityAccount, prior: plugins, writeEvidence });
                if (!plugins)
                    return false;
            }
            if (captureOnly)
                return true;
            const prepared = (0, account_continuity_1.prepareAccountConfigBeforeSpawn)({ ...input(), apply: true }).state === "ready";
            if (prepared) {
                this.nativeContinuityDeferred.delete(account);
                this.nativeContinuityReasons.delete(account);
            }
    ...
    async initializeChild(child) {
        ...
        try {
            ...
            if (this.childFeatureReplay.get(child) === replay)
                this.childFeatureReplay.delete(child);
        }
    }
    async initializeAndReplayChild(child) {
        if (this.closed)
            return false;
        for (const [client, params] of this.initializedDesktopClients) {
            ...
            if (!child.ready && (!await child.initialize(params, client) || !child.ready || this.closed))
                return false;
            ...
            if (targetRevision !== this.featureRevision)
                return this.initializeAndReplayChild(child);
            this.childFeatureRevision.set(child, targetRevision);
            return true;
        }
        return false;
    }
    nativeSectionMoveRequest(request, account) {
        if (!(0, types_1.isPlainRecord)(request.params) || typeof request.params.threadId !== "string")
            return null;
        const params = { ...request.params };
        const home = this.nativeSectionHomes.get(request.params.threadId);
        if (!home || home.expiresAt <= Date.now() || home.account !== account)
            return null;
        if (typeof params.sectionId === "string") {
            if (!this.nativeSectionAvailable(account, params.sectionId))
                return null;
        }
        ...
        if (typeof params.beforeThreadId === "string") {
            ...
            if (beforeOwner !== account) {
                ...
                params.beforeThreadId = start < 0 ? null : order.slice(start + 1).find((threadId) => (this.store.snapshot().threadOwners[threadId] ?? this.nativeReadHint(threadId)) === account) ?? null;
            }
        }
        else if (params.beforeThreadId !== undefined && params.beforeThreadId !== null)
            return null;
        return { ...request, params };
    }
    captureNativeSectionHomes(account, result) {
        if (!this.nativeHistory || !(0, types_1.isPlainRecord)(result) || !Array.isArray(result.data))
            return;
        for (const row of result.data) {
            const thread = (0, types_1.isPlainRecord)(row) && (0, types_1.isPlainRecord)(row.thread) ? row.thread : row;
            if (!(0, types_1.isPlainRecord)(thread) || typeof thread.id !== "string")
                continue;
            const currentLocalId = (0, types_1.isPlainRecord)(thread.section) && typeof thread.section.id === "string" ? thread.section.id : null;
            this.nativeSectionHomes.set(thread.id, { account, currentLocalId, expiresAt: Date.now() + HISTORY_SECTION_BINDING_TTL_MS });
            while (this.nativeSectionHomes.size > 10_000)
                this.nativeSectionHomes.delete(this.nativeSectionHomes.keys().next().value);
        }
    }
    nativeSectionAvailabilityKey(account, localId) {
        return `${account}\0${localId}`;
    }
    nativeSectionAvailable(account, localId) {
    ...
    nativeSectionOrderKey(localId) {
        return `nso_${(0, node_crypto_1.createHmac)("sha256", this.secret).update(`native-section-order:v1\0${localId}`, "utf8").digest("base64url")}`;
    }
    nativeSectionMoveMetadata(params) {
        ...
        if (params.sectionId !== null && (typeof params.sectionId !== "string" || !this.nativeSectionAvailable(home.account, params.sectionId)))
            return undefined;
    ...
    commitNativeSectionMove(move) {
        this.store.update((state) => {
            ...
            if (before < 0)
                order.push(move.threadId);
            else
                order.splice(before, 0, move.threadId);
            orders[move.sectionKey] = order;
        });
    }
    async warmEnabledChildren() {
        ...
        for (let offset = 0; offset < accounts.length; offset += 4) {
            await Promise.allSettled(accounts.slice(offset, offset + 4).map(async (account) => {
                const child = this.broker.acquireChild(account);
                if (child)
                    await this.initializeChild(child);
            }));
        }
    }
    broadcastFeatureEnablement(client, request) {
        if (!(0, types_1.isPlainRecord)(request.params) || typeof request.params.feature !== "string" || request.params.feature.length === 0
        ...
        const operation = this.featureBroadcastTail.then(async () => {
            ...
            for (const { account, child } of responses) {
                if (this.children.get(account) === child)
                    this.childFeatureRevision.set(child, this.featureRevision);
            }
    ...
    deferForModelCatalog(client, request) {
        if (this.modelCheckedRequests.has(request) || !["thread/start", "turn/start", "thread/settings/update"].includes(request.method))
            return false;
        const model = (0, models_1.requestedModelFromParamsV1)(request.params);
        if (!model) {
    ...
    async readAccountModels(account) {
        const child = this.broker.acquireChild(account);
        if (!child || !await this.initializeChild(child))
        ...
        for (let page = 0; page < 64; page += 1) {
            ...
            if (bytes > 2 * 1024 * 1024 || data.length + response.result.data.length > 10_000)
                throw new Error("model catalog unavailable");
            data.push(...response.result.data);
            const next = response.result.nextCursor;
            if (next === null || next === undefined)
                return { data, nextCursor: null };
            if (typeof next !== "string" || next.length === 0 || next.length > 4_096 || seen.has(next))
                throw new Error("model catalog unavailable");
            seen.add(next);
            cursor = next;
        }
    ...
    deferUntilChildReady(client, request, child, resume, always = false) {
        if (child.ready && this.childFeatureRevision.get(child) === this.featureRevision && !always)
            return false;
        const key = automaticCapacityRequestKey(client, request);
    ...
    isolatedAuthHome(account) {
        if (!this.nativeHistory)
            return null;
        const home = (0, native_history_1.nativeHistoryEffectiveAuthHomeV1)(this.nativeHistory, account);
        if (!home)
            throw new Error("native authentication binding unavailable");
        return home === this.nativeAccountBinding(account)?.codexHome ? null : home;
    }
    pendingAuthRefreshRequests = new Map();
    authHelperTails = new Map();
    authHelperQueued = new Map();
    activeAuthHelpers = new Map();
    authRefreshes = new Map();
    async withAuthHelper(account, operation) {
        const home = this.isolatedAuthHome(account);
        const initialization = this.desktopInitialization;
        if (!home || !initialization || this.closed)
            return null;
        const queued = this.authHelperQueued.get(account) ?? 0;
        if (queued >= 8 || [...this.enrollmentHelpers.values()].some((helper) => helper.opaqueAccountId === account && helper.isolatedReconnect))
            throw new NativeAuthHelperBusyError();
        const previous = this.authHelperTails.get(account) ?? Promise.resolve();
        let release;
        const complete = new Promise((resolvePromise) => { release = resolvePromise; });
        ...
        this.authHelperTails.set(account, tail);
        this.authHelperQueued.set(account, queued + 1);
        let queueTimer;
        try {
            const admitted = await Promise.race([previous.then(() => true), new Promise((resolvePromise) => {
                    queueTimer = setTimeout(() => resolvePromise(false), 30_000);
                    queueTimer.unref();
                })]);
            if (queueTimer)
            ...
            if (!admitted)
                throw new NativeAuthHelperBusyError();
            if (this.closed || this.isolatedAuthHome(account) !== home)
                return null;
            const helper = new BrokerProcessChild(account, (0, node_child_process_1.spawn)(this.command, credentialStoreArgs(this.args, "file"), {
                cwd: home, env: brokerChildEnvironment(home, home), stdio: ["pipe", "pipe", "ignore"],
            }), () => { }, () => { });
            this.activeAuthHelpers.set(account, helper);
            const timer = setTimeout(() => { void helper.terminateAndWait(); }, 25_000);
            timer.unref();
            try {
                if (!await helper.initialize(initialization.params, initialization.client) || this.isolatedAuthHome(account) !== home || this.closed)
                    return null;
                ...
                if (this.activeAuthHelpers.get(account) === helper)
                    this.activeAuthHelpers.delete(account);
            }
        }
        finally {
            if (queueTimer)
                clearTimeout(queueTimer);
            release();
            const remaining = (this.authHelperQueued.get(account) ?? 1) - 1;
            if (remaining)
                this.authHelperQueued.set(account, remaining);
            else
                this.authHelperQueued.delete(account);
            void tail.then(() => { if (this.authHelperTails.get(account) === tail)
                this.authHelperTails.delete(account); });
        }
    }
    refreshIsolatedAuth(account) {
        const existing = this.authRefreshes.get(account);
        if (existing)
            return existing;
        const pending = this.withAuthHelper(account, async (helper, home) => {
            ...
            if (!response || response.error || !providerAuthenticated(response.result) || this.isolatedAuthHome(account) !== home)
                return null;
            return (0, native_auth_binding_1.readNativeExternalTokensV1)(home, entry, this.secret);
        }).catch((error) => { if (error instanceof NativeAuthHelperBusyError)
            throw error; return null; });
        this.authRefreshes.set(account, pending);
        const cleanup = () => { if (this.authRefreshes.get(account) === pending)
            this.authRefreshes.delete(account); };
        void pending.then(cleanup, cleanup);
        return pending;
    }
    createChild(account) {
        if (this.nativeTransferHeldAccounts.has(account))
            throw new Error("native account writer handoff in progress");
        const existing = this.children.get(account);
        if (existing)
            return existing;
        ...
        if (!nativeSource && !(0, app_server_mux_1.preflightRouterHomes)(this.config, this.stateRoot)) {
        ...
        const codexHome = nativeSource?.codexHome ?? (0, node_path_1.join)(accountRoot, "codex-home");
        const sqliteHome = nativeSource?.sqliteHome ?? (0, node_path_1.join)(accountRoot, "sqlite-home");
        ...
        const childArgs = nativeSource ? authHome ? credentialStoreArgs(this.args, "ephemeral") : this.args : (0, app_server_mux_1.sharedPluginChildArgs)(this.stateRoot, this.args);
        if (!childArgs)
            throw new Error("accounts broker refused child with invalid shared plugin enablement");
        const mode = this.sharedNativeMode();
        if (mode?.state === "blocked")
            throw new Error("accounts broker refused unresolved shared native mode");
        if (mode?.state === "ready") {
            const binary = (0, node_fs_1.lstatSync)(this.command);
            if (!binary.isFile() || binary.isSymbolicLink()
                || (0, node_crypto_1.createHash)("sha256").update((0, node_fs_1.readFileSync)(this.command)).digest("hex") !== mode.document.resolverBinarySha256) {
        ...
        const child = new BrokerProcessChild(account, childProcess, (message) => this.receiveChild(account, child, message), (expectedTermination) => {
            ...
            if (!expectedTermination)
                this.broker.markChildUnavailable(account);
        }, authHome ? async (candidate) => {
            if (this.isolatedAuthHome(account) !== authHome || !nativeSource)
                return false;
            ...
            if (!tokens || this.isolatedAuthHome(account) !== authHome)
                return false;
     ...
     */
    failPendingForChild(account, child) {
    ...
    receiveChild(account, child, message) {
        ...
        if (this.captureEnrollmentNotification(account, message))
            return;
        this.routeChildNotification(account, message);
    }
    captureEnrollmentNotification(account, message) {
        if (!(0, protocol_1.isNotification)(message) || message.method !== "account/login/completed" || !(0, types_1.isPlainRecord)(message.params)
            || typeof message.params.loginId !== "string" || typeof message.params.success !== "boolean")
            return false;
        for (const helper of this.enrollmentHelpers.values()) {
            const helperAccount = helper.transportAccountId;
            if (helperAccount !== account || helper.loginId !== message.params.loginId)
                continue;
            if (helper.isolatedReconnect) {
                if (message.params.success)
                    void this.completeIsolatedReconnect(helper);
                else {
                    helper.completion = { success: false };
                    clearTimeout(helper.isolatedReconnect.timer);
                    helper.isolatedReconnect.preimage.fill(0);
                    void helper.child.terminateAndWait();
                }
                ...
                helper.completion = { success: message.params.success };
            return true;
        }
        return false;
    }
    async completeIsolatedReconnect(helper) {
        const repair = helper.isolatedReconnect;
        const account = helper.opaqueAccountId;
        if (!repair || !account || !helper.root)
            return;
        try {
            const entry = this.nativeAccountBinding(account);
            ...
            const current = (0, native_auth_binding_1.readNativeAuthPrivateFileV1)((0, node_path_1.join)(repair.authHome, "auth.json"));
            try {
                if (!current.equals(repair.preimage))
                    throw new Error("unavailable");
                const target = (0, node_path_1.join)(repair.authHome, "auth.json");
                const before = (0, node_fs_1.lstatSync)(target);
                const staged = (0, node_path_1.join)(repair.authHome, `.auth-reconnect-${(0, node_crypto_1.randomBytes)(16).toString("hex")}`);
                (0, node_fs_1.writeFileSync)(staged, fresh, { flag: "wx", mode: 0o600 });
                const priorEvidence = (0, node_path_1.join)(repair.authHome, `.auth-before-reconnect-${(0, node_crypto_1.randomBytes)(16).toString("hex")}`);
                (0, node_fs_1.writeFileSync)(priorEvidence, current, { flag: "wx", mode: 0o600 });
                for (const path of [staged, priorEvidence]) {
                ...
                (0, node_fs_1.renameSync)(staged, target);
                const fd = (0, node_fs_1.openSync)(repair.authHome, node_fs_1.constants.O_RDONLY);
                try {
                    (0, node_fs_1.fsyncSync)(fd);
                }
                finally {
            ...
            repair.preimage.fill(0);
            await helper.child.terminateAndWait();
        }
    }
    resolveDesktopResponse(account, _child, response) {
        if (typeof response.id !== "string")
            return;
        const pending = this.pendingDesktop.get(response.id);
        if (!pending || pending.account !== account || pending.acknowledged)
            return;
        ...
        if (!response.error)
            void this.refreshSharedNativeRuntimes(pending.method);
        ...
        if (!response.error && pending.nativeSectionMove) {
            try {
                this.commitNativeSectionMove(pending.nativeSectionMove);
            }
            catch {
                this.sendDesktop(pending.client, (0, redaction_1.redactedRouterError)(pending.desktopId, "post_start_failure"));
                return;
        ...
        if (["thread/start", "thread/fork", "thread/resume", "thread/unarchive"].includes(pending.method) && threadId) {
            if (pending.method === "thread/start")
                this.seedProvenNewThreadBalance(account, threadId);
    ...
    settleTerminalTurn(account, taskRef) {
        if (!taskRef)
            return false;
        for (const [id, pending] of this.pendingDesktop) {
            if (pending.account !== account || pending.taskRef !== taskRef || pending.method !== "turn/start")
                continue;
            this.removePendingDesktop(id);
            pending.client.outstanding = Math.max(0, pending.client.outstanding - 1);
            return true;
    ...
    routeChildRequest(account, child, request) {
        if (request.method === "account/chatgptAuthTokens/refresh") {
            const requests = this.pendingAuthRefreshRequests.get(child) ?? new Set();
            ...
            this.pendingAuthRefreshRequests.set(child, requests);
            void (async () => {
                try {
                    if (this.children.get(account) !== child || !this.isolatedAuthHome(account))
                        throw new Error("unavailable");
                    if (!(0, types_1.isPlainRecord)(request.params) || request.params.reason !== "unauthorized"
                        || Object.keys(request.params).some((key) => key !== "reason" && key !== "previousAccountId"))
                        throw new Error("unavailable");
                    const previous = request.params.previousAccountId;
                    if (previous !== null && previous !== undefined) {
                        const entry = this.nativeAccountBinding(account);
                        if (typeof previous !== "string" || !entry || (0, native_history_1.nativeHistoryAuthIdentityHmacV1)(previous, this.secret) !== entry.authIdentityHmac)
                            throw new Error("unavailable");
                    }
                    const tokens = await this.refreshIsolatedAuth(account);
                    if (!tokens || this.children.get(account) !== child)
                        throw new Error("unavailable");
                    child.send({ jsonrpc: "2.0", id: request.id, result: tokens });
                }
                catch (error) {
                    if (!(error instanceof NativeAuthHelperBusyError))
                        this.broker.markChildUnavailable(account);
                    try {
                        child.send((0, redaction_1.redactedRouterError)(request.id, "post_start_failure"));
                    }
                    catch { }
                }
                finally {
                    ...
                    if (!requests.size)
                        this.pendingAuthRefreshRequests.delete(child);
                }
            })();
            return;
        }
        const threadId = (0, protocol_1.threadIdFrom)(request.params);
        if (!threadId) {
            ...
            try {
                child.send((0, redaction_1.redactedRouterError)(request.id, "unknown_thread_owner"));
            }
            catch { }
            return;
        }
    ...
    resolveChildRequest(client, response) {
        if (typeof response.id !== "string")
            return;
        ...
        try {
            pending.child.send({ ...response, id: pending.childRequestId });
        }
        catch { }
    }
    routeChildNotification(account, notification) {
        if ((0, protocol_1.isNotification)(notification) && notification.method === "account/rateLimits/updated") {
            const quota = providerQuotaProjection(notification.params);
            if (quota)
                this.broker.updateQuota({ opaqueAccountId: account, ...quota });
            const params = (0, pooled_quota_1.pooledNativeQuotaV1)(this.broker.pool().accounts, this.broker.quota());
            for (const client of this.clients.values())
                this.sendDesktop(client, { jsonrpc: "2.0", method: notification.method, params });
            return;
        }
        if (this.nativeHistory && (0, protocol_1.isNotification)(notification) && notification.method === "project/changed") {
            ...
            for (const client of this.clients.values())
                this.sendDesktop(client, publicNotification);
            return;
        }
        if (this.nativeHistory && this.nativeLegacyProjects && (0, protocol_1.isNotification)(notification) && notification.method === "thread/project/updated"
        ...
        const threadId = (0, protocol_1.threadIdFrom)((0, protocol_1.isNotification)(notification) ? notification.params : undefined);
        if (!threadId || this.store.snapshot().threadOwners[threadId] !== account)
            return;
        const taskRef = this.taskRefsByThread.get(threadId);
        const route = taskRef ? this.broker.routeAppTools(taskRef) : null;
        const client = route ? this.clients.get(route.ownerRendererRef) ?? null : null;
        ...
        let canonicalCompletedTurn = null;
        if ((0, protocol_1.isNotification)(notification) && notification.method === "turn/completed" && (0, types_1.isPlainRecord)(notification.params)) {
            const turn = (0, types_1.isPlainRecord)(notification.params.turn) ? notification.params.turn : null;
            ...
            if (conversationId && client) {
                ...
                if (committed) {
                    ...
                    if (!this.nativeHistory)
                        this.publishPeerCommittedTranscript(conversationId, committed, client.rendererRef);
                    const publicThreadId = this.canonicalHistory.publicThreadId(conversationId);
                    ...
                    if (canonicalTurn)
                        canonicalCompletedTurn = canonicalTurnProjection(canonicalTurn, this.secret);
                }
                else
                    this.canonicalHistory.markIncomplete(conversationId, account, threadId);
                this.publishConversation(conversationId);
        ...
        if ((0, protocol_1.isNotification)(notification) && (notification.method === "turn/completed" || notification.method === "thread/closed")) {
            const settled = this.settleTerminalTurn(account, taskRef ?? null);
            if (notification.method === "thread/closed")
                this.settleBalancedTurn(account, threadId, null, null);
        ...
        const wireThreadId = conversationId
            ? this.nativeHistory ? this.nativeOutboundThreadId(conversationId) : this.canonicalHistory.publicThreadId(conversationId)
            : null;
        let desktopNotification = this.nativeHistory
            ? wireThreadId ? withNativeNotificationThreadId(notification, wireThreadId) : notification
            : canonicalCompletedTurn && wireThreadId
                ? { jsonrpc: "2.0", method: "turn/completed", params: { threadId: wireThreadId, turn: canonicalCompletedTurn } }
                : wireThreadId ? withNotificationThreadId(notification, wireThreadId) : notification;
        if (this.nativeHistory)
            desktopNotification = this.withNativePublicProjectIdsNotification(account, desktopNotification);
        this.sendDesktop(client, desktopNotification);
    }
    async forwardNativeContinuation(delivery) {
        const transfer = this.nativeTransfer;
        const threadId = this.threadByTaskRef.get(delivery.taskRef);
        const held = this.heldDesktopContinuations.get(delivery.handoffRef);
        if (!transfer || !threadId || !held || !isJsonRpcMessage(delivery.continuation) || !(0, protocol_1.isRequest)(delivery.continuation)
            ...
            || this.store.snapshot().threadOwners[threadId] !== delivery.fromOpaqueAccountId)
            return "rejected";
        const input = portableTurnStartInput(delivery.continuation.params);
        if (input.kind !== "portable")
            return "linked_continuation_required";
        if (!hasFreshPositiveQuota(this.broker.quota().find((quota) => quota.opaqueAccountId === delivery.toOpaqueAccountId)))
            return "rejected";
        const targetSchemaChild = this.broker.acquireChild(delivery.toOpaqueAccountId);
        if (!targetSchemaChild || !await this.initializeChild(targetSchemaChild))
            return "rejected";
        ...
        let preparedOperationId = null;
        try {
            const targetBeforePrepare = await this.loadedNativeThreads(delivery.toOpaqueAccountId);
            if (targetBeforePrepare === null || targetBeforePrepare.length !== 0)
                return "rejected";
            if (!await this.quiesceIdleAccount(delivery.toOpaqueAccountId, delivery.handoffRef))
                return "rejected";
            if (!await transfer.sourceProjectionReady(threadId, delivery.fromOpaqueAccountId))
                return "rejected";
            const sourceBeforePrepare = await this.loadedNativeThreads(delivery.fromOpaqueAccountId);
            if (sourceBeforePrepare === null || sourceBeforePrepare.some((loadedThreadId) => loadedThreadId !== threadId))
                return "rejected";
            if (!await this.quiesceIdleAccount(delivery.fromOpaqueAccountId, delivery.handoffRef))
                return "rejected";
            ...
            if (prepared.state !== "ready") {
                if (prepared.state === "busy")
                    held.rejectionCode = "account_history_busy";
                return "rejected";
            }
            ...
            const target = this.broker.acquireChild(delivery.toOpaqueAccountId);
            this.nativeTransferHeldAccounts.add(delivery.toOpaqueAccountId);
            if (!target || !await this.initializeChild(target))
                return "rejected";
            const targetBeforeResume = await this.loadedNativeThreads(delivery.toOpaqueAccountId);
            if (targetBeforeResume === null || targetBeforeResume.length !== 0)
                return "rejected";
            if (!await transfer.revalidatePrepared(prepared.operationId))
                return "rejected";
            ...
            if (!warmup || warmup.error?.code !== -32600 || warmup.error.message !== `thread ${threadId} already has an active writer`)
                return "ambiguous";
            ...
            if (this.children.get(delivery.toOpaqueAccountId) !== target || !target.ready
                || targetAfterWarmup === null || targetAfterWarmup.length !== 0)
                return "ambiguous";
            if (!await transfer.confirmPreparationProbeBlocked(prepared.operationId, warmup))
                return "ambiguous";
            if (!await transfer.revalidatePrepared(prepared.operationId))
                return "ambiguous";
            transfer.markResumeDispatching(prepared.operationId);
            ...
            if (settlement.state !== "proved") {
                if (resumed?.error?.code === -32600 && resumed.error.message === `thread ${threadId} already has an active writer`)
                    held.rejectionCode = "account_history_busy";
            ...
            try {
                this.store.update((state) => {
                    if (state.threadOwners[threadId] !== delivery.fromOpaqueAccountId)
                    ...
                    if (!receipt || receipt.state !== "pending")
                        throw new Error("native handoff receipt changed");
                ...
                if (!this.broker.transferNativeTask(delivery.taskRef, delivery.fromOpaqueAccountId, delivery.toOpaqueAccountId))
                    throw new Error("native task binding changed");
                ...
                try {
                    ...
                    if (retirement.state !== "retired")
                        throw new Error("native source retirement remains held");
                }
                finally {
                    this.nativeRetirementHandoffs.delete(delivery.fromOpaqueAccountId);
                }
                this.syncBrokerAssignedTaskCounts();
                const logicalTurnId = this.canonicalHistory.beginTurn(delivery.conversationId, delivery.toOpaqueAccountId, threadId, held.client.rendererRef, this.clientLabel(held.client), delivery.continuation.params);
                this.canonicalHistory.markTurnDispatching(delivery.conversationId, delivery.toOpaqueAccountId, threadId);
                if (!this.broker.beginRun(delivery.taskRef))
                ...
                if (held.automaticCapacityRefreshKey)
                    this.automaticCapacityRefreshByChild.set(childId, held.automaticCapacityRefreshKey);
                if (!this.nativeHistoryWritersSafe() || transfer.preflightSharedWriterLocks().state !== "ready")
                    throw new Error("native transfer binding changed");
                if (!hasFreshPositiveQuota(this.broker.quota().find((quota) => quota.opaqueAccountId === delivery.toOpaqueAccountId)))
                    throw new Error("native target quota changed");
                dispatched = true;
                target.send({ ...delivery.continuation, id: childId, params: { ...((0, types_1.isPlainRecord)(delivery.continuation.params) ? delivery.continuation.params : {}), threadId } });
                this.store.update((state) => { if (state.pendingHandoffs)
                    delete state.pendingHandoffs[delivery.handoffRef]; });
                ...
                try {
                    if (dispatched)
                        this.canonicalHistory.markAmbiguous(delivery.conversationId, delivery.toOpaqueAccountId, threadId);
                    else
                        this.canonicalHistory.markIncomplete(delivery.conversationId, delivery.toOpaqueAccountId, threadId);
                }
                catch { }
                if (held.forwardChildId) {
                    this.removePendingDesktop(held.forwardChildId);
                    this.automaticCapacityRefreshByChild.delete(held.forwardChildId);
                }
            ...
            if (preparedOperationId)
                transfer.releasePreparationLease(preparedOperationId);
            this.nativeTransferHeldAccounts.delete(delivery.fromOpaqueAccountId);
            this.nativeTransferHeldAccounts.delete(delivery.toOpaqueAccountId);
        }
    }
    async forwardContinuation(delivery) {
        if (this.nativeHistory)
            return this.forwardNativeContinuation(delivery);
        const threadId = this.threadByTaskRef.get(delivery.taskRef);
        const held = this.heldDesktopContinuations.get(delivery.handoffRef);
        if (!threadId || !held || !isJsonRpcMessage(delivery.continuation) || !(0, protocol_1.isRequest)(delivery.continuation)
            || delivery.continuation.method !== "turn/start")
            return "rejected";
        const input = portableTurnStartInput(delivery.continuation.params);
        ...
        if (input.kind === "nonportable" || !context || context.digest !== `sha256:${(0, node_crypto_1.createHash)("sha256").update(context.text, "utf8").digest("hex")}`) {
        ...
        if (!hasFreshPositiveQuota(targetQuota))
            return "rejected";
        if (this.nativeHistory && !this.nativeHistoryWritersSafe())
            return "rejected";
        const child = this.broker.acquireChild(delivery.toOpaqueAccountId);
        if (!child || !await this.initializeChild(child))
            return "rejected";
        ...
        try {
            this.store.update((state) => {
                if (state.threadOwners[threadId] !== delivery.fromOpaqueAccountId)
                    throw new Error("handoff owner changed");
                const handoffs = state.pendingHandoffs ?? (state.pendingHandoffs = {});
                const existing = handoffs[delivery.handoffRef];
                if (!existing || existing.state !== "pending")
                    throw new Error("handoff receipt unavailable");
        ...
        if (started.error)
            return "rejected";
        const targetThreadId = (0, protocol_1.threadIdFrom)(started.result);
        if (!targetThreadId)
            return "ambiguous";
        if (this.nativeHistory && !this.nativeHistoryWritersSafe()) {
        ...
        let targetTaskRef = null;
        let targetBalanceReservation = null;
        ...
        try {
            ...
            const turnRequest = { jsonrpc: "2.0", id: childId, method: "turn/start", params: {
            ...
            if (!targetTaskRef)
                throw new Error("target task binding rejected");
            targetBalanceReservation = this.reserveBalancedTurn(delivery.toOpaqueAccountId, targetThreadId, turnRequest.params);
            const logicalTurnId = this.canonicalHistory.beginTurn(delivery.conversationId, delivery.toOpaqueAccountId, targetThreadId, held.client.rendererRef, this.clientLabel(held.client), { input: input.input });
            this.canonicalHistory.markTurnDispatching(delivery.conversationId, delivery.toOpaqueAccountId, targetThreadId);
            this.publishConversation(delivery.conversationId);
            if (!this.broker.beginRun(targetTaskRef))
                throw new Error("target run rejected");
            this.addPendingDesktop(childId, {
                client: held.client,
            ...
            try {
                ...
                this.removePendingDesktop(childId);
                if (!targetBalanceDispatched)
                    this.releasePreDispatchBalance(targetBalanceReservation, targetThreadId);
                return "ambiguous";
            }
            this.store.update((state) => {
                const handoffs = state.pendingHandoffs ?? {};
                const receipt = handoffs[delivery.handoffRef];
                if (!receipt || receipt.state !== "forwarding")
                    throw new Error("handoff forwarding receipt changed");
            ...
            if (targetTaskRef)
                this.broker.finishRun(targetTaskRef);
            if (!targetBalanceDispatched)
                this.releasePreDispatchBalance(targetBalanceReservation, targetThreadId);
            try {
                this.canonicalHistory.markAmbiguous(delivery.conversationId, delivery.toOpaqueAccountId, targetThreadId);
    ...
    sendDesktop(client, message) {
        if (!client.socket.writable)
            return;
        if (!writeAppFrame(client.socket, { version: 1, kind: "message", message }, exports.ACCOUNTS_BROKER_APP_SERVER_MAX_FRAME_BYTES))
            client.socket.destroy();
    }
}
exports.AccountsBrokerOwnerV1 = AccountsBrokerOwnerV1;
class BrokerProcessChild {
    opaqueAccountId;
    child;
    onMessage;
    onClose;
    ...
    initializationFingerprint = null;
    initializedResult;
    initializedReady = false;
    initializationClient = null;
    protocolReady = false;
    privateRequests = new Map();
    initializeId = `abi:${(0, node_crypto_1.randomBytes)(16).toString("hex")}`;
    constructor(opaqueAccountId, child, onMessage, onClose, authenticate) {
        ...
        if (!child.stdin || !child.stdout)
            throw new Error("accounts broker child lacks JSONL stdio");
        this.closedPromise = new Promise((resolvePromise) => { this.resolveClosed = resolvePromise; });
        this.lines = (0, node_readline_1.createInterface)({ input: child.stdout, crlfDelay: Infinity });
        ...
        this.lines.on("line", (line) => {
            ...
            if (message && (0, protocol_1.isResponse)(message) && typeof message.id === "string" && this.privateRequests.has(message.id)) {
                const pending = this.privateRequests.get(message.id);
                this.privateRequests.delete(message.id);
                clearTimeout(pending.timer);
                pending.resolve(message);
                return;
            }
            if (message && (0, protocol_1.isResponse)(message) && message.id === this.initializeId) {
                if (!this.settleInitialization)
                    return;
                if (message.error) {
                    this.finishInitialization(false);
                ...
                if (this.authenticate)
                    void this.authenticate(this).then((ready) => {
                        if (!ready || this.closed || this.transportFailed || this.expectedTermination) {
                            this.finishInitialization(false);
                            this.terminate("capacity");
                        }
                        else
                ...
                if (!this.authenticate || this.ready || (0, protocol_1.isRequest)(message))
        ...
        const closed = () => {
            if (this.closed)
                return;
            this.closed = true;
            for (const pending of this.privateRequests.values()) {
                clearTimeout(pending.timer);
                pending.resolve(null);
            }
    ...
    get ready() { return this.initializedReady && !this.closed && !this.transportFailed && !this.expectedTermination; }
    get initializeResult() { return this.initializedResult; }
    initialize(params, client) {
        if (this.closed || this.transportFailed || this.expectedTermination)
            return Promise.resolve(false);
        const fingerprint = JSON.stringify(params);
        if (this.initializationFingerprint !== null && this.initializationFingerprint !== fingerprint)
            return Promise.resolve(false);
        // Readiness is current transport state, not the historical handshake result.
        if (this.initialization)
            return this.initialization.then((ready) => ready && this.ready);
        this.initializationFingerprint = fingerprint;
        this.initializationClient = client;
        this.initialization = new Promise((resolvePromise) => { this.settleInitialization = resolvePromise; });
        ...
        try {
            this.child.stdin.write(`${JSON.stringify({ jsonrpc: "2.0", id: this.initializeId, method: "initialize", params })}\n`);
        }
        catch {
            this.finishInitialization(false);
            this.terminate("capacity");
    ...
    requestPrivate(method, params, timeoutMs = 10_000) {
        if (!this.protocolReady || this.closed || this.transportFailed || this.expectedTermination || this.privateRequests.size >= 8)
            return Promise.resolve(null);
        const id = `auth:${(0, node_crypto_1.randomBytes)(16).toString("hex")}`;
        return new Promise((resolvePromise) => {
            const timer = setTimeout(() => { this.privateRequests.delete(id); resolvePromise(null); }, timeoutMs);
            timer.unref();
            this.privateRequests.set(id, { resolve: resolvePromise, timer });
            try {
    ...
    finishInitialization(ready) {
        ...
        if (settle) {
            this.initializedReady = ready;
            settle(ready);
        }
    }
    send(message) {
        if (this.closed || this.transportFailed || this.expectedTermination || !this.child.stdin || this.child.exitCode !== null || this.child.signalCode !== null)
            throw new Error("accounts broker child is unavailable");
        if (!(0, protocol_1.isResponse)(message) && !this.ready)
            throw new Error("accounts broker child is not initialized");
    ...
    async terminateAndWait() {
        ...
        if (this.closed)
            return true;
        const force = setTimeout(() => { if (!this.closed)
            this.child.kill("SIGKILL"); }, 1_000);
        const result = await Promise.race([this.closedPromise.then(() => true), new Promise((resolvePromise) => {
                const timer = setTimeout(() => resolvePromise(false), 3_000);
                this.closedPromise.then(() => clearTimeout(timer));
            })]);
        clearTimeout(force);
        return result;
    }
...
exports.BrokerProcessChild = BrokerProcessChild;
/** Owner command environment: account-local homes plus a forced disable bit. */
function brokerChildEnvironment(codexHome, sqliteHome, source = process.env) {
...
async function runAccountsBrokerOwnerCli(argv = process.argv.slice(2)) {
    ...
    const owner = new AccountsBrokerOwnerV1(selection.config, parsed.stateRoot, secret, parsed.command, parsed.args, { onStartupStage: createBrokerStartupDiagnostics(parsed.stateRoot) });
    try {
        await owner.start();
    }
    catch {
...
function createBrokerStartupDiagnostics(root) {
    const identity = (0, node_fs_1.lstatSync)(root);
    if (!identity.isDirectory() || identity.isSymbolicLink() || identity.uid !== process.getuid?.()
        || (identity.mode & 0o077) !== 0)
        throw new Error("invalid broker diagnostics root");
    const file = `broker-startup-${process.pid}-${(0, node_crypto_1.randomBytes)(8).toString("hex")}.json`;
    const events = [];
    return (event) => {
        try {
            if (events.length >= 16 || !["election", "probe", "recovery", "connect"].includes(event.stage)
                ...
                || !Number.isSafeInteger(event.elapsedMs) || event.elapsedMs < 0 || event.elapsedMs > 600_000)
                return;
            const current = (0, node_fs_1.lstatSync)(root);
            if (!current.isDirectory() || current.isSymbolicLink() || current.dev !== identity.dev || current.ino !== identity.ino
                || current.uid !== identity.uid || (current.mode & 0o077) !== 0)
                return;
            events.push({ stage: event.stage, code: event.code, elapsedMs: event.elapsedMs });
            (0, state_store_1.writePrivateJsonAtomic)(root, file, { version: 1, pid: process.pid, events });
        }
...
function installBrokerTerminationDiagnostics(root) {
    const identity = (0, node_fs_1.lstatSync)(root);
    if (!identity.isDirectory() || identity.isSymbolicLink() || identity.uid !== process.getuid?.()
        || (identity.mode & 0o077) !== 0)
        throw new Error("invalid broker diagnostics root");
    const path = (0, node_path_1.join)(root, `broker-termination-${process.pid}-${(0, node_crypto_1.randomBytes)(8).toString("hex")}.json`);
    const record = (kind, exitCode, error) => {
        try {
            const current = (0, node_fs_1.lstatSync)(root);
            if (!current.isDirectory() || current.isSymbolicLink() || current.dev !== identity.dev
                || current.ino !== identity.ino || current.uid !== identity.uid || (current.mode & 0o077) !== 0)
                return;
            ...
            for (const frame of (error?.stack ?? "").slice(0, 32_768).split("\n").slice(1, 65)) {
                ...
                if (match && modules.has(match[1]))
                    locations.push({ module: match[1], line: Number(match[2]), column: Number(match[3]) });
            ...
            (0, node_fs_1.writeFileSync)(path, JSON.stringify({ version: 1, observedAt: new Date().toISOString(), pid: process.pid,
...
async function connectAccountsBrokerAppServerClient(options, onMessage) {
    const path = (0, broker_socket_1.accountsBrokerSocketPath)(options.root, exports.ACCOUNTS_BROKER_APP_SERVER_SOCKET_FILE);
    return new Promise((resolvePromise, reject) => {
        ...
        let settled = false;
        let resolveClosed;
        const whenClosed = new Promise((resolvePromise) => { resolveClosed = resolvePromise; });
        ...
        socket.once("close", () => {
            ...
            if (!settled)
                settle(undefined, new Error("accounts broker app server unavailable"));
        });
        socket.setEncoding("utf8");
        socket.on("data", (chunk) => {
            bytes += Buffer.byteLength(chunk, "utf8");
            if (bytes > exports.ACCOUNTS_BROKER_APP_SERVER_MAX_FRAME_BYTES * 2) {
                socket.destroy();
                return;
            }
            buffered += chunk;
            while (true) {
                ...
                if (isAppHandshakeResult(frame)) {
                    ...
                    clearTimeout(timeout);
                    settle({
                        send: (message) => writeAppFrame(socket, { version: 1, kind: "message", message }, exports.ACCOUNTS_BROKER_APP_SERVER_MAX_FRAME_BYTES),
        ...
        socket.once("connect", () => {
            ...
            const handshake = { ...unsigned, proof: (0, broker_1.createBrokerHandshakeProof)(options.secret, unsigned) };
            if (!writeAppFrame(socket, { version: 1, kind: "handshake", handshake }, exports.ACCOUNTS_BROKER_APP_SERVER_MAX_FRAME_BYTES))
                socket.destroy();
        });
    });
}
function createBrokerAppRendererRef(secret) {
    return `br_${(0, node_crypto_1.createHmac)("sha256", secret).update(`app-server-renderer:v1:${(0, node_crypto_1.randomBytes)(24).toString("base64url")}`, "utf8").digest("base64url")}`;
}
function createBrokerAppToolsRef(secret) {
    return `bat_${(0, node_crypto_1.createHmac)("sha256", secret).update(`app-server-tools:v1:${(0, node_crypto_1.randomBytes)(24).toString("base64url")}`, "utf8").digest("base64url")}`;
}
function persistentHandoff(handoff, state) {
    return {
        version: 1,
 ...
 */
function mergeHistoryResponses(method, responses, bindOwner, ownerForThread, bindSection, rejectAmbiguousOwners = false, includeThread = undefined) {
    if (!HISTORY_READ_METHODS.has(method) || responses.length === 0)
        return null;
    const first = responses[0].response.result;
    if (!(0, types_1.isPlainRecord)(first))
        return null;
    const sectionRows = [];
    const candidates = new Map();
    for (const { account, response } of responses) {
        if (response.error || !(0, types_1.isPlainRecord)(response.result) || !Array.isArray(response.result.data))
            return null;
        if (response.result.data.length > 512)
            return null;
        for (const entry of response.result.data) {
            if (method === "threadSection/list") {
                if (!(0, types_1.isPlainRecord)(entry) || typeof entry.id !== "string" || entry.id.length < 1 || entry.id.length > 512)
                    return null;
            ...
            if (!threadId)
                return null;
            if (includeThread && !includeThread(account, threadId))
                continue;
    ...
    if (method === "threadSection/list") {
        if (sectionRows.length > 512)
            return null;
        sectionRows.sort((left, right) => historyEntrySort(method, left, "", right, ""));
        const result = { ...first, data: sectionRows, nextCursor: null };
        if (Object.prototype.hasOwnProperty.call(first, "backwardsCursor"))
    ...
    for (const [threadId, values] of candidates) {
        ...
        if (rejectAmbiguousOwners && values.length !== 1)
            return null;
    ...
    const result = { ...first, data: rows.map((row) => row.entry), nextCursor: null };
    if (Object.prototype.hasOwnProperty.call(first, "backwardsCursor"))
        result.backwardsCursor = null;
    return result;
}
...
function historyEntrySort(method, left, leftId, right, rightId) {
    if (method === "thread/search") {
        const score = (value) => (0, types_1.isPlainRecord)(value) && typeof value.score === "number" && Number.isFinite(value.score) ? value.score : null;
    ...
    const timestamp = (value) => {
        const record = (0, types_1.isPlainRecord)(value) && method === "thread/search" && (0, types_1.isPlainRecord)(value.thread) ? value.thread : value;
        if (!(0, types_1.isPlainRecord)(record))
            return 0;
...
function historyBaseParams(value) {
    ...
    if (!(0, types_1.isPlainRecord)(value))
        return { params: value, publicCursor: null };
...
function nativeMergedBaseParams(value) {
    ...
    if (typeof cursor !== "string" || !/^nmc_[A-Za-z0-9_-]{16,128}$/.test(cursor))
        return null;
    const { cursor: _cursor, ...params } = value;
    return { params, publicCursor: cursor };
}
function nativeThreadReadOmitsTurns(value) {
    return (0, types_1.isPlainRecord)(value) && value.includeTurns === false;
}
function historyRequestForAccount(request, providerCursor) {
    const base = historyBaseParams(request.params);
    if (!base)
        return null;
    if (providerCursor === null)
        return base.publicCursor === null ? { ...request, params: base.params } : null;
    if (providerCursor.length < 1 || providerCursor.length > 512 || /[\u0000-\u001f\u007f]/.test(providerCursor))
        return null;
    const params = (0, types_1.isPlainRecord)(base.params) ? { ...base.params, cursor: providerCursor } : { cursor: providerCursor };
    return { ...request, params };
}
function historyQueryFingerprint(secret, method, params) {
    let serialized;
    try {
        serialized = JSON.stringify(params) ?? "null";
    }
    catch {
        serialized = "invalid";
    }
    if (serialized.length > 32 * 1024)
        serialized = serialized.slice(0, 32 * 1024);
    return (0, node_crypto_1.createHmac)("sha256", secret).update(`history-query:v1:${method}:`, "utf8").update(serialized, "utf8").digest("base64url");
}
function providerHistoryCursor(result) {
    if (!(0, types_1.isPlainRecord)(result) || typeof result.nextCursor !== "string")
        return null;
    return result.nextCursor.length > 0 && result.nextCursor.length <= 512 && !/[\u0000-\u001f\u007f]/.test(result.nextCursor)
        ? result.nextCursor : null;
}
function historyThreadId(method, value) {
    if (method === "thread/loaded/list")
        return safeHistoryIdentifier(value);
    if (!(0, types_1.isPlainRecord)(value))
        return null;
    if (method === "thread/search")
        return (0, types_1.isPlainRecord)(value.thread) ? safeHistoryIdentifier(value.thread.id) : null;
    return safeHistoryIdentifier(value.id) ?? safeHistoryIdentifier((0, protocol_1.threadIdFrom)(value));
}
function safeHistoryIdentifier(value) {
 ...
 */
function historyCacheKey(request, account) {
    if (request.method !== "thread/list" || !emptyHistoryParams(request.params))
        return null;
    return `${account}\u0000thread/list`;
}
function emptyHistoryParams(value) {
    return value === undefined || ((0, types_1.isPlainRecord)(value) && Object.keys(value).length === 0);
}
function withThreadId(request, nativeThreadId) {
    if (!(0, types_1.isPlainRecord)(request.params))
        return request;
    if (typeof request.params.threadId === "string")
        return { ...request, params: { ...request.params, threadId: nativeThreadId } };
    if (typeof request.params.thread_id === "string")
        return { ...request, params: { ...request.params, thread_id: nativeThreadId } };
    return request;
}
function withProjectId(request, nativeProjectId) {
    if (!(0, types_1.isPlainRecord)(request.params))
        return request;
    return { ...request, params: { ...request.params, projectId: nativeProjectId } };
}
function nativeThreadStartProjectId(params) {
    return nativeProjectReference(params);
}
function nativeThreadListProjectId(params) {
    return nativeProjectReference(params);
}
function nativeLegacyProjectListInput(params, expectedProjectId) {
    if (!(0, types_1.isPlainRecord)(params) || params.projectId !== expectedProjectId)
        return null;
    const cursor = params.cursor;
    if (cursor !== undefined && cursor !== null && (typeof cursor !== "string" || !/^nlp_[A-Za-z0-9_-]{16,128}$/.test(cursor)))
        return null;
    const rawLimit = params.limit;
    if (rawLimit !== undefined && rawLimit !== null && (typeof rawLimit !== "number" || !Number.isSafeInteger(rawLimit) || rawLimit < 1 || rawLimit > 256))
        return null;
...
function nativeProjectReference(params) {
    if (!(0, types_1.isPlainRecord)(params) || !Object.prototype.hasOwnProperty.call(params, "projectId") || params.projectId === null || params.projectId === undefined) {
        return { state: "absent" };
    }
    return validNativeProjectId(params.projectId) ? { state: "value", projectId: params.projectId } : { state: "invalid" };
}
function validNativeProjectId(value) {
    return typeof value === "string" && value.length > 0 && value.length <= 256 && !/[\u0000-\u001f\u007f]/.test(value);
}
function nativeProjectWriteMethod(method) {
    return method !== "project/list" && method !== "project/read";
}
function nativeThreadReadMatches(value, expectedThreadId) {
    return (0, types_1.isPlainRecord)(value) && (0, types_1.isPlainRecord)(value.thread) && value.thread.id === expectedThreadId;
}
/** Exact metadata read for an old assignment; native non-null project wins. */
function nativeLegacyThreadListRow(value, expectedThreadId, publicProjectId, visible) {
    if (!(0, types_1.isPlainRecord)(value) || !(0, types_1.isPlainRecord)(value.thread) || value.thread.id !== expectedThreadId || value.thread.projectId !== null || !visible(expectedThreadId))
        return null;
    return { ...value.thread, projectId: publicProjectId };
}
function nativeNonNullProjectListRow(value, expectedProjectId, visible) {
    return (0, types_1.isPlainRecord)(value) && typeof value.id === "string" && value.projectId === expectedProjectId && visible(value.id);
}
function nativeThreadNotFound(response, expectedThreadId) {
    ...
    if (error.code === -32600 && error.message === `no rollout found for thread id ${expectedThreadId}`)
        return true;
    const dataCode = (0, types_1.isPlainRecord)(error.data) && typeof error.data.code === "string" ? error.data.code : null;
...
function rewriteResponseProjectIds(value, publicId) {
    return rewriteProjectEnvelope(value, publicId, 0);
}
function rewriteProjectEnvelope(value, publicId, depth) {
    if (depth > 4)
        return value;
    let changed = false;
    const output = { ...value };
    if (typeof value.projectId === "string") {
        ...
        if (mapped && mapped !== value.projectId) {
            output.projectId = mapped;
            changed = true;
        }
    }
    if ((0, types_1.isPlainRecord)(value.thread)) {
    ...
    if ((0, types_1.isPlainRecord)(value.project)) {
        const project = { ...value.project };
        if (typeof project.id === "string") {
            const mapped = publicId(project.id);
            if (mapped && mapped !== project.id) {
                project.id = mapped;
                changed = true;
            }
        }
    ...
    if (Array.isArray(value.data) && value.data.length <= 512) {
        ...
        const data = sourceData.map((entry) => (0, types_1.isPlainRecord)(entry) ? rewriteProjectEnvelope(entry, publicId, depth + 1) : entry);
 ...
 */
function rewriteLegacyProjectEnvelope(value, publicProjectForThread, depth = 0) {
    if (depth > 4)
        return value;
    let changed = false;
    let output = value;
    ...
    if (Array.isArray(value.data) && value.data.length <= 16_384) {
        const sourceData = value.data;
        const data = sourceData.map((entry) => (0, types_1.isPlainRecord)(entry) ? rewriteLegacyProjectEnvelope(entry, publicProjectForThread, depth + 1) : entry);
        if (data.some((entry, index) => entry !== sourceData[index])) {
            if (!changed)
                output = { ...output };
            output.data = data;
...
function nativeProjectChangedId(params) {
    if (!(0, types_1.isPlainRecord)(params))
        return null;
    if (validNativeProjectId(params.projectId))
        return params.projectId;
    return (0, types_1.isPlainRecord)(params.project) && validNativeProjectId(params.project.id) ? params.project.id : null;
}
function nativeThreadProjectUpdate(params) {
    if (!(0, types_1.isPlainRecord)(params) || !validNativeTargetId(params.threadId) || !(params.projectId === null || validNativeProjectId(params.projectId)))
        return null;
...
function nativeThreadSnapshot(value, expectedThreadId) {
    if (!(0, types_1.isPlainRecord)(value) || !(0, types_1.isPlainRecord)(value.thread) || value.thread.id !== expectedThreadId || !Array.isArray(value.thread.turns))
        return null;
    const turns = [];
    for (const turn of value.thread.turns) {
        if (!(0, types_1.isPlainRecord)(turn) || !validNativeTargetId(turn.id) || !Array.isArray(turn.items))
            return null;
        if (turn.items.some((item) => !(0, types_1.isPlainRecord)(item) || !validNativeTargetId(item.id)))
            return null;
        turns.push(turn);
    }
...
function nativeVisibleHistoryTurn(turn, marker) {
    if (!marker || !Array.isArray(turn.items))
        return turn;
...
function isExactNativeHandoffContextItem(item, marker) {
    if (!(0, types_1.isPlainRecord)(item) || item.type !== "userMessage" || !Array.isArray(item.content) || item.content.length !== 1)
        return false;
    ...
    return (0, types_1.isPlainRecord)(content) && content.type === "text" && typeof content.text === "string"
        && content.text.startsWith(`${marker}\nNative account history continuation.`);
}
function combinedNativeHistoryResult(method, params, rootThreadId, snapshots, offset, maxResultBytes) {
    const root = snapshots[0];
    if (!root || snapshots.length > 64 || !Number.isSafeInteger(offset) || offset < 0 || !Number.isSafeInteger(maxResultBytes) || maxResultBytes < 1024)
        return null;
    ...
    for (const turn of turns) {
        ...
        if (!validNativeTargetId(turnId) || identities.has(turnId))
            return null;
        ...
        for (const item of turnItems) {
            if (!(0, types_1.isPlainRecord)(item) || !validNativeTargetId(item.id) || identities.has(item.id))
                return null;
            identities.add(item.id);
        }
    }
    if (method === "thread/read") {
        if (!(0, types_1.isPlainRecord)(params) || (params.includeTurns !== undefined && typeof params.includeTurns !== "boolean"))
            return null;
    ...
    if (method === "thread/turns/list")
        return nativeMergedPage(ordered, paging.limit, offset, maxResultBytes);
    if (method === "thread/items/list") {
        const selected = paging.turnId === null ? ordered : ordered.filter((turn) => turn.id === paging.turnId);
        if (paging.turnId !== null && selected.length !== 1)
            return null;
 ...
 */
function nativeMergedPage(data, limit, offset, maxResultBytes) {
    ...
    if (page.length === 0 && offset < data.length)
        return null;
...
function nativeCombinedPaging(params) {
    if (!(0, types_1.isPlainRecord)(params) || (params.cursor !== undefined && params.cursor !== null)
        ...
        || (params.turnId !== undefined && params.turnId !== null && !validNativeTargetId(params.turnId)))
        return null;
    return {
...
function withResponseThreadId(response, publicThreadId) {
    if (!(0, types_1.isPlainRecord)(response.result))
        return response;
    const result = { ...response.result };
    if (typeof result.threadId === "string")
        result.threadId = publicThreadId;
    if (typeof result.thread_id === "string")
        result.thread_id = publicThreadId;
    if ((0, types_1.isPlainRecord)(result.thread) && typeof result.thread.id === "string") {
        result.thread = { ...result.thread, id: publicThreadId };
    }
    return { ...response, result };
}
/** Native mode deliberately preserves provider ids on the desktop wire. */
function withNativeResponseThreadId(response, nativeThreadId) {
    if (!(0, types_1.isPlainRecord)(response.result))
        return response;
    const result = { ...response.result };
    if (typeof result.threadId === "string")
        result.threadId = nativeThreadId;
    if (typeof result.thread_id === "string")
        result.thread_id = nativeThreadId;
    if ((0, types_1.isPlainRecord)(result.thread) && typeof result.thread.id === "string") {
        const thread = { ...result.thread, id: nativeThreadId };
        result.thread = thread;
    }
    return { ...response, result };
}
function withNotificationThreadId(notification, nativeThreadId) {
    ...
    if ((0, types_1.isPlainRecord)(params.thread) && typeof params.thread.id === "string") {
        params.thread = { ...params.thread, id: nativeThreadId };
        changed = true;
    }
    return changed ? { ...notification, params } : notification;
}
function withNativeNotificationThreadId(notification, nativeThreadId) {
    if (!(0, protocol_1.isNotification)(notification) || !(0, types_1.isPlainRecord)(notification.params))
        return notification;
    const params = { ...notification.params };
    let changed = false;
    ...
    if ((0, types_1.isPlainRecord)(params.thread) && typeof params.thread.id === "string") {
        const thread = { ...params.thread, id: nativeThreadId };
 ...
 */
function portableTurnStartInput(params) {
    ...
    for (const entry of params.input) {
        if (!(0, types_1.isPlainRecord)(entry) || typeof entry.type !== "string")
            return { kind: "invalid" };
        if (entry.type !== "text" && entry.type !== "input_text")
            return { kind: "nonportable" };
        if (Object.keys(entry).sort().join("\0") !== ["text", "type"].join("\0"))
            return { kind: "nonportable" };
        if (typeof entry.text !== "string" || entry.text.length === 0 || /[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f]/.test(entry.text))
            return { kind: "invalid" };
...
function validNativeTargetId(value) {
    return typeof value === "string" && value.length > 0 && value.length <= 512 && !/[\u0000-\u001f\u007f]/.test(value);
}
function canonicalThreadProjection(conversation, secret, includeTurns) {
    ...
    return {
        ...
        preview,
        projectId: null,
        sessionId: `session_${(0, node_crypto_1.createHmac)("sha256", secret).update(`renderer-session:v1:${conversation.conversationId}`, "utf8").digest("base64url")}`,
        source: "appServer",
...
function canonicalListOptions(value) {
    if (value !== undefined && !(0, types_1.isPlainRecord)(value))
        return null;
    const params = (value ?? {});
    ...
    if (!base || !optionalSearchTerm(params.searchTerm) || (params.sortKey !== undefined && params.sortKey !== null && !["created_at", "updated_at", "recency_at", "section_position"].includes(String(params.sortKey)))
        ...
        || !validCwdFilter(params.cwd) || !validStringArray(params.modelProviders) || !validStringArray(params.sourceKinds))
        return null;
    if (typeof params.ancestorThreadId === "string" && typeof params.parentThreadId === "string")
        return null;
...
function canonicalSearchOptions(value) {
    if (!(0, types_1.isPlainRecord)(value))
        return null;
    const allowed = new Set(["archived", "cursor", "limit", "searchTerm", "sortDirection", "sortKey", "sourceKinds"]);
    if (Object.keys(value).some((key) => !allowed.has(key)) || typeof value.searchTerm !== "string" || !optionalSearchTerm(value.searchTerm))
        return null;
    ...
    if (!base || (value.sortKey !== undefined && value.sortKey !== null && !["created_at", "updated_at", "recency_at"].includes(String(value.sortKey)))
        || (value.archived !== undefined && value.archived !== null && typeof value.archived !== "boolean") || !validStringArray(value.sourceKinds))
        return null;
    return {
        ...base,
...
function canonicalPagingOptions(params) {
    if ((params.cursor !== undefined && params.cursor !== null) || !nullableLimit(params.limit)
        || (params.sortDirection !== undefined && params.sortDirection !== null && params.sortDirection !== "asc" && params.sortDirection !== "desc"))
        return null;
    return { limit: typeof params.limit === "number" ? params.limit : null, sortDirection: params.sortDirection === "asc" ? "asc" : "desc" };
}
function canonicalTurnsListParams(value) {
    if (!(0, types_1.isPlainRecord)(value) || Object.keys(value).some((key) => !["cursor", "itemsView", "limit", "sortDirection", "threadId"].includes(key))
        || typeof value.threadId !== "string" || !canonicalPagingOptions(value)
        || (value.itemsView !== undefined && value.itemsView !== null && !["notLoaded", "summary", "full"].includes(String(value.itemsView))))
        return null;
...
function canonicalItemsListParams(value) {
    if (!(0, types_1.isPlainRecord)(value) || Object.keys(value).some((key) => !["cursor", "limit", "sortDirection", "threadId", "turnId"].includes(key))
        || typeof value.threadId !== "string" || !nullableString(value.turnId) || !canonicalPagingOptions(value))
        return null;
    return { threadId: value.threadId, turnId: typeof value.turnId === "string" ? value.turnId : null, ...canonicalPagingOptions(value) };
}
function canonicalListRows(rows, options) {
    if (options.empty)
        return [];
...
function orderedLimited(rows, sortDirection, limit) {
    const ordered = sortDirection === "asc" ? [...rows] : [...rows].reverse();
    return limit === null ? ordered : ordered.slice(0, limit);
}
function nullableString(value) { return value === undefined || value === null || (typeof value === "string" && value.length <= 512 && !/[\u0000-\u001f\u007f]/.test(value)); }
function optionalSearchTerm(value) { return value === undefined || value === null || (typeof value === "string" && value.length <= 240 && !/[\u0000-\u001f\u007f]/.test(value)); }
function nullableLimit(value) { return value === undefined || value === null || (typeof value === "number" && Number.isInteger(value) && value >= 0 && value <= 512); }
function validStringArray(value) { return value === undefined || value === null || (Array.isArray(value) && value.length <= 32 && value.every((item) => typeof item === "string" && item.length <= 160 && !/[\u0000-\u001f\u007f]/.test(item))); }
function validCwdFilter(value) { return value === undefined || value === null || nullableString(value) || (Array.isArray(value) && value.length <= 16 && value.every((item) => nullableString(item) && typeof item === "string")); }
function canonicalCwdMatch(value) { return value === undefined || value === null || value === "/private/var/empty" || (Array.isArray(value) && value.includes("/private/var/empty")); }
function canonicalThreadPreview(conversation) {
    return conversation.turns.flatMap((turn) => turn.items).find((item) => item.kind === "user")?.text ?? "";
}
function canonicalPublicTurnId(turnId, secret) {
    return `turn_${(0, node_crypto_1.createHmac)("sha256", secret).update(`renderer-turn:v1:${turnId}`, "utf8").digest("base64url")}`;
}
function canonicalPublicItemId(turnId, index, secret) {
    return `item_${(0, node_crypto_1.createHmac)("sha256", secret).update(`renderer-item:v1:${turnId}:${index}`, "utf8").digest("base64url")}`;
}
function canonicalTurnProjection(turn, secret, itemsView = "full") {
    const id = canonicalPublicTurnId(turn.turnId, secret);
    const itemId = (index) => canonicalPublicItemId(turn.turnId, index, secret);
    return {
        id,
        status: "completed",
        itemsView,
...
function canonicalThreadItem(item, id) {
    if (item.kind === "user")
        return { id, type: "userMessage", content: [{ type: "text", text: item.text }] };
    if (item.kind === "assistant")
        return { id, type: "agentMessage", text: item.text };
    if (item.kind === "plan")
        return { id, type: "plan", text: item.text };
    return { id, type: "functionCallOutput", name: item.name, output: item.result };
}
function isContinuationRequestMethod(method) {
...
function totalTokenUsage(value) {
    const source = (0, types_1.isPlainRecord)(value) && (0, types_1.isPlainRecord)(value.total) ? value.total : (0, types_1.isPlainRecord)(value) ? value : null;
    if (!source)
        return null;
...
function nativeTurnIdFrom(value) {
    if (!(0, types_1.isPlainRecord)(value))
        return null;
    if (typeof value.turnId === "string")
        return value.turnId;
    return (0, types_1.isPlainRecord)(value.turn) && typeof value.turn.id === "string" ? value.turn.id : null;
}
/** Owner-private native ids are scoped by account for late balance correlation. */
function balanceThreadKey(account, nativeThreadId) {
    return `${account}\u0000${nativeThreadId}`;
}
/** Renderer ref and request id identify one externally-originated automatic dispatch. */
function automaticCapacityRequestKey(client, request) {
...
function estimatedTurnTokens(params) {
    let bytes = 0;
    try {
        bytes = Buffer.byteLength(JSON.stringify(params ?? null), "utf8");
    }
    catch {
        bytes = 0;
    }
...
function compareNewWorkCapacity(left, leftAssigned, leftId, right, rightAssigned, rightId, leftIndex, rightIndex) {
    const candidate = (value, id, assigned, index) => ({
        opaqueAccountId: id,
    ...
    return (0, quota_2.compareQuotaCandidates)(candidate(left, leftId, leftAssigned, leftIndex), candidate(right, rightId, rightAssigned, rightIndex), Date.now());
}
/** Automatic routing consumes only current positive provider quota facts. */
function hasFreshPositiveQuota(quota) {
    const now = Date.now();
    ...
    return quota?.freshness === "fresh"
        ...
        && Number.isFinite(observedAt) && observedAt <= now && now - observedAt <= quota_2.QUOTA_STALE_AFTER_MS
...
function quotaNeedsRefresh(quota) {
    if (!quota || quota.freshness !== "fresh")
        return true;
    const now = Date.now();
    if (typeof quota.observedAt !== "number" || !Number.isFinite(quota.observedAt) || quota.observedAt > now || now - quota.observedAt > quota_2.QUOTA_STALE_AFTER_MS)
        return true;
    if (!quota.resetAt)
        return true;
...
function isDeviceStartResult(value) {
 ...
 */
function providerSafeProfileProjection(value) {
    if (!(0, types_1.isPlainRecord)(value))
        return null;
    const account = (0, types_1.isPlainRecord)(value.account) ? value.account : value;
    const profile = (0, types_1.isPlainRecord)(account.profile) ? account.profile : null;
...
function providerAuthenticated(value) {
    if (!(0, types_1.isPlainRecord)(value) || !(0, types_1.isPlainRecord)(value.account))
        return false;
    return (0, quota_2.parseAccountRead)(value, Date.now())?.health === "authenticated";
}
function firstSafePlan(...records) {
...
function firstMaskedEmail(...records) {
    for (const record of records) {
        if (!record)
            continue;
...
function maskProviderEmail(value) {
    if (typeof value !== "string" || value.length < 3 || value.length > 254 || /[\u0000-\u001f\u007f\s]/.test(value))
        return null;
    ...
    if (!/^[A-Za-z0-9.!#$%&'*+/=?^_`{|}~-]{1,64}$/.test(local)
        || !/^(?=.{1,80}$)[A-Za-z0-9](?:[A-Za-z0-9.-]*[A-Za-z0-9])?$/.test(domain))
        return null;
    const visible = local.slice(0, Math.min(3, local.length));
    return `${visible}${"*".repeat(Math.max(3, local.length - visible.length))}@${domain}`;
}
function firstSafeAvatarUrl(...records) {
    for (const record of records) {
        if (!record)
            continue;
 ...
 */
function providerOAuthHandoffUrl(value) {
    if (!(0, types_1.isPlainRecord)(value) || typeof value.authorizationUrl !== "string")
        return null;
    const raw = value.authorizationUrl;
    if (raw.length < 12 || raw.length > 2_048)
        return null;
    try {
...
function providerQuotaProjection(value) {
    const parsed = (0, quota_2.parseRateLimitsRead)(value, Date.now());
    if (!parsed || parsed.weeklyRemainingPercent === null || parsed.weeklyResetAt === null)
        return null;
    ...
    const nested = (0, types_1.isPlainRecord)(value) && (0, types_1.isPlainRecord)(value.rateLimits) ? value.rateLimits.resetCredits : null;
...
function providerConnectionRows(value, kind) {
    if (!(0, types_1.isPlainRecord)(value))
        return null;
    ...
    if (kind === "plugin" && value.marketplaceLoadErrors !== undefined
        && (!Array.isArray(value.marketplaceLoadErrors) || value.marketplaceLoadErrors.length > 0))
        return null;
    if (kind === "plugin" && Array.isArray(value.marketplaces)) {
        if (value.marketplaces.length > 128)
            return null;
        const entries = [];
        for (const marketplace of value.marketplaces) {
            if (!(0, types_1.isPlainRecord)(marketplace) || !Array.isArray(marketplace.plugins) || marketplace.plugins.length > 65_536)
                return null;
            entries.push(...marketplace.plugins.filter((plugin) => (0, types_1.isPlainRecord)(plugin) && plugin.installed === true));
        }
        raw = entries;
    }
    ...
    for (const row of raw) {
        if (!(0, types_1.isPlainRecord)(row))
            return null;
        ...
        if (!name || name.length > 256 || /[\u0000-\u001f\u007f]/.test(name) || seen.has(name))
            return null;
        seen.add(name);
        ...
        const displayLabel = (kind === "plugin" && (0, types_1.isPlainRecord)(row.interface) ? (0, broker_1.safeConnectionDisplayLabel)(row.interface.displayName) : undefined)
            ?? (0, broker_1.safeConnectionDisplayLabel)(row.displayName) ?? (0, broker_1.safeConnectionDisplayLabel)(row.name)
            ?? (kind === "app" ? (0, broker_1.safeConnectionDisplayLabel)(row.runtimeName) : undefined);
        output.push({ name, ...(displayLabel ? { displayLabel } : {}), status });
    }
    return output;
}
function providerConnectionKey(account, kind, definitionRef) {
 ...
 */
function recoverEnrollmentMaterialization(root, fallbackConfig, secret) {
    ...
    if (!journal)
        return readCommittedRouterConfig(root) ?? fallbackConfig;
    ...
    if (!diskConfig || !state)
        throw new Error("accounts broker enrollment recovery requires complete prior state");
    ...
    if (!configPhase || !statePhase
        || !(0, state_store_1.validateRouterState)(state, statePhase === "prior" ? priorConfig : nextConfig)) {
        throw new Error("accounts broker enrollment recovery refused mixed state");
    }
    const source = (0, node_path_1.join)(root, "enrollments", journal.enrollmentRef);
    const target = (0, node_path_1.join)(root, "accounts", journal.opaqueAccountId);
    const sourcePresent = isPrivateEnrollmentDirectory(source);
    const targetPresent = isPrivateEnrollmentDirectory(target);
    if (sourcePresent && targetPresent)
        throw new Error("accounts broker enrollment recovery found duplicate home");
    if (sourcePresent) {
        ...
        if (configPhase !== "prior" || statePhase !== "prior")
            throw new Error("accounts broker enrollment home missing after publication");
    ...
    if (statePhase === "prior") {
        const migrated = (0, state_store_1.migrateIdleRouterStateV3)(state, nextConfig);
        ...
        (0, state_store_1.writePrivateJsonAtomic)(root, "router-state.json", migrated);
    }
    if (configPhase === "prior")
        (0, state_store_1.writePrivateJsonAtomic)(root, config_1.ACCOUNT_ROUTER_CONFIG_FILE, nextConfig);
...
function prepareEnrollmentNativeExtension(root, journal, secret) {
    ...
    if (extension.prepared) {
        ...
        if (recovery.state === "invalid")
            throw new Error("native enrollment extension proof invalid");
        return;
    }
    const codexHome = (0, node_path_1.join)(root, "accounts", journal.opaqueAccountId, "codex-home");
    const sqliteHome = (0, node_path_1.join)(root, "accounts", journal.opaqueAccountId, "sqlite-home");
    const raw = (0, app_server_mux_1.readOwnerPrivateAuthAccountId)(codexHome);
    if (!raw)
...
function publishEnrollmentNativeExtension(root, journal, secret) {
    const extension = journal.nativeExtension;
    if (!extension)
        return;
    if (!extension.prepared)
...
function writeEnrollmentMaterializationJournal(root, journal) {
    if (!isEnrollmentMaterializationJournal(journal))
        throw new Error("invalid accounts broker enrollment journal");
    const bytes = Buffer.byteLength(JSON.stringify(journal), "utf8");
    if (bytes > MAX_ENROLLMENT_MATERIALIZATION_JOURNAL_BYTES)
        throw new Error("accounts broker enrollment journal exceeds its bound");
    (0, state_store_1.writePrivateJsonAtomic)(root, exports.ENROLLMENT_MATERIALIZATION_JOURNAL_FILE, journal);
}
function readEnrollmentMaterializationJournal(root) {
    const path = (0, node_path_1.join)(root, exports.ENROLLMENT_MATERIALIZATION_JOURNAL_FILE);
    if (!(0, node_fs_1.existsSync)(path))
        return null;
    (0, state_store_1.assertPrivateRegularFile)(path, MAX_ENROLLMENT_MATERIALIZATION_JOURNAL_BYTES);
    const raw = (0, node_fs_1.readFileSync)(path, "utf8");
    if (Buffer.byteLength(raw, "utf8") > MAX_ENROLLMENT_MATERIALIZATION_JOURNAL_BYTES)
        throw new Error("accounts broker enrollment journal exceeds its bound");
    let parsed;
    try {
        ...
        throw new Error("accounts broker enrollment journal is unreadable");
    }
    if (!isEnrollmentMaterializationJournal(parsed))
        throw new Error("accounts broker enrollment journal is invalid");
    return parsed;
}
function clearEnrollmentMaterializationJournal(root) {
    const path = (0, node_path_1.join)(root, exports.ENROLLMENT_MATERIALIZATION_JOURNAL_FILE);
    if (!(0, node_fs_1.existsSync)(path))
        return;
    (0, state_store_1.assertPrivateRegularFile)(path, MAX_ENROLLMENT_MATERIALIZATION_JOURNAL_BYTES);
...
function readCommittedRouterConfig(root) {
    const path = (0, node_path_1.join)(root, config_1.ACCOUNT_ROUTER_CONFIG_FILE);
    try {
        (0, state_store_1.assertPrivateRegularFile)(path, MAX_ENROLLMENT_MATERIALIZATION_JOURNAL_BYTES);
        return validatedRouterConfigV3(JSON.parse((0, node_fs_1.readFileSync)(path, "utf8")));
    }
    catch {
        return null;
    }
}
function readCommittedRouterState(root) {
    const path = (0, node_path_1.join)(root, "router-state.json");
    try {
        (0, state_store_1.assertPrivateRegularFile)(path, 2 * 1024 * 1024);
        const parsed = JSON.parse((0, node_fs_1.readFileSync)(path, "utf8"));
        return (0, types_1.isPlainRecord)(parsed) ? parsed : null;
    }
    catch {
        return null;
    }
}
function validatedRouterConfigV3(value) {
    const parsed = (0, config_1.validateRouterConfig)(value);
    return parsed?.schemaVersion === 3 ? parsed : null;
}
function isEnrollmentMaterializationJournal(value) {
    if (!(0, types_1.isPlainRecord)(value) || Object.keys(value).sort().join("\0") !== [
    ...
    if (value.version !== 1 || typeof value.enrollmentRef !== "string" || !/^be_[A-Za-z0-9_-]{16,128}$/.test(value.enrollmentRef)
        || !(0, types_1.isOpaqueAccountId)(value.opaqueAccountId)
    ...
    if (!priorConfig || !nextConfig || !nextConfig.accounts.some((account) => account.opaqueAccountId === value.opaqueAccountId)
        || priorConfig.accounts.some((account) => account.opaqueAccountId === value.opaqueAccountId))
        return false;
    const digest = (candidate) => typeof candidate === "string" && /^sha256:[a-f0-9]{64}$/.test(candidate);
...
function nativeInventoryConsumer(method) {
...
function isNativePluginInventory(value, root) {
    if (!(0, types_1.isPlainRecord)(value) || Object.keys(value).sort().join() !== "native_base_root,plugins,schema_version"
        || value.schema_version !== 1 || value.native_base_root !== root || !Array.isArray(value.plugins) || value.plugins.length > 4096)
        return false;
    let previous = "";
    for (const plugin of value.plugins) {
        if (!(0, types_1.isPlainRecord)(plugin) || Object.keys(plugin).sort().join() !== "enabled,id,version"
            ...
            || (plugin.version !== null && (typeof plugin.version !== "string" || !plugin.version || Buffer.byteLength(plugin.version) > 256)))
            return false;
        previous = plugin.id;
    }
    return Buffer.byteLength(JSON.stringify(value)) < NATIVE_INVENTORY_MAX_BYTES;
}
function nativePluginInventoryFromResponse(value, root) {
    if (!(0, types_1.isPlainRecord)(value) || !Array.isArray(value.marketplaces) || value.marketplaces.length > 128
        || !Array.isArray(value.marketplaceLoadErrors) || value.marketplaceLoadErrors.length !== 0)
        return null;
    const plugins = [];
    for (const marketplace of value.marketplaces) {
        if (!(0, types_1.isPlainRecord)(marketplace) || !Array.isArray(marketplace.plugins))
            return null;
        if (marketplace.name !== "openai-curated-remote")
            continue;
        if (marketplace.plugins.length > 4096)
            return null;
        for (const plugin of marketplace.plugins) {
            if (!(0, types_1.isPlainRecord)(plugin) || typeof plugin.installed !== "boolean")
                return null;
            if (!plugin.installed)
                continue;
            plugins.push({ id: plugin.id, enabled: plugin.enabled, version: plugin.version });
        }
    }
    if (plugins.some((plugin) => typeof plugin.id !== "string"))
        return null;
    plugins.sort((a, b) => a.id < b.id ? -1 : a.id > b.id ? 1 : 0);
    const inventory = { schema_version: 1, native_base_root: root, plugins };
    return isNativePluginInventory(inventory, root) ? inventory : null;
}
function durableDigest(value) {
    return `sha256:${(0, node_crypto_1.createHash)("sha256").update(stableJson(value), "utf8").digest("hex")}`;
}
...
function isPrivateEnrollmentDirectory(path) {
    try {
...
function assertPrivateEnrollmentDirectory(path) {
    if (!isPrivateEnrollmentDirectory(path))
        throw new Error("accounts broker enrollment directory is unsafe");
}
function validateCommittedEnrollmentHome(target, opaqueAccountId, secret) {
    try {
        const codexHome = (0, node_path_1.join)(target, "codex-home");
        const sqliteHome = (0, node_path_1.join)(target, "sqlite-home");
        if (!isPrivateEnrollmentDirectory(target) || !isPrivateEnrollmentDirectory(codexHome) || !isPrivateEnrollmentDirectory(sqliteHome))
            return false;
        const configPath = (0, node_path_1.join)(codexHome, "config.toml");
        (0, state_store_1.assertPrivateRegularFile)(configPath, 4 * 1024);
        if ((0, node_fs_1.readFileSync)(configPath, "utf8") !== "")
            return false;
        const rawAccountId = (0, app_server_mux_1.readOwnerPrivateAuthAccountId)(codexHome);
        if (!rawAccountId)
            return false;
        ...
        return expected === opaqueAccountId
            && (0, app_server_mux_1.sharedSkillsHomeMatches)((0, node_path_1.dirname)((0, node_path_1.dirname)(target)), codexHome)
            && (0, app_server_mux_1.sharedPluginsHomeMatches)((0, node_path_1.dirname)((0, node_path_1.dirname)(target)), codexHome);
    }
    catch {
        return false;
    }
}
function parseOwnerArguments(argv) {
...
function isAppHandshakeResult(value) {
    return (0, types_1.isPlainRecord)(value) && Object.keys(value).sort().join("\0") === ["kind", "ok", "version"].join("\0")
        && value.version === 1 && value.kind === "handshake" && typeof value.ok === "boolean";
}
function isAppMessage(value) {
    return (0, types_1.isPlainRecord)(value) && Object.keys(value).sort().join("\0") === ["kind", "message", "version"].join("\0")
        && value.version === 1 && value.kind === "message" && isJsonRpcMessage(value.message);
}
function isJsonRpcMessage(value) {
    return (0, types_1.isPlainRecord)(value) && ((typeof value.method === "string" && (value.id === undefined || typeof value.id === "string" || typeof value.id === "number"))
        || (value.method === undefined && (typeof value.id === "string" || typeof value.id === "number" || value.id === null)));
}
function writeAppFrame(socket, frame, maxFrameBytes) {
    try {
...
function appDesktopMessageFits(message) {
    try {
        return Buffer.byteLength(JSON.stringify({ version: 1, kind: "message", message }), "utf8") + 1 <= exports.ACCOUNTS_BROKER_APP_SERVER_MAX_FRAME_BYTES;
    }
    catch {
        return false;
    }
...
async function removeStaleAppSocket(path) {
    if (!(0, node_fs_1.existsSync)(path))
        return;
    assertPrivateSocket(path);
    if (await socketLive(path))
        throw new Error("accounts broker app socket is already active");
    (0, node_fs_1.unlinkSync)(path);
}
function socketLive(path) {
    return new Promise((resolvePromise) => {
        const socket = (0, node_net_1.createConnection)(path);
        let settled = false;
        const finish = (value) => {
...
function isReadOnlyNativeAccountMethod(method) {
...
function credentialStoreArgs(args, store) {
    const result = [];
    for (let i = 0; i < args.length; i += 1) {
        const arg = args[i];
        if ((arg === "-c" || arg === "--config") && /^cli_auth_credentials_store\s*=/.test(args[i + 1] ?? "")) {
            i += 1;
            continue;
        }
        if (/^--config=cli_auth_credentials_store\s*=/.test(arg))
            continue;
        result.push(arg);
    }
    return [...result, "-c", `cli_auth_credentials_store="${store}"`];
}
function isolatedAuthMutation(method, params) {
    if (["account/login/start", "account/login/cancel", "account/logout"].includes(method))
        return true;
    ...
    return !Array.isArray(edits) || edits.some((edit) => !(0, types_1.isPlainRecord)(edit) || edit.keyPath === "cli_auth_credentials_store" || edit.keyPath === "forced_chatgpt_workspace_id");
}
class NativeAuthHelperBusyError extends Error {
    constructor() { super("native authentication helper busy"); }
}
//# sourceMappingURL=broker-host.js.map