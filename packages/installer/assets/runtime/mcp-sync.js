...
exports.hasStrayManagedMcpEndMarker = hasStrayManagedMcpEndMarker;
...
function planManagedMcpReconciliation(tweaks, currentToml = "", options = {}) {
    ...
    const reservedKeyPattern = new RegExp(`(^|[\\r\\n{,])\\s*(?:${exports.RESERVED_MANAGED_MCP_ENV_KEYS.join("|")})\\s*=`);
    const ownedLegacyNames = new Set();
    for (const tweak of ownedByName.values()) {
        const legacy = legacyMcpServerName(tweak.manifest.id);
        if (legacy)
            ownedLegacyNames.add(legacy);
    }
    const residueBaseNames = new Set();
    for (const table of manualTables) {
        if (!reservedKeyPattern.test(table.body))
            continue;
    ...
    for (const table of manualTables) {
        if (residueBaseNames.has(table.name.split(".")[0]))
            rangesToRemove.push(table);
    }
    const tables = manualTables.filter((table) => !residueBaseNames.has(table.name.split(".")[0]));
    for (const [canonicalName, tweak] of ownedByName) {
        ...
        entries.push(formatMcpServer(canonicalName, tweak.manifest.id, tweak.dir, mcp, preservedOptions[canonicalName], tweak.dataDir));
    }
...
function stripManagedMcpBlock(toml) {
    ...
    const consumePrecedingNewline = (start) => {
 ...
 */
function hasStrayManagedMcpEndMarker(toml) {
    const startMarkers = new Set([exports.MCP_MANAGED_START, LEGACY_MCP_MANAGED_START]);
    const endMarkers = new Set([exports.MCP_MANAGED_END, LEGACY_MCP_MANAGED_END]);
    for (const line of classifyTomlLines(toml)) {
        if (!line.structural)
            continue;
        const marker = line.text.trim();
...
function normalizeMcpServer(value) {
    ...
    if (value.env !== undefined) {
        ...
        if (Object.values(value.env).some((envValue) => typeof envValue !== "string"))
            return null;
        const reserved = exports.RESERVED_MANAGED_MCP_ENV_KEYS.find((key) => Object.hasOwn(value.env, key));
        if (reserved)
            throw new Error(`MCP environment variable ${reserved} is reserved for Tweakers`);
    }
    return value;
}
...
function managedMcpEnvironment(tweakId, tweakDir, declared, dataDir) {
    if (exports.RESERVED_MANAGED_MCP_ENV_KEYS.some((key) => Object.hasOwn(declared, key))) {
...
function managedMcpDataDir(tweakId, tweakDir, explicit) {
    if (explicit !== undefined) {
        if (!(0, node_path_1.isAbsolute)(explicit) || explicit.includes("\0")) {
            throw new Error(`Tweak ${tweakId} has an invalid managed MCP data directory`);
        }
        return (0, node_path_1.resolve)(explicit);
    }
    return (0, node_path_1.resolve)(tweakDir, "..", "..", "tweak-data", tweakId);
}
function withoutExactReservedManagedEnv(observed, injected) {
    const comparable = { ...observed };
    for (const key of exports.RESERVED_MANAGED_MCP_ENV_KEYS) {
        if (!Object.hasOwn(comparable, key))
            continue;
        if (comparable[key] !== injected[key])
            return null;
