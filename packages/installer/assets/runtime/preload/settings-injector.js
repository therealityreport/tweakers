...
function renderIndependentSettingsSurface(sectionsWrap, projection, subtitle, cleanups) {
    if (subtitle) {
...
function renderIndependentDoctorSummary(sectionsWrap, cleanups) {
    const section = document.createElement("section");
    section.className = "flex flex-col gap-2";
    ...
    const render = (report, error) => {
        ...
        if (!report) {
            ...
            if (error)
                card.appendChild(rowSimple("Error", String(error)));
        }
        else {
            ...
            if (error)
                updates.querySelector("[data-tweaker-row-actions]")?.appendChild(statusBadge("warn", "Stale"));
            card.appendChild(updates);
            if (health !== "Healthy")
                card.appendChild(rowSimple("Health", "Open Manager to see the exact finding and available recovery action."));
        }
        ...
        const actions = manager.querySelector("[data-tweaker-row-actions]");
        const openManager = compactButton("Open Manager", () => {
            openManager.disabled = true;
            void electron_1.ipcRenderer.invoke("tweaker:manager-open")
                .then(() => {
    ...
    const refresh = () => {
        ...
        void electron_1.ipcRenderer.invoke("tweaker:doctor-status")
            .then((value) => {
            lastReport = value;
            lastError = null;
            render(lastReport);
...
function renderEnvironmentSection(sectionsWrap, cardUpdates) {
    ...
    async function commitPreparedEnvironment(receipt) {
        ...
        let result;
        try {
            result = await electron_1.ipcRenderer.invoke("tweaker:commit-environment", {
    ...
    function appendEnvironmentTransactionRow() {
        ...
        const stalePreparedGeneration = (0, environment_config_controller_1.preparedEnvironmentReceiptNeedsFreshV2Preparation)(receipt, environment?.cacheV2);
        if (stalePreparedGeneration) {
            card.appendChild(rowSimple("Fresh preparation required", "The submitted confirmation refers to a stale sealed-pair generation. Cancel it and prepare again; it will not switch automatically."));
        }
    ...
    const draw = () => {
        ...
        if (!selected || !environment) {
            ...
            if (environment?.cacheV2)
                card.appendChild(environmentModeCacheRow(environment.cacheV2));
...
function environmentModeCacheRow(cache) {
    ...
    const staleSafety = cache.state === "stale"
        ...
        : "";
    const row = actionRow("Sealed pair cache", [cache.state, generation, roles, preparation, pin, supersession, timings, reason].filter((value) => Boolean(value)).join(" · ") + staleSafety);
    const left = row.firstElementChild;
    if (left) {
        left.prepend(statusBadge(cache.state === "ready" ? "ok" : cache.state === "unavailable" ? "warn" : "warn", cache.state === "ready" ? "Ready" : environmentTransactionLabel(cache.state)));
    }
...
function normalizeEnvironmentModeCacheStatus(value) {
    if (!value || typeof value !== "object")
        return undefined;
    const candidate = value;
    if (candidate.schemaVersion !== 2
        ...
        || (candidate.preparation.generationId !== null && typeof candidate.preparation.generationId !== "string"))
        return undefined;
    ...
    if (roles !== null && roles !== undefined
        && (roles.live?.role !== "live"
            ...
            || !isEnvironmentModeExperience(roles.inactive?.experience)))
        return undefined;
    return {
        schemaVersion: 2,
        ...
        preparation: {
            ...
            phase: candidate.preparation.phase,
        },
...
function isEnvironmentModeExperience(value) {
...
function renderTweakersRuntimeRefreshSection(sectionsWrap, cardUpdates) {
    const section = document.createElement("section");
    section.className = "flex flex-col gap-2";
    ...
    const draw = () => {
        card.textContent = "";
        const row = actionRow("ChatGPT Tweaker mode", detail ?? "Refreshes the separately selected ChatGPT Tweaker mode from its manager-verified source.");
        const actions = row.querySelector("[data-tweaker-row-actions]");
        const button = compactButton("Refresh Tweaker Mode", () => {
            if (busy)
                return;
            busy = true;
        ...
        actions?.appendChild(button);
        row.querySelector("[data-tweaker-row-actions]")?.prepend(statusBadge("ok", "Manager ready"));
        card.appendChild(row);
