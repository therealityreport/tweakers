...
@interface TweakerMetalRenderer : NSObject <MTKViewDelegate>
@property(nonatomic, strong) id<MTLCommandQueue> queue;
- (instancetype)initWithDevice:(id<MTLDevice>)device;
@end

@implementation TweakerMetalRenderer
- (instancetype)initWithDevice:(id<MTLDevice>)device {
  self = [super init];
  if (self) {
...
@interface TweakerNativeInstance : NSObject
@property(nonatomic, copy) NSString *identifier;
@property(nonatomic, strong) NSWindow *window;
@property(nonatomic, strong) MTKView *metalView;
@property(nonatomic, strong) TweakerMetalRenderer *renderer;
@property(nonatomic, assign) BOOL disposed;
- (instancetype)initWithIdentifier:(NSString *)identifier
                      parentWindow:(NSWindow *)parentWindow
                            bounds:(TweakerBounds)bounds
                       transparent:(BOOL)transparent
                  passthroughMouse:(BOOL)passthroughMouse
                             metal:(BOOL)metal;
- (void)setBounds:(TweakerBounds)bounds;
- (void)show;
- (void)hide;
...
@implementation TweakerNativeInstance
- (instancetype)initWithIdentifier:(NSString *)identifier
                      parentWindow:(NSWindow *)parentWindow
                            bounds:(TweakerBounds)bounds
                       transparent:(BOOL)transparent
  ...
  if (metal) {
    ...
    if (device) {
      ...
      TweakerMetalRenderer *renderer = [[TweakerMetalRenderer alloc] initWithDevice:device];
...
static NSMutableDictionary<NSString *, TweakerNativeInstance *> *TweakerInstances() {
  static NSMutableDictionary<NSString *, TweakerNativeInstance *> *instances;
...
static TweakerBounds GetBounds(napi_env env, napi_value object, TweakerBounds fallback) {
  napi_value bounds;
...
static TweakerNativeInstance *GetWrappedInstance(napi_env env, napi_value thisArg) {
  void *data = nullptr;
  if (napi_unwrap(env, thisArg, &data) != napi_ok || !data) return nil;
  return (__bridge TweakerNativeInstance *)data;
}
...
static void FinalizeInstance(napi_env env, void *data, void *hint) {
  ...
  (void)hint;
  if (!data) return;
  TweakerNativeInstance *instance = CFBridgingRelease(data);
  [instance dispose];
}
...
static napi_value InstanceSetBounds(napi_env env, napi_callback_info info) {
  ...
  TweakerNativeInstance *instance = GetWrappedInstance(env, thisArg);
  if (!instance || argc < 1) return Undefined(env);
  TweakerBounds bounds = GetBounds(env, args[0], {0, 0, 480, 320});
  RunOnMainSync(^{
...
static napi_value InstanceDispose(napi_env env, napi_callback_info info) {
  ...
  if (instance) {
    RunOnMainSync(^{
      [instance dispose];
      [TweakerInstances() removeObjectForKey:instance.identifier];
    });
  }
  return Undefined(env);
}
...
static napi_value WrapInstance(napi_env env, TweakerNativeInstance *instance) {
  napi_value object;
...
static napi_value CreateNativeWindow(napi_env env, napi_callback_info info, BOOL metal) {
  ...
  __block TweakerNativeInstance *instance = nil;
  ...
  RunOnMainSync(^{
    ...
    NSString *identifier = [NSString stringWithFormat:@"tweaker-native-%llu", sequence];
    TweakerBounds bounds = GetBounds(env, options, {0, 0, 480, 320});
    ...
    instance = [[TweakerNativeInstance alloc] initWithIdentifier:identifier
                                                    parentWindow:parent
                                                          bounds:bounds
                                                ...
                                                passthroughMouse:passthrough
                                                           metal:metal];
    if (instance) {
      TweakerInstances()[identifier] = instance;
    }
  });
...
static napi_value DisposeAll(napi_env env, napi_callback_info info) {
  (void)info;
  RunOnMainSync(^{
    NSArray<TweakerNativeInstance *> *instances = TweakerInstances().allValues;
    for (TweakerNativeInstance *instance in instances) {
      [instance dispose];
    }
    [TweakerInstances() removeAllObjects];
  });
  return Undefined(env);
}
