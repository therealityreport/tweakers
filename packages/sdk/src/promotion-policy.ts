/**
 * Policy-bearing slices of Codex's global state that promotion health proves.
 *
 * The desktop process is allowed to persist unrelated UI/session state while
 * it starts. Promotion must nevertheless detect every change to the complete
 * User Questions policy slots: local agent mode, every thread's active
 * permission profile, approval policy, sandbox policy, reviewer and workspace
 * roots, plus the exact top-level MCP-form control.
 */
export const PROMOTION_POLICY_FILE_MAX_BYTES = 10 * 1024 * 1024;
export const PROMOTION_POLICY_CANONICAL_MAX_CHARS = 12 * 1024 * 1024;
export const PROMOTION_POLICY_MAX_DEPTH = 128;
export const PROMOTION_POLICY_MAX_NODES = 250_000;

export const PROMOTION_POLICY_PERSISTED_ATOMS_KEY = "electron-persisted-atom-state";
export const PROMOTION_POLICY_AGENT_MODES_KEY = "agent-mode-by-host-id";
export const PROMOTION_POLICY_THREAD_PERMISSIONS_KEY = "heartbeat-thread-permissions-by-id";
export const PROMOTION_POLICY_MCP_FORM_KEY = "electron-openai-mcp-form-elicitations-enabled";
export const PROMOTION_POLICY_HASH_DOMAIN = "tweakers-promotion-policy-v1\0";

interface CanonicalBudget {
  nodes: number;
}

/** Parse and canonicalize only actual policy-bearing global-state semantics. */
export function canonicalPromotionPolicyText(raw: string): string {
  ...
  return canonicalPromotionPolicyState(parsed);
}

/**
 * Produce a stable, key-order-independent representation. Complete selected
 * records are retained, so any nested policy or per-thread change is visible.
 */
export function canonicalPromotionPolicyState(value: unknown): string {
  const root = requireRecord(value, "Promotion policy state root");
  const atomsSlot = policySlot(root, PROMOTION_POLICY_PERSISTED_ATOMS_KEY);
  const atoms = atomsSlot.present
    ? requireRecord(atomsSlot.value, "Promotion persisted atom state")
    : null;
  const modesSlot = atoms === null
    ? { present: false as const }
    : policyRecordSlot(atoms, PROMOTION_POLICY_AGENT_MODES_KEY, "Promotion agent-mode state");
  ...
  const mcpFormElicitationsEnabled = policySlot(root, PROMOTION_POLICY_MCP_FORM_KEY);
...
function projectThreadPermissions(
  ...
  const slot = policyRecordSlot(
    atoms,
    PROMOTION_POLICY_THREAD_PERMISSIONS_KEY,
    "Promotion thread-permission state",
  );
