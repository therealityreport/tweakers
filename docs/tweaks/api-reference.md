...
This is the full public API surface exported by `@therealityreport/tweakers-sdk`.
...
It does not check that the entry file exists; `tweaker validate-tweak`
...
Declares an MCP server managed by Tweaker. See [MCP servers](./mcp.md).
...
```text
localStorage["tweaker:storage:<tweak-id>"]
```
...
[Owl runtime surface](../OWL-RUNTIME.md) for the private upstream APIs Tweaker
currently observes.
...
When `route` is provided, Tweaker loads the Codex app route through
...
Tweaker prefers Owl's private `contentView.addChildView(view.webContentsView)`
...
CDP is opt-in. Tweaker reports status but does not silently enable it.
...
`path` is resolved inside the tweak directory. The file must exist, and Tweaker
...
For `.node` modules, Tweaker calls `module.request(method, payload)` when
...
Tweaker uses its built-in native host and defaults `factory` to `createPanel`.
The runtime supplies the parent native window handle to the module when
available. After creation,
Tweaker forwards parent window bounds, focus, visibility, fullscreen, minimize,
...
When `moduleId` is omitted, Tweaker uses its built-in native host and defaults
...
Native APIs are main-process backed. Renderer tweaks call them through Tweaker
