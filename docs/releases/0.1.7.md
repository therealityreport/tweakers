...
- Added `tweaker install --local` and `tweaker repair --local` as an opt-in stable local macOS signing path. Ad-hoc signing remains the default to avoid Keychain identity prompts.
- Added a solid blue Update pill in the Tweaker Settings sidebar that opens the GitHub Releases page.
- Added a Tweak Store sidebar badge showing how many installed tweaks have newer approved versions.
- Cleaned up Windows uninstall by removing Tweaker Explorer context-menu entries.
...
```sh
bun install -g github:b-nnett/tweaker#0.1.7
