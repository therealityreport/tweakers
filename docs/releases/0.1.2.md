...
Tweaker 0.1.2 is a resilience and tooling release. It fixes the Codex `26.429.20946` startup breakage, makes repair failures visible, and adds the first round of tweak developer tooling.
...
- Reworked the macOS self-repair patcher around a fingerprint-based hook so minified Codex internals are less likely to break Tweaker on future updates.
- Added macOS repair-failure alerts with Dismiss and Report on GitHub actions.
- Added a Tweaker Config page health check for the auto-repair watcher, including a Check Now action.
...
If Tweaker is already installed, update from the repo or your existing install path:
