...
if (run.error) {
  console.error(`[!] Could not start tweaker: ${run.error.message}`);
  process.exit(1);
}
process.exit(run.status ?? 1);

function bootstrap() {
  console.error("[tweaker] First run from source install; building CLI...");
  runOrExit(npmCommand(), installArgs(), "install tweaker dependencies");
  runOrExit(npmCommand(), ["run", "build"], "build tweaker");
}
